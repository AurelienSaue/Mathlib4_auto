...
extends subsemiring R, add_subgroup R
where
...
protected def mk' {R : Type u} [ring R] (s : set R) (sm : submonoid R) (sa : add_subgroup R) (hm : ↑sm = s) (ha : ↑sa = s) : subring R :=
  mk s sorry sorry sorry sorry sorry

@[simp] theorem coe_mk' {R : Type u} [ring R] {s : set R} {sm : submonoid R} (hm : ↑sm = s) {sa : add_subgroup R} (ha : ↑sa = s) : ↑(subring.mk' s sm sa hm ha) = s :=
  rfl

@[simp] theorem mem_mk' {R : Type u} [ring R] {s : set R} {sm : submonoid R} (hm : ↑sm = s) {sa : add_subgroup R} (ha : ↑sa = s) {x : R} : x ∈ subring.mk' s sm sa hm ha ↔ x ∈ s :=
  iff.rfl

@[simp] theorem mk'_to_submonoid {R : Type u} [ring R] {s : set R} {sm : submonoid R} (hm : ↑sm = s) {sa : add_subgroup R} (ha : ↑sa = s) : to_submonoid (subring.mk' s sm sa hm ha) = sm :=
  submonoid.ext' (Eq.symm hm)

@[simp] theorem mk'_to_add_subgroup {R : Type u} [ring R] {s : set R} {sm : submonoid R} (hm : ↑sm = s) {sa : add_subgroup R} (ha : ↑sa = s) : to_add_subgroup (subring.mk' s sm sa hm ha) = sa :=
...
protected theorem subring.exists {R : Type u} [ring R] {s : subring R} {p : ↥s → Prop} : (∃ (x : ↥s), p x) ↔ ∃ (x : R), ∃ (H : x ∈ s), p { val := x, property := H } :=
  set_coe.exists

protected theorem subring.forall {R : Type u} [ring R] {s : subring R} {p : ↥s → Prop} : (∀ (x : ↥s), p x) ↔ ∀ (x : R) (H : x ∈ s), p { val := x, property := H } :=
  set_coe.forall
...
protected theorem ext'_iff {R : Type u} [ring R] {s : subring R} {t : subring R} : s = t ↔ ↑s = ↑t :=
  { mp := fun (h : s = t) => h ▸ rfl, mpr := fun (h : ↑s = ↑t) => ext' h }

/-- Two subrings are equal if they have the same elements. -/
theorem ext {R : Type u} [ring R] {S : subring R} {T : subring R} (h : ∀ (x : R), x ∈ S ↔ x ∈ T) : S = T :=
  ext' (set.ext h)

/-- A subring contains the ring's 1. -/
theorem one_mem {R : Type u} [ring R] (s : subring R) : 1 ∈ s :=
  one_mem' s

/-- A subring contains the ring's 0. -/
theorem zero_mem {R : Type u} [ring R] (s : subring R) : 0 ∈ s :=
  zero_mem' s
...
theorem neg_mem {R : Type u} [ring R] (s : subring R) {x : R} : x ∈ s → -x ∈ s :=
  neg_mem' s

/-- A subring is closed under subtraction -/
theorem sub_mem {R : Type u} [ring R] (s : subring R) {x : R} {y : R} (hx : x ∈ s) (hy : y ∈ s) : x - y ∈ s :=
...
theorem list_prod_mem {R : Type u} [ring R] (s : subring R) {l : List R} : (∀ (x : R), x ∈ l → x ∈ s) → list.prod l ∈ s :=
  submonoid.list_prod_mem (to_submonoid s)

/-- Sum of a list of elements in a subring is in the subring. -/
theorem list_sum_mem {R : Type u} [ring R] (s : subring R) {l : List R} : (∀ (x : R), x ∈ l → x ∈ s) → list.sum l ∈ s :=
...
theorem multiset_prod_mem {R : Type u_1} [comm_ring R] (s : subring R) (m : multiset R) : (∀ (a : R), a ∈ m → a ∈ s) → multiset.prod m ∈ s :=
  submonoid.multiset_prod_mem (to_submonoid s) m

/-- Sum of a multiset of elements in an `subring` of a `ring` is
in the `subring`. -/
theorem multiset_sum_mem {R : Type u_1} [ring R] (s : subring R) (m : multiset R) : (∀ (a : R), a ∈ m → a ∈ s) → multiset.sum m ∈ s :=
...
/-- Product of elements of a subring of a `comm_ring` indexed by a `finset` is in the
    subring. -/
theorem prod_mem {R : Type u_1} [comm_ring R] (s : subring R) {ι : Type u_2} {t : finset ι} {f : ι → R} (h : ∀ (c : ι), c ∈ t → f c ∈ s) : (finset.prod t fun (i : ι) => f i) ∈ s :=
  submonoid.prod_mem (to_submonoid s) h

/-- Sum of elements in a `subring` of a `ring` indexed by a `finset`
is in the `subring`. -/
theorem sum_mem {R : Type u_1} [ring R] (s : subring R) {ι : Type u_2} {t : finset ι} {f : ι → R} (h : ∀ (c : ι), c ∈ t → f c ∈ s) : (finset.sum t fun (i : ι) => f i) ∈ s :=
...
@[simp] theorem coe_add {R : Type u} [ring R] (s : subring R) (x : ↥s) (y : ↥s) : ↑(x + y) = ↑x + ↑y :=
  rfl

@[simp] theorem coe_neg {R : Type u} [ring R] (s : subring R) (x : ↥s) : ↑(-x) = -↑x :=
  rfl

@[simp] theorem coe_mul {R : Type u} [ring R] (s : subring R) (x : ↥s) (y : ↥s) : ↑(x * y) = ↑x * ↑y :=
  rfl

@[simp] theorem coe_zero {R : Type u} [ring R] (s : subring R) : ↑0 = 0 :=
  rfl

@[simp] theorem coe_one {R : Type u} [ring R] (s : subring R) : ↑1 = 1 :=
  rfl
...
@[simp] theorem coe_subtype {R : Type u} [ring R] (s : subring R) : ⇑(subtype s) = coe :=
  rfl

/-! # Partial order -/

protected instance partial_order {R : Type u} [ring R] : partial_order (subring R) :=
  partial_order.mk (fun (s t : subring R) => ∀ {x : R}, x ∈ s → x ∈ t) partial_order.lt sorry sorry sorry

theorem le_def {R : Type u} [ring R] {s : subring R} {t : subring R} : s ≤ t ↔ ∀ {x : R}, x ∈ s → x ∈ t :=
  iff.rfl

@[simp] theorem coe_subset_coe {R : Type u} [ring R] {s : subring R} {t : subring R} : ↑s ⊆ ↑t ↔ s ≤ t :=
  iff.rfl

@[simp] theorem coe_ssubset_coe {R : Type u} [ring R] {s : subring R} {t : subring R} : ↑s ⊂ ↑t ↔ s < t :=
  iff.rfl

@[simp] theorem mem_coe {R : Type u} [ring R] {S : subring R} {m : R} : m ∈ ↑S ↔ m ∈ S :=
  iff.rfl

@[simp] theorem coe_coe {R : Type u} [ring R] (s : subring R) : ↥↑s = ↥s :=
  rfl

@[simp] theorem mem_to_submonoid {R : Type u} [ring R] {s : subring R} {x : R} : x ∈ to_submonoid s ↔ x ∈ s :=
  iff.rfl

@[simp] theorem coe_to_submonoid {R : Type u} [ring R] (s : subring R) : ↑(to_submonoid s) = ↑s :=
  rfl

@[simp] theorem mem_to_add_subgroup {R : Type u} [ring R] {s : subring R} {x : R} : x ∈ to_add_subgroup s ↔ x ∈ s :=
  iff.rfl

@[simp] theorem coe_to_add_subgroup {R : Type u} [ring R] (s : subring R) : ↑(to_add_subgroup s) = ↑s :=
  rfl
...
@[simp] theorem mem_top {R : Type u} [ring R] (x : R) : x ∈ ⊤ :=
  set.mem_univ x

@[simp] theorem coe_top {R : Type u} [ring R] : ↑⊤ = set.univ :=
  rfl
...
@[simp] theorem coe_comap {R : Type u} {S : Type v} [ring R] [ring S] (s : subring S) (f : R →+* S) : ↑(comap f s) = ⇑f ⁻¹' ↑s :=
  rfl

@[simp] theorem mem_comap {R : Type u} {S : Type v} [ring R] [ring S] {s : subring S} {f : R →+* S} {x : R} : x ∈ comap f s ↔ coe_fn f x ∈ s :=
  iff.rfl

theorem comap_comap {R : Type u} {S : Type v} {T : Type w} [ring R] [ring S] [ring T] (s : subring T) (g : S →+* T) (f : R →+* S) : comap f (comap g s) = comap (ring_hom.comp g f) s :=
  rfl
...
@[simp] theorem coe_map {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : subring R) : ↑(map f s) = ⇑f '' ↑s :=
  rfl

@[simp] theorem mem_map {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} {s : subring R} {y : S} : y ∈ map f s ↔ ∃ (x : R), ∃ (H : x ∈ s), coe_fn f x = y :=
  set.mem_image_iff_bex

theorem map_map {R : Type u} {S : Type v} {T : Type w} [ring R] [ring S] [ring T] (s : subring R) (g : S →+* T) (f : R →+* S) : map g (map f s) = map (ring_hom.comp g f) s :=
  ext' (set.image_image (fun (a : S) => coe_fn g a) (fun (a : R) => coe_fn f a) (carrier s))

theorem map_le_iff_le_comap {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} {s : subring R} {t : subring S} : map f s ≤ t ↔ s ≤ comap f t :=
  set.image_subset_iff

theorem gc_map_comap {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) : galois_connection (map f) (comap f) :=
...
@[simp] theorem coe_range {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) : ↑(range f) = set.range ⇑f :=
  set.image_univ

@[simp] theorem mem_range {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} {y : S} : y ∈ range f ↔ ∃ (x : R), coe_fn f x = y := sorry

theorem mem_range_self {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (x : R) : coe_fn f x ∈ range f :=
  iff.mpr mem_range (Exists.intro x rfl)

theorem map_range {R : Type u} {S : Type v} {T : Type w} [ring R] [ring S] [ring T] (g : S →+* T) (f : R →+* S) : subring.map g (range f) = range (comp g f) :=
  subring.map_map ⊤ g f
...
def cod_restrict' {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : subring S) (h : ∀ (x : R), coe_fn f x ∈ s) : R →+* ↥s :=
  mk (fun (x : R) => { val := coe_fn f x, property := h x }) sorry sorry sorry sorry

theorem surjective_onto_range {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) : function.surjective ⇑(cod_restrict' f (range f) (mem_range_self f)) := sorry
...
protected instance nontrivial {D : Type u_1} [ring D] [nontrivial D] (S : subring D) : nontrivial ↥S :=
  subsemiring.nontrivial (to_subsemiring S)

/-- A subring of a ring with no zero divisors has no zero divisors. -/
protected instance no_zero_divisors {D : Type u_1} [ring D] [no_zero_divisors D] (S : subring D) : no_zero_divisors ↥S :=
  subsemiring.no_zero_divisors (to_subsemiring S)

/-- A subring of an integral domain is an integral domain. -/
protected instance subring.domain {D : Type u_1} [integral_domain D] (S : subring D) : integral_domain ↥S :=
...
protected instance inhabited {R : Type u} [ring R] : Inhabited (subring R) :=
  { default := ⊥ }

theorem coe_bot {R : Type u} [ring R] : ↑⊥ = set.range coe :=
  ring_hom.coe_range (int.cast_ring_hom R)

theorem mem_bot {R : Type u} [ring R] {x : R} : x ∈ ⊥ ↔ ∃ (n : ℤ), ↑n = x :=
  ring_hom.mem_range
...
@[simp] theorem coe_inf {R : Type u} [ring R] (p : subring R) (p' : subring R) : ↑(p ⊓ p') = ↑p ∩ ↑p' :=
  rfl

@[simp] theorem mem_inf {R : Type u} [ring R] {p : subring R} {p' : subring R} {x : R} : x ∈ p ⊓ p' ↔ x ∈ p ∧ x ∈ p' :=
  iff.rfl
...
@[simp] theorem coe_Inf {R : Type u} [ring R] (S : set (subring R)) : ↑(Inf S) = set.Inter fun (s : subring R) => set.Inter fun (H : s ∈ S) => ↑s :=
  rfl

theorem mem_Inf {R : Type u} [ring R] {S : set (subring R)} {x : R} : x ∈ Inf S ↔ ∀ (p : subring R), p ∈ S → x ∈ p :=
  set.mem_bInter_iff

@[simp] theorem Inf_to_submonoid {R : Type u} [ring R] (s : set (subring R)) : to_submonoid (Inf s) = infi fun (t : subring R) => infi fun (H : t ∈ s) => to_submonoid t :=
  mk'_to_submonoid (has_Inf._proof_1 s) (has_Inf._proof_2 s)

@[simp] theorem Inf_to_add_subgroup {R : Type u} [ring R] (s : set (subring R)) : to_add_subgroup (Inf s) = infi fun (t : subring R) => infi fun (H : t ∈ s) => to_add_subgroup t :=
...
theorem mem_closure {R : Type u} [ring R] {x : R} {s : set R} : x ∈ closure s ↔ ∀ (S : subring R), s ⊆ ↑S → x ∈ S :=
  mem_Inf
...
@[simp] theorem closure_le {R : Type u} [ring R] {s : set R} {t : subring R} : closure s ≤ t ↔ s ⊆ ↑t :=
...
theorem closure_eq_of_le {R : Type u} [ring R] {s : set R} {t : subring R} (h₁ : s ⊆ ↑t) (h₂ : t ≤ closure s) : closure s = t :=
...
of the closure of `s`. -/
theorem closure_induction {R : Type u} [ring R] {s : set R} {p : R → Prop} {x : R} (h : x ∈ closure s) (Hs : ∀ (x : R), x ∈ s → p x) (H0 : p 0) (H1 : p 1) (Hadd : ∀ (x y : R), p x → p y → p (x + y)) (Hneg : ∀ (x : R), p x → p (-x)) (Hmul : ∀ (x y : R), p x → p y → p (x * y)) : p x :=
  iff.mpr closure_le Hs x h

theorem mem_closure_iff {R : Type u} [ring R] {s : set R} {x : R} : x ∈ closure s ↔ x ∈ add_subgroup.closure ↑(submonoid.closure s) := sorry

theorem exists_list_of_mem_closure {R : Type u} [ring R] {s : set R} {x : R} (h : x ∈ closure s) : ∃ (L : List (List R)), (∀ (t : List R), t ∈ L → ∀ (y : R), y ∈ t → y ∈ s ∨ y = -1) ∧ list.sum (list.map list.prod L) = x := sorry
...
@[simp] theorem closure_univ {R : Type u} [ring R] : closure set.univ = ⊤ :=
  coe_top ▸ closure_eq ⊤

theorem closure_union {R : Type u} [ring R] (s : set R) (t : set R) : closure (s ∪ t) = closure s ⊔ closure t :=
  galois_connection.l_sup (galois_insertion.gc (subring.gi R))

theorem closure_Union {R : Type u} [ring R] {ι : Sort u_1} (s : ι → set R) : closure (set.Union fun (i : ι) => s i) = supr fun (i : ι) => closure (s i) :=
  galois_connection.l_supr (galois_insertion.gc (subring.gi R))

theorem closure_sUnion {R : Type u} [ring R] (s : set (set R)) : closure (⋃₀s) = supr fun (t : set R) => supr fun (H : t ∈ s) => closure t :=
  galois_connection.l_Sup (galois_insertion.gc (subring.gi R))

theorem map_sup {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) (t : subring R) (f : R →+* S) : map f (s ⊔ t) = map f s ⊔ map f t :=
  galois_connection.l_sup (gc_map_comap f)

theorem map_supr {R : Type u} {S : Type v} [ring R] [ring S] {ι : Sort u_1} (f : R →+* S) (s : ι → subring R) : map f (supr s) = supr fun (i : ι) => map f (s i) :=
  galois_connection.l_supr (gc_map_comap f)

theorem comap_inf {R : Type u} {S : Type v} [ring R] [ring S] (s : subring S) (t : subring S) (f : R →+* S) : comap f (s ⊓ t) = comap f s ⊓ comap f t :=
  galois_connection.u_inf (gc_map_comap f)

theorem comap_infi {R : Type u} {S : Type v} [ring R] [ring S] {ι : Sort u_1} (f : R →+* S) (s : ι → subring S) : comap f (infi s) = infi fun (i : ι) => comap f (s i) :=
...
@[simp] theorem comap_top {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) : comap f ⊤ = ⊤ :=
  galois_connection.u_top (gc_map_comap f)

/-- Given `subring`s `s`, `t` of rings `R`, `S` respectively, `s.prod t` is `s × t`
as a subring of `R × S`. -/
def prod {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) (t : subring S) : subring (R × S) :=
  mk (set.prod ↑s ↑t) sorry sorry sorry sorry sorry

theorem coe_prod {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) (t : subring S) : ↑(prod s t) = set.prod ↑s ↑t :=
  rfl

theorem mem_prod {R : Type u} {S : Type v} [ring R] [ring S] {s : subring R} {t : subring S} {p : R × S} : p ∈ prod s t ↔ prod.fst p ∈ s ∧ prod.snd p ∈ t :=
  iff.rfl

theorem prod_mono {R : Type u} {S : Type v} [ring R] [ring S] {s₁ : subring R} {s₂ : subring R} (hs : s₁ ≤ s₂) {t₁ : subring S} {t₂ : subring S} (ht : t₁ ≤ t₂) : prod s₁ t₁ ≤ prod s₂ t₂ :=
  set.prod_mono hs ht

theorem prod_mono_right {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) : monotone fun (t : subring S) => prod s t :=
  prod_mono (le_refl s)

theorem prod_mono_left {R : Type u} {S : Type v} [ring R] [ring S] (t : subring S) : monotone fun (s : subring R) => prod s t :=
  fun (s₁ s₂ : subring R) (hs : s₁ ≤ s₂) => prod_mono hs (le_refl t)

theorem prod_top {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) : prod s ⊤ = comap (ring_hom.fst R S) s := sorry

theorem top_prod {R : Type u} {S : Type v} [ring R] [ring S] (s : subring S) : prod ⊤ s = comap (ring_hom.snd R S) s := sorry
...
def prod_equiv {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) (t : subring S) : ↥(prod s t) ≃+* ↥s × ↥t :=
...
theorem mem_supr_of_directed {R : Type u} [ring R] {ι : Sort u_1} [hι : Nonempty ι] {S : ι → subring R} (hS : directed LessEq S) {x : R} : (x ∈ supr fun (i : ι) => S i) ↔ ∃ (i : ι), x ∈ S i := sorry

theorem coe_supr_of_directed {R : Type u} [ring R] {ι : Sort u_1} [hι : Nonempty ι] {S : ι → subring R} (hS : directed LessEq S) : ↑(supr fun (i : ι) => S i) = set.Union fun (i : ι) => ↑(S i) := sorry

theorem mem_Sup_of_directed_on {R : Type u} [ring R] {S : set (subring R)} (Sne : set.nonempty S) (hS : directed_on LessEq S) {x : R} : x ∈ Sup S ↔ ∃ (s : subring R), ∃ (H : s ∈ S), x ∈ s := sorry

theorem coe_Sup_of_directed_on {R : Type u} [ring R] {S : set (subring R)} (Sne : set.nonempty S) (hS : directed_on LessEq S) : ↑(Sup S) = set.Union fun (s : subring R) => set.Union fun (H : s ∈ S) => ↑s := sorry
...
@[simp] theorem restrict_apply {R : Type u} {S : Type v} [ring R] [ring S] {s : subring R} (f : R →+* S) (x : ↥s) : coe_fn (restrict f s) x = coe_fn f ↑x :=
  rfl
...
@[simp] theorem coe_range_restrict {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (x : R) : ↑(coe_fn (range_restrict f) x) = coe_fn f x :=
  rfl

theorem range_top_iff_surjective {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} : range f = ⊤ ↔ function.surjective ⇑f := sorry

/-- The range of a surjective ring homomorphism is the whole of the codomain. -/
theorem range_top_of_surjective {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (hf : function.surjective ⇑f) : range f = ⊤ :=
...
theorem eq_on_set_closure {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} {g : R →+* S} {s : set R} (h : set.eq_on (⇑f) (⇑g) s) : set.eq_on ⇑f ⇑g ↑(subring.closure s) :=
  (fun (this : subring.closure s ≤ eq_locus f g) => this) (iff.mpr subring.closure_le h)

theorem eq_of_eq_on_set_top {R : Type u} {S : Type v} [ring R] [ring S] {f : R →+* S} {g : R →+* S} (h : set.eq_on ⇑f ⇑g ↑⊤) : f = g :=
  ext fun (x : R) => h trivial

theorem eq_of_eq_on_set_dense {R : Type u} {S : Type v} [ring R] [ring S] {s : set R} (hs : subring.closure s = ⊤) {f : R →+* S} {g : R →+* S} (h : set.eq_on (⇑f) (⇑g) s) : f = g :=
  eq_of_eq_on_set_top (hs ▸ eq_on_set_closure h)

theorem closure_preimage_le {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : set S) : subring.closure (⇑f ⁻¹' s) ≤ subring.comap f (subring.closure s) :=
  iff.mpr subring.closure_le
    fun (x : R) (hx : x ∈ ⇑f ⁻¹' s) => iff.mpr subring.mem_coe (iff.mpr subring.mem_comap (subring.subset_closure hx))
...
theorem map_closure {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : set R) : subring.map f (subring.closure s) = subring.closure (⇑f '' s) := sorry
...
@[simp] theorem range_subtype {R : Type u} [ring R] (s : subring R) : ring_hom.range (subtype s) = s :=
  ext' (Eq.trans (ring_hom.coe_srange (subtype s)) subtype.range_coe)

@[simp] theorem range_fst {R : Type u} {S : Type v} [ring R] [ring S] : ring_hom.srange (ring_hom.fst R S) = ⊤ :=
  ring_hom.srange_top_of_surjective (ring_hom.fst R S) prod.fst_surjective

@[simp] theorem range_snd {R : Type u} {S : Type v} [ring R] [ring S] : ring_hom.srange (ring_hom.snd R S) = ⊤ :=
  ring_hom.srange_top_of_surjective (ring_hom.snd R S) prod.snd_surjective

@[simp] theorem prod_bot_sup_bot_prod {R : Type u} {S : Type v} [ring R] [ring S] (s : subring R) (t : subring S) : prod s ⊥ ⊔ prod ⊥ t = prod s t := sorry
...
theorem closure_preimage_le {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : set S) : closure (⇑f ⁻¹' s) ≤ comap f (closure s) :=
  iff.mpr closure_le fun (x : R) (hx : x ∈ ⇑f ⁻¹' s) => iff.mpr mem_coe (iff.mpr mem_comap (subset_closure hx))

end subring


theorem add_subgroup.int_mul_mem {R : Type u} [ring R] {G : add_subgroup R} (k : ℤ) {g : R} (h : g ∈ G) : ↑k * g ∈ G := sorry
