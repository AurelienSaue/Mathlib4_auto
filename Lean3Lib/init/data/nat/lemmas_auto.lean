...
theorem succ_ne_zero (n : ℕ) : Nat.succ n ≠ 0 :=
  fun (h : Nat.succ n = 0) => nat.no_confusion h

theorem succ_ne_self (n : ℕ) : Nat.succ n ≠ n := sorry

protected theorem one_ne_zero : 1 ≠ 0 :=
  fun (h : 1 = 0) => nat.no_confusion h

protected theorem zero_ne_one : 0 ≠ 1 :=
  fun (h : 0 = 1) => nat.no_confusion h
...
@[simp] theorem pred_zero : Nat.pred 0 = 0 :=
  rfl

@[simp] theorem pred_succ (n : ℕ) : Nat.pred (Nat.succ n) = n :=
  rfl

protected theorem mul_zero (n : ℕ) : n * 0 = 0 :=
  rfl

theorem mul_succ (n : ℕ) (m : ℕ) : n * Nat.succ m = n * m + n :=
  rfl
...
protected theorem le_of_eq {n : ℕ} {m : ℕ} (p : n = m) : n ≤ m :=
  p ▸ less_than_or_equal.refl

theorem le_succ_of_le {n : ℕ} {m : ℕ} (h : n ≤ m) : n ≤ Nat.succ m :=
  nat.le_trans h (le_succ m)

theorem le_of_succ_le {n : ℕ} {m : ℕ} (h : Nat.succ n ≤ m) : n ≤ m :=
  nat.le_trans (le_succ n) h

protected theorem le_of_lt {n : ℕ} {m : ℕ} (h : n < m) : n ≤ m :=
  le_of_succ_le h

def lt.step {n : ℕ} {m : ℕ} : n < m → n < Nat.succ m :=
  less_than_or_equal.step

theorem eq_zero_or_pos (n : ℕ) : n = 0 ∨ n > 0 :=
  nat.cases_on n (Or.inl rfl) fun (n : ℕ) => Or.inr (succ_pos n)

protected theorem pos_of_ne_zero {n : ℕ} : n ≠ 0 → n > 0 :=
...
def lt.base (n : ℕ) : n < Nat.succ n :=
  nat.le_refl (Nat.succ n)

theorem lt_succ_self (n : ℕ) : n < Nat.succ n :=
  lt.base n

protected theorem le_antisymm {n : ℕ} {m : ℕ} (h₁ : n ≤ m) : m ≤ n → n = m :=
  less_than_or_equal.cases_on h₁ (fun (a : n ≤ n) => rfl)
    fun (a : ℕ) (b : less_than_or_equal n a) (c : Nat.succ a ≤ n) => absurd (nat.lt_of_le_of_lt b c) (nat.lt_irrefl n)
...
protected instance linear_order : linear_order ℕ :=
  linear_order.mk less_than_or_equal nat.lt nat.le_refl nat.le_trans nat.le_antisymm nat.le_total nat.decidable_le
    nat.decidable_eq nat.decidable_lt

theorem eq_zero_of_le_zero {n : ℕ} (h : n ≤ 0) : n = 0 :=
  le_antisymm h (zero_le n)

theorem succ_lt_succ {a : ℕ} {b : ℕ} : a < b → Nat.succ a < Nat.succ b :=
  succ_le_succ

theorem lt_of_succ_lt {a : ℕ} {b : ℕ} : Nat.succ a < b → a < b :=
  le_of_succ_le

theorem lt_of_succ_lt_succ {a : ℕ} {b : ℕ} : Nat.succ a < Nat.succ b → a < b :=
  le_of_succ_le_succ

theorem pred_lt_pred {n : ℕ} {m : ℕ} : n ≠ 0 → n < m → Nat.pred n < Nat.pred m := sorry

theorem lt_of_succ_le {a : ℕ} {b : ℕ} (h : Nat.succ a ≤ b) : a < b :=
  h

theorem succ_le_of_lt {a : ℕ} {b : ℕ} (h : a < b) : Nat.succ a ≤ b :=
  h

theorem le_add_right (n : ℕ) (k : ℕ) : n ≤ n + k := sorry

theorem le_add_left (n : ℕ) (m : ℕ) : n ≤ m + n :=
  nat.add_comm n m ▸ le_add_right n m

theorem le.dest {n : ℕ} {m : ℕ} : n ≤ m → ∃ (k : ℕ), n + k = m := sorry

theorem le.intro {n : ℕ} {m : ℕ} {k : ℕ} (h : n + k = m) : n ≤ m :=
  h ▸ le_add_right n k
...
protected theorem add_le_add_right {n : ℕ} {m : ℕ} (h : n ≤ m) (k : ℕ) : n + k ≤ m + k :=
  eq.mpr (id (Eq._oldrec (Eq.refl (n + k ≤ m + k)) (nat.add_comm n k)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (k + n ≤ m + k)) (nat.add_comm m k))) (nat.add_le_add_left h k))
...
protected theorem le_of_add_le_add_right {k : ℕ} {n : ℕ} {m : ℕ} : n + k ≤ m + k → n ≤ m :=
  eq.mpr (id (Eq._oldrec (Eq.refl (n + k ≤ m + k → n ≤ m)) (nat.add_comm n k)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (k + n ≤ m + k → n ≤ m)) (nat.add_comm m k))) nat.le_of_add_le_add_left)
...
protected theorem lt_of_add_lt_add_right {a : ℕ} {b : ℕ} {c : ℕ} (h : a + b < c + b) : a < c := sorry
...
protected theorem add_lt_add {a : ℕ} {b : ℕ} {c : ℕ} {d : ℕ} (h₁ : a < b) (h₂ : c < d) : a + c < b + d :=
  lt_trans (nat.add_lt_add_right h₁ c) (nat.add_lt_add_left h₂ b)

protected theorem add_le_add {a : ℕ} {b : ℕ} {c : ℕ} {d : ℕ} (h₁ : a ≤ b) (h₂ : c ≤ d) : a + c ≤ b + d :=
...
protected theorem mul_lt_mul_of_pos_left {n : ℕ} {m : ℕ} {k : ℕ} (h : n < m) (hk : k > 0) : k * n < k * m :=
  nat.lt_of_lt_of_le (nat.lt_add_of_pos_right hk) (mul_succ k n ▸ mul_le_mul_left k (succ_le_of_lt h))

protected theorem mul_lt_mul_of_pos_right {n : ℕ} {m : ℕ} {k : ℕ} (h : n < m) (hk : k > 0) : n * k < m * k :=
  nat.mul_comm k m ▸ nat.mul_comm k n ▸ nat.mul_lt_mul_of_pos_left h hk

protected theorem le_of_mul_le_mul_left {a : ℕ} {b : ℕ} {c : ℕ} (h : c * a ≤ c * b) (hc : c > 0) : a ≤ b :=
  iff.mp decidable.not_lt
    fun (h1 : b < a) => (fun (h2 : c * b < c * a) => not_le_of_gt h2 h) (nat.mul_lt_mul_of_pos_left h1 hc)

theorem le_of_lt_succ {m : ℕ} {n : ℕ} : m < Nat.succ n → m ≤ n :=
  le_of_succ_le_succ

theorem eq_of_mul_eq_mul_left {m : ℕ} {k : ℕ} {n : ℕ} (Hn : n > 0) (H : n * m = n * k) : m = k :=
  le_antisymm (nat.le_of_mul_le_mul_left (le_of_eq H) Hn) (nat.le_of_mul_le_mul_left (le_of_eq (Eq.symm H)) Hn)

/- sub properties -/

@[simp] protected theorem zero_sub (a : ℕ) : 0 - a = 0 := sorry

theorem sub_lt_succ (a : ℕ) (b : ℕ) : a - b < Nat.succ a :=
...
protected theorem bit1_eq_succ_bit0 (n : ℕ) : bit1 n = Nat.succ (bit0 n) :=
  rfl
...
protected theorem bit0_ne_bit1 (n : ℕ) (m : ℕ) : bit0 n ≠ bit1 m :=
...
protected theorem bit0_lt {n : ℕ} {m : ℕ} (h : n < m) : bit0 n < bit0 m :=
  nat.add_lt_add h h
...
@[simp] protected theorem sub_zero (n : ℕ) : n - 0 = n :=
  rfl

theorem sub_succ (n : ℕ) (m : ℕ) : n - Nat.succ m = Nat.pred (n - m) :=
  rfl

theorem succ_sub_succ (n : ℕ) (m : ℕ) : Nat.succ n - Nat.succ m = n - m :=
...
protected theorem add_sub_add_left (k : ℕ) (n : ℕ) (m : ℕ) : k + n - (k + m) = n - m :=
  eq.mpr (id (Eq._oldrec (Eq.refl (k + n - (k + m) = n - m)) (nat.add_comm k n)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (n + k - (k + m) = n - m)) (nat.add_comm k m)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (n + k - (m + k) = n - m)) (nat.add_sub_add_right n k m))) (Eq.refl (n - m))))

protected theorem add_sub_cancel (n : ℕ) (m : ℕ) : n + m - m = n :=
  (fun (this : n + m - (0 + m) = n) => eq.mp (Eq._oldrec (Eq.refl (n + m - (0 + m) = n)) (nat.zero_add m)) this)
...
theorem le_of_le_of_sub_le_sub_right {n : ℕ} {m : ℕ} {k : ℕ} (h₀ : k ≤ m) (h₁ : n - k ≤ m - k) : n ≤ m := sorry

protected theorem sub_le_sub_right_iff (n : ℕ) (m : ℕ) (k : ℕ) (h : k ≤ m) : n - k ≤ m - k ↔ n ≤ m :=
...
theorem sub_one (n : ℕ) : n - 1 = Nat.pred n :=
  rfl

theorem succ_sub_one (n : ℕ) : Nat.succ n - 1 = n :=
  rfl
...
protected theorem add_sub_assoc {m : ℕ} {k : ℕ} (h : k ≤ m) (n : ℕ) : n + m - k = n + (m - k) := sorry

protected theorem sub_eq_iff_eq_add {a : ℕ} {b : ℕ} {c : ℕ} (ab : b ≤ a) : a - b = c ↔ a = c + b := sorry

protected theorem lt_of_sub_eq_succ {m : ℕ} {n : ℕ} {l : ℕ} (H : m - n = Nat.succ l) : n < m := sorry

@[simp] theorem zero_min (a : ℕ) : min 0 a = 0 :=
  min_eq_left (zero_le a)

@[simp] theorem min_zero (a : ℕ) : min a 0 = 0 :=
...
@[simp] theorem sub_add_min_cancel (n : ℕ) (m : ℕ) : n - m + min n m = n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (n - m + min n m = n)) (sub_eq_sub_min n m)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (n - min n m + min n m = n)) (nat.sub_add_cancel (min_le_left n m)))) (Eq.refl n))

/- TODO(Leo): sub + inequalities -/

protected def strong_rec_on {p : ℕ → Sort u} (n : ℕ) (h : (n : ℕ) → ((m : ℕ) → m < n → p m) → p n) : p n :=
...
protected theorem strong_induction_on {p : ℕ → Prop} (n : ℕ) (h : ∀ (n : ℕ), (∀ (m : ℕ), m < n → p m) → p n) : p n :=
  nat.strong_rec_on n h

protected theorem case_strong_induction_on {p : ℕ → Prop} (a : ℕ) (hz : p 0) (hi : ∀ (n : ℕ), (∀ (m : ℕ), m ≤ n → p m) → p (Nat.succ n)) : p a := sorry
...
theorem discriminate {B : Sort u} {n : ℕ} (H1 : n = 0 → B) (H2 : (m : ℕ) → n = Nat.succ m → B) : B :=
  (fun (_x : ℕ) (h : n = _x) =>
      Nat.rec (fun (h : n = 0) => H1 h) (fun (n_1 : ℕ) (ih : n = n_1 → B) (h : n = Nat.succ n_1) => H2 n_1 h) _x h)
    n rfl

theorem one_succ_zero : 1 = Nat.succ 0 :=
  rfl

theorem two_step_induction {P : ℕ → Sort u} (H1 : P 0) (H2 : P 1) (H3 : (n : ℕ) → P n → P (Nat.succ n) → P (Nat.succ (Nat.succ n))) (a : ℕ) : P a := sorry

theorem sub_induction {P : ℕ → ℕ → Sort u} (H1 : (m : ℕ) → P 0 m) (H2 : (n : ℕ) → P (Nat.succ n) 0) (H3 : (n m : ℕ) → P n m → P (Nat.succ n) (Nat.succ m)) (n : ℕ) (m : ℕ) : P n m := sorry
...
theorem lt_le_antisymm {n : ℕ} {m : ℕ} (h₁ : n < m) (h₂ : m ≤ n) : False :=
  le_lt_antisymm h₂ h₁

protected theorem lt_asymm {n : ℕ} {m : ℕ} (h₁ : n < m) : ¬m < n :=
...
protected def lt_by_cases {a : ℕ} {b : ℕ} {C : Sort u} (h₁ : a < b → C) (h₂ : a = b → C) (h₃ : b < a → C) : C :=
  nat.lt_ge_by_cases h₁ fun (h₁ : a ≥ b) => nat.lt_ge_by_cases h₃ fun (h : b ≥ a) => h₂ (nat.le_antisymm h h₁)
...
theorem lt_succ_of_lt {a : ℕ} {b : ℕ} (h : a < b) : a < Nat.succ b :=
  le_succ_of_le h
...
protected theorem sub_le_sub_left {n : ℕ} {m : ℕ} (k : ℕ) (h : n ≤ m) : k - m ≤ k - n :=
  less_than_or_equal.drec (le_refl (k - n))
    (fun {h_b : ℕ} (h_ᾰ : less_than_or_equal n h_b) (h_ih : k - h_b ≤ k - n) => le_trans (pred_le (Nat.sub k h_b)) h_ih) h
...
protected theorem sub.right_comm (m : ℕ) (n : ℕ) (k : ℕ) : m - n - k = m - k - n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (m - n - k = m - k - n)) (nat.sub_sub m n k)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (m - (n + k) = m - k - n)) (nat.sub_sub m k n)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (m - (n + k) = m - (k + n))) (nat.add_comm n k))) (Eq.refl (m - (k + n)))))
...
theorem mul_pred_right (n : ℕ) (m : ℕ) : n * Nat.pred m = n * m - n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (n * Nat.pred m = n * m - n)) (nat.mul_comm n (Nat.pred m))))
    (eq.mpr (id (Eq._oldrec (Eq.refl (Nat.pred m * n = n * m - n)) (mul_pred_left m n)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (m * n - n = n * m - n)) (nat.mul_comm m n))) (Eq.refl (n * m - n))))

protected theorem mul_sub_right_distrib (n : ℕ) (m : ℕ) (k : ℕ) : (n - m) * k = n * k - m * k := sorry

protected theorem mul_sub_left_distrib (n : ℕ) (m : ℕ) (k : ℕ) : n * (m - k) = n * m - n * k := sorry

protected theorem mul_self_sub_mul_self_eq (a : ℕ) (b : ℕ) : a * a - b * b = (a + b) * (a - b) := sorry
...
protected theorem sub_add_comm {n : ℕ} {m : ℕ} {k : ℕ} (h : k ≤ n) : n + m - k = n - k + m :=
  iff.mpr (nat.sub_eq_iff_eq_add (nat.le_trans h (le_add_right n m)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (n + m = n - k + m + k)) (nat.add_right_comm (n - k) m k)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (n + m = n - k + k + m)) (nat.sub_add_cancel h))) (Eq.refl (n + m))))
...
protected def find_x {p : ℕ → Prop} [decidable_pred p] (H : ∃ (n : ℕ), p n) : Subtype fun (n : ℕ) => p n ∧ ∀ (m : ℕ), m < n → ¬p m :=
  well_founded.fix (wf_lbp H)
    (fun (m : ℕ)
      (IH :
      (y : ℕ) →
        lbp y m → (fun (k : ℕ) => (∀ (n : ℕ), n < k → ¬p n) → Subtype fun (n : ℕ) => p n ∧ ∀ (m : ℕ), m < n → ¬p m) y)
      (al : ∀ (n : ℕ), n < m → ¬p n) =>
...
protected theorem find_spec {p : ℕ → Prop} [decidable_pred p] (H : ∃ (n : ℕ), p n) : p (nat.find H) :=
  and.left (subtype.property (nat.find_x H))

protected theorem find_min {p : ℕ → Prop} [decidable_pred p] (H : ∃ (n : ℕ), p n) {m : ℕ} : m < nat.find H → ¬p m :=
  and.right (subtype.property (nat.find_x H))

protected theorem find_min' {p : ℕ → Prop} [decidable_pred p] (H : ∃ (n : ℕ), p n) {m : ℕ} (h : p m) : nat.find H ≤ m :=
...
@[simp] theorem add_mod_right (x : ℕ) (z : ℕ) : (x + z) % z = x % z :=
  eq.mpr (id (Eq._oldrec (Eq.refl ((x + z) % z = x % z)) (mod_eq_sub_mod (le_add_left z x))))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((x + z - z) % z = x % z)) (nat.add_sub_cancel x z))) (Eq.refl (x % z)))
...
@[simp] theorem add_mul_mod_self_right (x : ℕ) (y : ℕ) (z : ℕ) : (x + y * z) % z = x % z :=
  eq.mpr (id (Eq._oldrec (Eq.refl ((x + y * z) % z = x % z)) (nat.mul_comm y z)))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((x + z * y) % z = x % z)) (add_mul_mod_self_left x z y))) (Eq.refl (x % z)))
...
theorem cond_to_bool_mod_two (x : ℕ) [d : Decidable (x % bit0 1 = 1)] : cond (to_bool (x % bit0 1 = 1)) 1 0 = x % bit0 1 := sorry
...
@[simp] theorem add_div_right (x : ℕ) {z : ℕ} (H : z > 0) : (x + z) / z = Nat.succ (x / z) :=
  eq.mpr (id (Eq._oldrec (Eq.refl ((x + z) / z = Nat.succ (x / z))) (div_eq_sub_div H (le_add_left z x))))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((x + z - z) / z + 1 = Nat.succ (x / z))) (nat.add_sub_cancel x z)))
      (Eq.refl (x / z + 1)))
...
@[simp] theorem add_div_left (x : ℕ) {z : ℕ} (H : z > 0) : (z + x) / z = Nat.succ (x / z) :=
  eq.mpr (id (Eq._oldrec (Eq.refl ((z + x) / z = Nat.succ (x / z))) (nat.add_comm z x)))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((x + z) / z = Nat.succ (x / z))) (add_div_right x H))) (Eq.refl (Nat.succ (x / z))))
...
theorem add_mul_div_right (x : ℕ) (y : ℕ) {z : ℕ} (H : z > 0) : (x + y * z) / z = x / z + y :=
  eq.mpr (id (Eq._oldrec (Eq.refl ((x + y * z) / z = x / z + y)) (nat.mul_comm y z)))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((x + z * y) / z = x / z + y)) (add_mul_div_left x y H))) (Eq.refl (x / z + y)))
...
protected theorem div_eq_of_eq_mul_left {m : ℕ} {n : ℕ} {k : ℕ} (H1 : n > 0) (H2 : m = k * n) : m / n = k :=
  eq.mpr (id (Eq._oldrec (Eq.refl (m / n = k)) H2))
    (eq.mpr (id (Eq._oldrec (Eq.refl (k * n / n = k)) (nat.mul_div_cancel k H1))) (Eq.refl k))

protected theorem div_eq_of_eq_mul_right {m : ℕ} {n : ℕ} {k : ℕ} (H1 : n > 0) (H2 : m = n * k) : m / n = k :=
  eq.mpr (id (Eq._oldrec (Eq.refl (m / n = k)) H2))
    (eq.mpr (id (Eq._oldrec (Eq.refl (n * k / n = k)) (nat.mul_div_cancel_left k H1))) (Eq.refl k))

protected theorem div_eq_of_lt_le {m : ℕ} {n : ℕ} {k : ℕ} (lo : k * n ≤ m) (hi : m < Nat.succ k * n) : m / n = k := sorry

theorem mul_sub_div (x : ℕ) (n : ℕ) (p : ℕ) (h₁ : x < n * p) : (n * p - Nat.succ x) / n = p - Nat.succ (x / n) := sorry
...
protected theorem mul_div_mul {m : ℕ} (n : ℕ) (k : ℕ) (H : m > 0) : m * n / (m * k) = n / k :=
  eq.mpr (id (Eq._oldrec (Eq.refl (m * n / (m * k) = n / k)) (Eq.symm (nat.div_div_eq_div_mul (m * n) m k))))
    (eq.mpr (id (Eq._oldrec (Eq.refl (m * n / m / k = n / k)) (nat.mul_div_cancel_left n H))) (Eq.refl (n / k)))

/- dvd -/

@[simp] protected theorem dvd_mul_right (a : ℕ) (b : ℕ) : a ∣ a * b :=
  Exists.intro b rfl
...
protected theorem dvd_add_iff_left {k : ℕ} {m : ℕ} {n : ℕ} (h : k ∣ n) : k ∣ m ↔ k ∣ m + n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (k ∣ m ↔ k ∣ m + n)) (nat.add_comm m n))) (nat.dvd_add_iff_right h)

theorem dvd_sub {k : ℕ} {m : ℕ} {n : ℕ} (H : n ≤ m) (h₁ : k ∣ m) (h₂ : k ∣ n) : k ∣ m - n :=
  iff.mpr (nat.dvd_add_iff_left h₂) (eq.mpr (id (Eq._oldrec (Eq.refl (k ∣ m - n + n)) (nat.sub_add_cancel H))) h₁)

theorem dvd_mod_iff {k : ℕ} {m : ℕ} {n : ℕ} (h : k ∣ n) : k ∣ m % n ↔ k ∣ m :=
  let t : k ∣ m % n ↔ k ∣ m % n + n * (m / n) := nat.dvd_add_iff_left (nat.dvd_trans h (nat.dvd_mul_right n (m / n)));
...
protected theorem mul_div_assoc (m : ℕ) {n : ℕ} {k : ℕ} (H : k ∣ n) : m * n / k = m * (n / k) := sorry

theorem dvd_of_mul_dvd_mul_left {m : ℕ} {n : ℕ} {k : ℕ} (kpos : k > 0) (H : k * m ∣ k * n) : m ∣ n := sorry

theorem dvd_of_mul_dvd_mul_right {m : ℕ} {n : ℕ} {k : ℕ} (kpos : k > 0) (H : m * k ∣ n * k) : m ∣ n :=
...
protected theorem mul_le_mul_of_nonneg_left {a : ℕ} {b : ℕ} {c : ℕ} (h₁ : a ≤ b) : c * a ≤ c * b := sorry

protected theorem mul_le_mul_of_nonneg_right {a : ℕ} {b : ℕ} {c : ℕ} (h₁ : a ≤ b) : a * c ≤ b * c := sorry

protected theorem mul_lt_mul {a : ℕ} {b : ℕ} {c : ℕ} {d : ℕ} (hac : a < c) (hbd : b ≤ d) (pos_b : 0 < b) : a * b < c * d :=
  lt_of_lt_of_le (nat.mul_lt_mul_of_pos_right hac pos_b) (nat.mul_le_mul_of_nonneg_left hbd)

protected theorem mul_lt_mul' {a : ℕ} {b : ℕ} {c : ℕ} {d : ℕ} (h1 : a ≤ c) (h2 : b < d) (h3 : 0 < c) : a * b < c * d :=
  lt_of_le_of_lt (nat.mul_le_mul_of_nonneg_right h1) (nat.mul_lt_mul_of_pos_left h2 h3)

-- TODO: there are four variations, depending on which variables we assume to be nonneg

protected theorem mul_le_mul {a : ℕ} {b : ℕ} {c : ℕ} {d : ℕ} (hac : a ≤ c) (hbd : b ≤ d) : a * b ≤ c * d :=
