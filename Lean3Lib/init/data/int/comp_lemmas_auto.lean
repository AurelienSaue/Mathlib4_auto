...
protected theorem one_pos : 1 > 0 :=
  int.zero_lt_one

protected theorem bit0_pos {a : ℤ} : a > 0 → bit0 a > 0 :=
  fun (h : a > 0) => int.add_pos h h

protected theorem bit1_pos {a : ℤ} : a ≥ 0 → bit1 a > 0 :=
  fun (h : a ≥ 0) => int.lt_add_of_le_of_pos (int.add_nonneg h h) int.zero_lt_one

protected theorem zero_nonneg : 0 ≥ 0 :=
  le_refl 0

protected theorem one_nonneg : 1 ≥ 0 :=
  le_of_lt int.zero_lt_one

protected theorem bit0_nonneg {a : ℤ} : a ≥ 0 → bit0 a ≥ 0 :=
  fun (h : a ≥ 0) => int.add_nonneg h h

protected theorem bit1_nonneg {a : ℤ} : a ≥ 0 → bit1 a ≥ 0 :=
  fun (h : a ≥ 0) => le_of_lt (int.bit1_pos h)

protected theorem nonneg_of_pos {a : ℤ} : a > 0 → a ≥ 0 :=
  le_of_lt
...
theorem of_nat_ge_zero (n : ℕ) : Int.ofNat n ≥ 0 :=
  le.intro
    (eq.mpr (id (Eq._oldrec (Eq.refl (0 + ↑n = Int.ofNat n)) (int.zero_add ↑n)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (↑n = Int.ofNat n)) (int.coe_nat_eq n))) (Eq.refl (Int.ofNat n))))

theorem of_nat_nat_abs_eq_of_nonneg {a : ℤ} : a ≥ 0 → Int.ofNat (nat_abs a) = a := sorry

theorem ne_of_nat_abs_ne_nat_abs_of_nonneg {a : ℤ} {b : ℤ} (ha : a ≥ 0) (hb : b ≥ 0) (h : nat_abs a ≠ nat_abs b) : a ≠ b := sorry

protected theorem ne_of_nat_ne_nonneg_case {a : ℤ} {b : ℤ} {n : ℕ} {m : ℕ} (ha : a ≥ 0) (hb : b ≥ 0) (e1 : nat_abs a = n) (e2 : nat_abs b = m) (h : n ≠ m) : a ≠ b :=
...
theorem nat_abs_of_nat_core (n : ℕ) : nat_abs (Int.ofNat n) = n :=
  rfl

theorem nat_abs_of_neg_succ_of_nat (n : ℕ) : nat_abs (Int.negSucc n) = Nat.succ n :=
  rfl

protected theorem nat_abs_add_nonneg {a : ℤ} {b : ℤ} : a ≥ 0 → b ≥ 0 → nat_abs (a + b) = nat_abs a + nat_abs b := sorry

protected theorem nat_abs_add_neg {a : ℤ} {b : ℤ} : a < 0 → b < 0 → nat_abs (a + b) = nat_abs a + nat_abs b := sorry

protected theorem nat_abs_bit0 (a : ℤ) : nat_abs (bit0 a) = bit0 (nat_abs a) := sorry

protected theorem nat_abs_bit0_step {a : ℤ} {n : ℕ} (h : nat_abs a = n) : nat_abs (bit0 a) = bit0 n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (nat_abs (bit0 a) = bit0 n)) (Eq.symm h))) (int.nat_abs_bit0 a)

protected theorem nat_abs_bit1_nonneg {a : ℤ} (h : a ≥ 0) : nat_abs (bit1 a) = bit1 (nat_abs a) := sorry

protected theorem nat_abs_bit1_nonneg_step {a : ℤ} {n : ℕ} (h₁ : a ≥ 0) (h₂ : nat_abs a = n) : nat_abs (bit1 a) = bit1 n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (nat_abs (bit1 a) = bit1 n)) (Eq.symm h₂))) (int.nat_abs_bit1_nonneg h₁)
