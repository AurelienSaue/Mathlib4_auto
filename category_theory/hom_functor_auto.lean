...
@[simp] theorem hom_obj (C : Type u) [category C] (X : Cᵒᵖ × C) : obj (hom C) X = (opposite.unop (prod.fst X) ⟶ prod.snd X) :=
  rfl

@[simp] theorem hom_pairing_map (C : Type u) [category C] {X : Cᵒᵖ × C} {Y : Cᵒᵖ × C} (f : X ⟶ Y) : map (hom C) f = fun (h : obj (hom C) X) => has_hom.hom.unop (prod.fst f) ≫ h ≫ prod.snd f :=
  rfl
