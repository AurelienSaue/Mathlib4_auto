...
protected instance obj.is_left_adjoint (X : Type v₁) : is_left_adjoint (functor.obj limits.types.binary_product_functor X) :=
  is_left_adjoint.mk (functor.mk (fun (Y : Type v₁) => X ⟶ Y) fun (Y₁ Y₂ : Type v₁) (f : Y₁ ⟶ Y₂) (g : X ⟶ Y₁) => g ≫ f)
      ...
      (adjunction.core_unit_counit.mk (nat_trans.mk fun (Z : Type v₁) (z : Z) (x : X) => (x, z))
        (nat_trans.mk
          fun (Z : Type v₁)
            (xf :
            functor.obj
              ((functor.mk (fun (Y : Type v₁) => X ⟶ Y) fun (Y₁ Y₂ : Type v₁) (f : Y₁ ⟶ Y₂) (g : X ⟶ Y₁) => g ≫ f) ⋙
...
protected instance sort.cartesian_closed : cartesian_closed (Type v₁) :=
  monoidal_closed.mk
    fun (X : Type v₁) => closed.mk (adjunction.left_adjoint_of_nat_iso (iso.app limits.types.binary_product_iso_prod X))

protected instance functor.limits.has_finite_products {C : Type u₁} [category C] : limits.has_finite_products (C ⥤ Type u₁) :=
  limits.has_finite_products_of_has_products (C ⥤ Type u₁)

protected instance functor.cartesian_closed {C : Type v₁} [small_category C] : cartesian_closed (C ⥤ Type v₁) :=
  monoidal_closed.mk
    fun (F : C ⥤ Type v₁) =>
      closed.mk
