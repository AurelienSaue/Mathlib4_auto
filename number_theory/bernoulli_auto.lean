...
def bernoulli : ℕ → ℚ :=
  well_founded.fix nat.lt_wf
    fun (n : ℕ) (bernoulli : (y : ℕ) → nat.lt y n → ℚ) =>
      1 - finset.sum finset.univ fun (k : fin n) => ↑(nat.choose n ↑k) / (↑n - ↑k + 1) * bernoulli ↑k sorry

theorem bernoulli_def' (n : ℕ) : bernoulli n = 1 - finset.sum finset.univ fun (k : fin n) => ↑(nat.choose n ↑k) / (↑n - ↑k + 1) * bernoulli ↑k := sorry

theorem bernoulli_def (n : ℕ) : bernoulli n = 1 - finset.sum (finset.range n) fun (k : ℕ) => ↑(nat.choose n k) / (↑n - ↑k + 1) * bernoulli k := sorry
...
@[simp] theorem bernoulli_zero : bernoulli 0 = 1 :=
  rfl
...
@[simp] theorem bernoulli_four : bernoulli (bit0 (bit0 1)) = -1 / bit0 (bit1 (bit1 (bit1 1))) := sorry

@[simp] theorem sum_bernoulli (n : ℕ) : (finset.sum (finset.range n) fun (k : ℕ) => ↑(nat.choose n k) * bernoulli k) = ↑n := sorry
