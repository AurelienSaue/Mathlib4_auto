...
for good definitional properties of the default term. -/
class unique (α : Sort u) 
extends Inhabited α
where
  uniq : ∀ (a : α), a = Inhabited.default

protected instance punit.unique : unique PUnit :=
  unique.mk { default := PUnit.unit } sorry

theorem fin.eq_zero (n : fin 1) : n = 0 := sorry

protected instance fin.inhabited {n : ℕ} : Inhabited (fin (Nat.succ n)) :=
  { default := 0 }

@[simp] theorem fin.default_eq_zero (n : ℕ) : Inhabited.default = 0 :=
  rfl
...
protected instance inhabited {α : Sort u} [unique α] : Inhabited α :=
  to_inhabited _inst_1

theorem eq_default {α : Sort u} [unique α] (a : α) : a = Inhabited.default :=
  uniq _inst_1 a
  ...
  subsingleton.intro
    fun (a b : α) =>
      eq.mpr (id (Eq._oldrec (Eq.refl (a = b)) (eq_default a)))
        (eq.mpr (id (Eq._oldrec (Eq.refl (Inhabited.default = b)) (eq_default b))) (Eq.refl Inhabited.default))

theorem forall_iff {α : Sort u} [unique α] {p : α → Prop} : (∀ (a : α), p a) ↔ p Inhabited.default :=
  { mp := fun (h : ∀ (a : α), p a) => h Inhabited.default,
    mpr := fun (h : p Inhabited.default) (x : α) => eq.mpr (id (Eq._oldrec (Eq.refl (p x)) (eq_default x))) h }

theorem exists_iff {α : Sort u} [unique α] {p : α → Prop} : Exists p ↔ p Inhabited.default := sorry

protected theorem subsingleton_unique' {α : Sort u} (h₁ : unique α) (h₂ : unique α) : h₁ = h₂ := sorry
...
@[simp] theorem pi.default_def {α : Sort u} {β : α → Sort v} [(a : α) → Inhabited (β a)] : Inhabited.default = fun (a : α) => Inhabited.default :=
  rfl

theorem pi.default_apply {α : Sort u} {β : α → Sort v} [(a : α) → Inhabited (β a)] (a : α) : Inhabited.default a = Inhabited.default :=
  rfl

protected instance pi.unique {α : Sort u} {β : α → Sort v} [(a : α) → unique (β a)] : unique ((a : α) → β a) :=
...
protected def surjective.unique {α : Sort u} {β : Sort v} {f : α → β} (hf : surjective f) [unique α] : unique β :=
  unique.mk { default := f Inhabited.default } sorry

/-- If the codomain of an injective function is a subsingleton, then the domain
is a subsingleton as well. -/
protected theorem injective.subsingleton {α : Sort u} {β : Sort v} {f : α → β} (hf : injective f) [subsingleton β] : subsingleton α :=
  subsingleton.intro fun (x y : α) => hf (subsingleton.elim (f x) (f y))

/-- If `α` is inhabited and admits an injective map to a subsingleton type, then `α` is `unique`. -/
protected def injective.unique {α : Sort u} {β : Sort v} {f : α → β} [Inhabited α] [subsingleton β] (hf : injective f) : unique α :=
  unique.mk' α
