...
protected instance has_coe_to_sort : has_coe_to_sort TopCommRing := has_coe_to_sort.mk (Type u) α
...
protected instance category_theory.concrete_category :
  ...
  category_theory.concrete_category.mk
    (category_theory.functor.mk (fun (R : TopCommRing) => ↥R)
      fun (R S : TopCommRing) (f : R ⟶ S) => ⇑(subtype.val f))

/-- Construct a bundled `TopCommRing` from the underlying type and the appropriate typeclasses. -/
def of (X : Type u) [comm_ring X] [topological_space X] [topological_ring X] : TopCommRing := mk X

@[simp] theorem coe_of (X : Type u) [comm_ring X] [topological_space X] [topological_ring X] :
    ↥(of X) = X :=
  rfl

protected instance forget_topological_space (R : TopCommRing) :
    topological_space (category_theory.functor.obj (category_theory.forget TopCommRing) R) :=
  is_topological_space R

protected instance forget_comm_ring (R : TopCommRing) :
    comm_ring (category_theory.functor.obj (category_theory.forget TopCommRing) R) :=
  is_comm_ring R

protected instance forget_topological_ring (R : TopCommRing) :
    topological_ring (category_theory.functor.obj (category_theory.forget TopCommRing) R) :=
...
protected instance forget_to_CommRing_topological_space (R : TopCommRing) :
    topological_space
        ↥(category_theory.functor.obj (category_theory.forget₂ TopCommRing CommRing) R) :=
  is_topological_space R
...
protected instance forget_to_Top_comm_ring (R : TopCommRing) :
    comm_ring ↥(category_theory.functor.obj (category_theory.forget₂ TopCommRing Top) R) :=
  is_comm_ring R

protected instance forget_to_Top_topological_ring (R : TopCommRing) :
    topological_ring ↥(category_theory.functor.obj (category_theory.forget₂ TopCommRing Top) R) :=
  is_topological_ring R
...
-/
protected instance category_theory.forget₂.category_theory.reflects_isomorphisms :
  ...
  category_theory.reflects_isomorphisms.mk
    fun (X Y : TopCommRing) (f : X ⟶ Y)
      ...
      category_theory.is_iso
        (category_theory.functor.map (category_theory.forget₂ TopCommRing Top) f)) =>
      let i_Top :
        ...
        ring_equiv.mk (ring_hom.to_fun (subtype.val f))
          (equiv.inv_fun
            (category_theory.iso.to_equiv
              (category_theory.functor.map_iso (category_theory.forget Top) i_Top)))
