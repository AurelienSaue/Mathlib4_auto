...
theorem tendsto_extend_from {X : Type u_1} {Y : Type u_2} [topological_space X]
    [topological_space Y] {A : set X} {f : X → Y} {x : X}
    (h : ∃ (y : Y), filter.tendsto f (nhds_within x A) (nhds y)) :
    filter.tendsto f (nhds_within x A) (nhds (extend_from A f x)) :=
  tendsto_nhds_lim h

theorem extend_from_eq {X : Type u_1} {Y : Type u_2} [topological_space X] [topological_space Y]
    [t2_space Y] {A : set X} {f : X → Y} {x : X} {y : Y} (hx : x ∈ closure A)
    (hf : filter.tendsto f (nhds_within x A) (nhds y)) : extend_from A f x = y :=
  tendsto_nhds_unique (tendsto_nhds_lim (Exists.intro y hf)) hf

theorem extend_from_extends {X : Type u_1} {Y : Type u_2} [topological_space X]
    [topological_space Y] [t2_space Y] {f : X → Y} {A : set X} (hf : continuous_on f A) (x : X)
    (H : x ∈ A) : extend_from A f x = f x :=
...
theorem continuous_on_extend_from {X : Type u_1} {Y : Type u_2} [topological_space X]
    [topological_space Y] [regular_space Y] {f : X → Y} {A : set X} {B : set X} (hB : B ⊆ closure A)
    (hf : ∀ (x : X), x ∈ B → ∃ (y : Y), filter.tendsto f (nhds_within x A) (nhds y)) :
...
theorem continuous_extend_from {X : Type u_1} {Y : Type u_2} [topological_space X]
    [topological_space Y] [regular_space Y] {f : X → Y} {A : set X} (hA : dense A)
    (hf : ∀ (x : X), ∃ (y : Y), filter.tendsto f (nhds_within x A) (nhds y)) :
