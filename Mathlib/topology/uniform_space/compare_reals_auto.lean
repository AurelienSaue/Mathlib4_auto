...
theorem rat.uniform_space_eq :
    is_absolute_value.uniform_space abs = metric_space.to_uniform_space' :=
  sorry

/-- Cauchy reals packaged as a completion of ℚ using the absolute value route. -/
def rational_cau_seq_pkg : abstract_completion ℚ :=
  abstract_completion.mk ℝ coe metric_space.to_uniform_space' real.complete_space
    metric_space.to_separated sorry sorry
...
protected instance Q.uniform_space : uniform_space Q := is_absolute_value.uniform_space abs

/-- Real numbers constructed as in Bourbaki. -/
def Bourbakiℝ := uniform_space.completion Q
...
def Bourbaki_pkg : abstract_completion Q := uniform_space.completion.cpkg
