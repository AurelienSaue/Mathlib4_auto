...
theorem ultrafilter_is_open_basic {α : Type u} (s : set α) :
    is_open (set_of fun (u : ultrafilter α) => s ∈ u) :=
...
theorem ultrafilter_is_closed_basic {α : Type u} (s : set α) :
    is_closed (set_of fun (u : ultrafilter α) => s ∈ u) :=
  sorry

/-- Every ultrafilter `u` on `ultrafilter α` converges to a unique
  point of `ultrafilter α`, namely `mjoin u`. -/
theorem ultrafilter_converges_iff {α : Type u} {u : ultrafilter (ultrafilter α)}
    {x : ultrafilter α} : ↑u ≤ nhds x ↔ x = mjoin u :=
  sorry
...
protected instance ultrafilter.t2_space {α : Type u} : t2_space (ultrafilter α) :=
  iff.mpr t2_iff_ultrafilter
    fun (x y : ultrafilter α) (f : ultrafilter (ultrafilter α)) (fx : ↑f ≤ nhds x)
      (fy : ↑f ≤ nhds y) =>
      (fun (hx : x = mjoin f) =>
          (fun (hy : y = mjoin f) => Eq.trans hx (Eq.symm hy))
            (iff.mp ultrafilter_converges_iff fy))
...
theorem induced_topology_pure {α : Type u} :
    topological_space.induced pure ultrafilter.topological_space = ⊥ :=
  sorry
...
theorem dense_embedding_pure {α : Type u} : dense_embedding pure :=
  dense_embedding.mk
    (dense_inducing.mk (dense_inducing.to_inducing dense_inducing_pure)
      (dense_inducing.dense dense_inducing_pure))
...
theorem ultrafilter_extend_extends {α : Type u} {γ : Type u_1} [topological_space γ] [t2_space γ]
    (f : α → γ) : ultrafilter.extend f ∘ pure = f :=
  let _inst : topological_space α := ⊥;
  funext (dense_inducing.extend_eq dense_inducing_pure continuous_of_discrete_topology)

theorem continuous_ultrafilter_extend {α : Type u} {γ : Type u_1} [topological_space γ] [t2_space γ]
    [compact_space γ] (f : α → γ) : continuous (ultrafilter.extend f) :=
  sorry

/-- The value of `ultrafilter.extend f` on an ultrafilter `b` is the
  unique limit of the ultrafilter `b.map f` in `γ`. -/
theorem ultrafilter_extend_eq_iff {α : Type u} {γ : Type u_1} [topological_space γ] [t2_space γ]
    [compact_space γ] {f : α → γ} {b : ultrafilter α} {c : γ} :
...
protected instance stone_cech_setoid (α : Type u) [topological_space α] : setoid (ultrafilter α) :=
  setoid.mk
    (fun (x y : ultrafilter α) =>
      ∀ (γ : Type u) [_inst_2 : topological_space γ] [_inst_3 : t2_space γ]
        [_inst_4 : compact_space γ] (f : α → γ),
        continuous f → ultrafilter.extend f x = ultrafilter.extend f y)
    sorry

/-- The Stone-Čech compactification of a topological space. -/
def stone_cech (α : Type u) [topological_space α] := quotient (Mathlib.stone_cech_setoid α)

protected instance stone_cech.topological_space {α : Type u} [topological_space α] :
    topological_space (stone_cech α) :=
  eq.mpr sorry quotient.topological_space

protected instance stone_cech.inhabited {α : Type u} [topological_space α] [Inhabited α] :
...
theorem dense_range_stone_cech_unit {α : Type u} [topological_space α] :
...
def stone_cech_extend {α : Type u} [topological_space α] {γ : Type u} [topological_space γ]
    [t2_space γ] [compact_space γ] {f : α → γ} (hf : continuous f) : stone_cech α → γ :=
  quotient.lift (ultrafilter.extend f) sorry

theorem stone_cech_extend_extends {α : Type u} [topological_space α] {γ : Type u}
    [topological_space γ] [t2_space γ] [compact_space γ] {f : α → γ} (hf : continuous f) :
    stone_cech_extend hf ∘ stone_cech_unit = f :=
  ultrafilter_extend_extends f

theorem continuous_stone_cech_extend {α : Type u} [topological_space α] {γ : Type u}
    [topological_space γ] [t2_space γ] [compact_space γ] {f : α → γ} (hf : continuous f) :
    continuous (stone_cech_extend hf) :=
  continuous_quot_lift (stone_cech_extend._proof_1 hf) (continuous_ultrafilter_extend f)

theorem convergent_eqv_pure {α : Type u} [topological_space α] {u : ultrafilter α} {x : α}
    (ux : ↑u ≤ nhds x) : u ≈ pure x :=
  sorry

theorem continuous_stone_cech_unit {α : Type u} [topological_space α] :
    continuous stone_cech_unit :=
  sorry

protected instance stone_cech.t2_space {α : Type u} [topological_space α] :
    t2_space (stone_cech α) :=
  sorry

protected instance stone_cech.compact_space {α : Type u} [topological_space α] :
