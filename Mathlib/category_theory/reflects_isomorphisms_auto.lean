...
-/
class reflects_isomorphisms {C : Type u₁} [category C] {D : Type u₂} [category D] (F : C ⥤ D) where
  reflects : {A B : C} → (f : A ⟶ B) → [_inst_3 : is_iso (functor.map F f)] → is_iso f

/-- If `F` reflects isos and `F.map f` is an iso, then `f` is an iso. -/
def is_iso_of_reflects_iso {C : Type u₁} [category C] {D : Type u₂} [category D] {A : C} {B : C}
    (f : A ⟶ B) (F : C ⥤ D) [is_iso (functor.map F f)] [reflects_isomorphisms F] : is_iso f :=
  reflects_isomorphisms.reflects F f

protected instance of_full_and_faithful {C : Type u₁} [category C] {D : Type u₂} [category D]
    (F : C ⥤ D) [full F] [faithful F] : reflects_isomorphisms F :=
  reflects_isomorphisms.mk
    fun (X Y : C) (f : X ⟶ Y) (i : is_iso (functor.map F f)) =>
