...
/-- Category of groupoids -/
def Groupoid := bundled groupoid

namespace Groupoid


protected instance inhabited : Inhabited Groupoid := { default := bundled.of (single_obj PUnit) }

protected instance str (C : Groupoid) : groupoid (bundled.α C) := bundled.str C

/-- Construct a bundled `Groupoid` from the underlying type and the typeclass. -/
def of (C : Type u) [groupoid C] : Groupoid := bundled.of C

/-- Category structure on `Groupoid` -/
protected instance category : large_category Groupoid := category.mk
...
protected instance forget_to_Cat_full : full forget_to_Cat := full.mk fun (C D : Groupoid) => id

protected instance forget_to_Cat_faithful : faithful forget_to_Cat := faithful.mk
