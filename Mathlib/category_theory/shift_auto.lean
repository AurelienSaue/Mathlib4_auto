...
class has_shift (C : Type u) [category C] 
where
  shift : C ≌ C

/-- The shift autoequivalence, moving objects and morphisms 'up'. -/
def shift (C : Type u) [category C] [has_shift C] : C ≌ C :=
  has_shift.shift

-- Any better notational suggestions?

@[simp] theorem shift_zero_eq_zero (C : Type u) [category C] [has_shift C] [limits.has_zero_morphisms C] (X : C) (Y : C) (n : ℤ) : functor.map (equivalence.functor (shift C ^ n)) 0 = 0 :=
