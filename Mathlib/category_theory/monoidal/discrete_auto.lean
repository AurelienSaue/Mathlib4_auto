...
protected instance monoid_discrete (M : Type u) [monoid M] : monoid (discrete M) := id _inst_1

protected instance discrete.monoidal_category (M : Type u) [monoid M] :
...
-/
@[simp] theorem discrete.monoidal_functor_μ_is_iso {M : Type u} [monoid M] {N : Type u} [monoid N]
    (F : M →* N) (X : discrete M) (Y : discrete M) :
    monoidal_functor.μ_is_iso (discrete.monoidal_functor F) X Y =
        discrete.category_theory.is_iso
          (lax_monoidal_functor.μ
            (lax_monoidal_functor.mk
              (functor.mk ⇑F
                fun (X Y : discrete M) (f : X ⟶ Y) =>
                  eq_to_hom (discrete.monoidal_functor._proof_1 F X Y f))
...
-/
def discrete.monoidal_functor_comp {M : Type u} [monoid M] {N : Type u} [monoid N] {K : Type u}
    [monoid K] (F : M →* N) (G : N →* K) :
    discrete.monoidal_functor F ⊗⋙ discrete.monoidal_functor G ≅
        discrete.monoidal_functor (monoid_hom.comp G F) :=
