...
protected instance has_finite_limits {C : Type u} [category C] {B : C} [limits.has_finite_wide_pullbacks C] : limits.has_finite_limits (over B) :=
  limits.finite_limits_from_equalizers_and_finite_products

protected instance has_limits {C : Type u} [category C] {B : C} [limits.has_wide_pullbacks C] : limits.has_limits (over B) :=
