...
theorem has_limit_cospan_of_has_limit_pair_of_has_limit_parallel_pair {C : Type u} [𝒞 : category C]
    {X : C} {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) [has_limit (pair X Y)]
...
/-- If a category has all binary products and all equalizers, then it also has all pullbacks.
    ...
    pullbacks. -/
theorem has_pullbacks_of_has_binary_products_of_has_equalizers (C : Type u) [𝒞 : category C]
    [has_binary_products C] [has_equalizers C] : has_pullbacks C :=
...
theorem has_colimit_span_of_has_colimit_pair_of_has_colimit_parallel_pair {C : Type u}
    [𝒞 : category C] {X : C} {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) [has_colimit (pair Y Z)]
...
/-- If a category has all binary coproducts and all coequalizers, then it also has all pushouts.
    As usual, this is not an instance, since there may be a more direct way to construct
    pushouts. -/
theorem has_pushouts_of_has_binary_coproducts_of_has_coequalizers (C : Type u) [𝒞 : category C]
