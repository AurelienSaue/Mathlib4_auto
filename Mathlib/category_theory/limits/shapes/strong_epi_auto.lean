...
class strong_epi {C : Type u} [category C] {P : C} {Q : C} (f : P ⟶ Q) where
  epi : epi f
  has_lift :
    ∀ {X Y : C} {u : P ⟶ X} {v : Q ⟶ Y} {z : X ⟶ Y} [_inst_2 : mono z] (h : u ≫ z = f ≫ v),
      arrow.has_lift (arrow.hom_mk' h)

protected instance epi_of_strong_epi {C : Type u} [category C] {P : C} {Q : C} (f : P ⟶ Q)
    [strong_epi f] : epi f :=
  strong_epi.epi

/-- The composition of two strong epimorphisms is a strong epimorphism. -/
theorem strong_epi_comp {C : Type u} [category C] {P : C} {Q : C} {R : C} (f : P ⟶ Q) (g : Q ⟶ R)
    [strong_epi f] [strong_epi g] : strong_epi (f ≫ g) :=
  sorry

/-- If `f ≫ g` is a strong epimorphism, then so is g. -/
theorem strong_epi_of_strong_epi {C : Type u} [category C] {P : C} {Q : C} {R : C} (f : P ⟶ Q)
    (g : Q ⟶ R) [strong_epi (f ≫ g)] : strong_epi g :=
  sorry

/-- An isomorphism is in particular a strong epimorphism. -/
protected instance strong_epi_of_is_iso {C : Type u} [category C] {P : C} {Q : C} (f : P ⟶ Q)
    [is_iso f] : strong_epi f :=
  sorry

/-- A strong epimorphism that is a monomorphism is an isomorphism. -/
def is_iso_of_mono_of_strong_epi {C : Type u} [category C] {P : C} {Q : C} (f : P ⟶ Q) [mono f]
    [strong_epi f] : is_iso f :=
