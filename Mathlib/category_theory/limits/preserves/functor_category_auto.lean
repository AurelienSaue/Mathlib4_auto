...
-/
def functor_category.prod_preserves_colimits {C : Type v₂} [category C] {D : Type u₂} [category D]
    [limits.has_binary_products D] [limits.has_colimits D]
    [(X : D) → limits.preserves_colimits (functor.obj limits.prod.functor X)] (F : C ⥤ D) :
    limits.preserves_colimits (functor.obj limits.prod.functor F) :=
  sorry
