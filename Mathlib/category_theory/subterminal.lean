...
-/
theorem is_subterminal.mono_is_terminal_from {C : Type u₁} [category C] {A : C} (hA : is_subterminal A) {T : C} (hT : limits.is_terminal T) : mono (limits.is_terminal.from hT A) :=
  mono.mk fun (Z : C) (g h : Z ⟶ A) (_x : g ≫ limits.is_terminal.from hT A = h ≫ limits.is_terminal.from hT A) => hA g h
...
-/
theorem is_subterminal.mono_terminal_from {C : Type u₁} [category C] {A : C} [limits.has_terminal C] (hA : is_subterminal A) : mono (limits.terminal.from A) :=
...
-/
theorem is_subterminal_of_mono_is_terminal_from {C : Type u₁} [category C] {A : C} {T : C} (hT : limits.is_terminal T) [mono (limits.is_terminal.from hT A)] : is_subterminal A :=
  fun (Z : C) (f g : Z ⟶ A) =>
    eq.mpr (id (Eq._oldrec (Eq.refl (f = g)) (Eq.symm (propext (cancel_mono (limits.is_terminal.from hT A))))))
      (limits.is_terminal.hom_ext hT (f ≫ limits.is_terminal.from hT A) (g ≫ limits.is_terminal.from hT A))
...
-/
theorem is_subterminal_of_mono_terminal_from {C : Type u₁} [category C] {A : C} [limits.has_terminal C] [mono (limits.terminal.from A)] : is_subterminal A :=
  fun (Z : C) (f g : Z ⟶ A) =>
    eq.mpr (id (Eq._oldrec (Eq.refl (f = g)) (Eq.symm (propext (cancel_mono (limits.terminal.from A))))))
      (subsingleton.elim (f ≫ limits.terminal.from A) (g ≫ limits.terminal.from A))

theorem is_subterminal_of_is_terminal {C : Type u₁} [category C] {T : C} (hT : limits.is_terminal T) : is_subterminal T :=
  fun (Z : C) (f g : Z ⟶ T) => limits.is_terminal.hom_ext hT f g

theorem is_subterminal_of_terminal {C : Type u₁} [category C] [limits.has_terminal C] : is_subterminal (⊤_C) :=
...
-/
def is_subterminal.is_iso_diag {C : Type u₁} [category C] {A : C} (hA : is_subterminal A) [limits.has_binary_product A A] : is_iso (limits.diag A) :=
...
-/
theorem is_subterminal_of_is_iso_diag {C : Type u₁} [category C] {A : C} [limits.has_binary_product A A] [is_iso (limits.diag A)] : is_subterminal A := sorry

/-- If `A` is subterminal, it is isomorphic to `A ⨯ A`. -/
@[simp] theorem is_subterminal.iso_diag_hom {C : Type u₁} [category C] {A : C} (hA : is_subterminal A) [limits.has_binary_product A A] : iso.hom (is_subterminal.iso_diag hA) = inv (limits.diag A) :=
