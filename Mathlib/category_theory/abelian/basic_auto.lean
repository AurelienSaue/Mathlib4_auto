...
-/
class abelian (C : Type u) [category C] extends preadditive C where
...
theorem has_finite_biproducts {C : Type u} [category C] [abelian C] :
    limits.has_finite_biproducts C :=
...
theorem strong_epi_of_epi {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    [epi f] : strong_epi f :=
...
def is_iso_of_mono_of_epi {C : Type u} [category C] [abelian C] {X : C} {Y : C} (f : X ⟶ Y) [mono f]
    [epi f] : is_iso f :=
  is_iso_of_mono_of_strong_epi f

theorem mono_of_zero_kernel {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    (R : C)
    (l :
      limits.is_limit
        (limits.kernel_fork.of_ι 0
          ((fun (this : 0 ≫ f = 0) => this)
            (eq.mpr
              (id
                (Eq.trans
                  ((fun (a a_1 : R ⟶ Q) (e_1 : a = a_1) (ᾰ ᾰ_1 : R ⟶ Q) (e_2 : ᾰ = ᾰ_1) =>
...
theorem mono_of_kernel_ι_eq_zero {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    (h : limits.kernel.ι f = 0) : mono f :=
  preadditive.mono_of_kernel_zero h

theorem epi_of_zero_cokernel {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    (R : C)
    (l :
      limits.is_colimit
        (limits.cokernel_cofork.of_π 0
          ((fun (this : f ≫ 0 = 0) => this)
            (eq.mpr
              (id
                (Eq.trans
                  ((fun (a a_1 : P ⟶ R) (e_1 : a = a_1) (ᾰ ᾰ_1 : P ⟶ R) (e_2 : ᾰ = ᾰ_1) =>
                      congr (congr_arg Eq e_1) e_2)
...
theorem epi_of_cokernel_π_eq_zero {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    (h : limits.cokernel.π f = 0) : epi f :=
  sorry
...
protected def image.ι {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    images.image f ⟶ Q :=
  limits.kernel.ι (limits.cokernel.π f)

/-- There is a canonical epimorphism `p : P ⟶ image f` for every `f`. -/
protected def factor_thru_image {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    P ⟶ images.image f :=
  limits.kernel.lift (limits.cokernel.π f) f sorry

/-- `f` factors through its image via the canonical morphism `p`. -/
@[simp] theorem image.fac_assoc {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q)
    {X' : C} (f' : Q ⟶ X') : images.factor_thru_image f ≫ image.ι f ≫ f' = f ≫ f' :=
  sorry

/-- The map `p : P ⟶ image f` is an epimorphism -/
protected instance factor_thru_image.category_theory.epi {C : Type u} [category C] [abelian C]
    {P : C} {Q : C} (f : P ⟶ Q) : epi (images.factor_thru_image f) :=
...
theorem image_ι_comp_eq_zero {C : Type u} [category C] [abelian C] {P : C} {Q : C} {f : P ⟶ Q}
    {R : C} {g : Q ⟶ R} (h : f ≫ g = 0) : image.ι f ≫ g = 0 :=
  sorry

protected instance mono_factor_thru_image {C : Type u} [category C] [abelian C] {P : C} {Q : C}
    (f : P ⟶ Q) [mono f] : mono (images.factor_thru_image f) :=
  mono_of_mono_fac (image.fac f)

protected instance is_iso_factor_thru_image {C : Type u} [category C] [abelian C] {P : C} {Q : C}
    (f : P ⟶ Q) [mono f] : is_iso (images.factor_thru_image f) :=
  is_iso_of_mono_of_epi (images.factor_thru_image f)

/-- Factoring through the image is a strong epi-mono factorisation. -/
@[simp] theorem image_strong_epi_mono_factorisation_to_mono_factorisation_m {C : Type u}
    [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    limits.mono_factorisation.m
          (limits.strong_epi_mono_factorisation.to_mono_factorisation
            (image_strong_epi_mono_factorisation f)) =
        image.ι f :=
  Eq.refl
    (limits.mono_factorisation.m
      (limits.strong_epi_mono_factorisation.to_mono_factorisation
...
protected def coimage.π {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    P ⟶ coimages.coimage f :=
  limits.cokernel.π (limits.kernel.ι f)

/-- There is a canonical monomorphism `i : coimage f ⟶ Q`. -/
protected def factor_thru_coimage {C : Type u} [category C] [abelian C] {P : C} {Q : C}
    (f : P ⟶ Q) : coimages.coimage f ⟶ Q :=
  limits.cokernel.desc (limits.kernel.ι f) f sorry

/-- `f` factors through its coimage via the canonical morphism `p`. -/
protected theorem coimage.fac {C : Type u} [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    coimage.π f ≫ coimages.factor_thru_coimage f = f :=
  limits.cokernel.π_desc (limits.kernel.ι f) f (factor_thru_coimage._proof_1 f)

/-- The canonical morphism `i : coimage f ⟶ Q` is a monomorphism -/
protected instance factor_thru_coimage.category_theory.mono {C : Type u} [category C] [abelian C]
    {P : C} {Q : C} (f : P ⟶ Q) : mono (coimages.factor_thru_coimage f) :=
...
protected instance epi_factor_thru_coimage {C : Type u} [category C] [abelian C] {P : C} {Q : C}
    (f : P ⟶ Q) [epi f] : epi (coimages.factor_thru_coimage f) :=
  epi_of_epi_fac (coimage.fac f)

protected instance is_iso_factor_thru_coimage {C : Type u} [category C] [abelian C] {P : C} {Q : C}
    (f : P ⟶ Q) [epi f] : is_iso (coimages.factor_thru_coimage f) :=
  is_iso_of_mono_of_epi (coimages.factor_thru_coimage f)

/-- Factoring through the coimage is a strong epi-mono factorisation. -/
@[simp] theorem coimage_strong_epi_mono_factorisation_to_mono_factorisation_m {C : Type u}
    [category C] [abelian C] {P : C} {Q : C} (f : P ⟶ Q) :
    limits.mono_factorisation.m
          (limits.strong_epi_mono_factorisation.to_mono_factorisation
            (coimage_strong_epi_mono_factorisation f)) =
...
protected instance category_theory.limits.has_strong_epi_mono_factorisations {C : Type u}
    [category C] [abelian C] : limits.has_strong_epi_mono_factorisations C :=
  limits.has_strong_epi_mono_factorisations.mk
    fun (X Y : C) (f : X ⟶ Y) => images.image_strong_epi_mono_factorisation f
...
def coimage_iso_image {C : Type u} [category C] [abelian C] {X : C} {Y : C} (f : X ⟶ Y) :
    coimages.coimage f ≅ images.image f :=
  limits.is_image.iso_ext
    (limits.strong_epi_mono_factorisation.to_mono_is_image
      (coimages.coimage_strong_epi_mono_factorisation f))
    (limits.strong_epi_mono_factorisation.to_mono_is_image
      (images.image_strong_epi_mono_factorisation f))

/-- There is a canonical isomorphism between the abelian image and the categorical image of a
    morphism. -/
def image_iso_image {C : Type u} [category C] [abelian C] {X : C} {Y : C} (f : X ⟶ Y) :
    images.image f ≅ limits.image f :=
  limits.is_image.iso_ext
    (limits.strong_epi_mono_factorisation.to_mono_is_image
      (images.image_strong_epi_mono_factorisation f))
...
/-- There is a canonical isomorphism between the abelian coimage and the categorical image of a
    morphism. -/
def coimage_iso_image' {C : Type u} [category C] [abelian C] {X : C} {Y : C} (f : X ⟶ Y) :
    coimages.coimage f ≅ limits.image f :=
  limits.is_image.iso_ext
    (limits.strong_epi_mono_factorisation.to_mono_is_image
      (coimages.coimage_strong_epi_mono_factorisation f))
    (limits.image.is_image f)

theorem full_image_factorisation {C : Type u} [category C] [abelian C] {X : C} {Y : C} (f : X ⟶ Y) :
    coimages.coimage.π f ≫ iso.hom (coimage_iso_image f) ≫ images.image.ι f = f :=
  sorry

/-- In an abelian category, an epi is the cokernel of its kernel. More precisely:
    If `f` is an epimorphism and `s` is some limit kernel cone on `f`, then `f` is a cokernel
    of `fork.ι s`. -/
def epi_is_cokernel_of_kernel {C : Type u} [category C] [abelian C] {X : C} {Y : C} {f : X ⟶ Y}
    [epi f] (s : limits.fork f 0) (h : limits.is_limit s) :
...
/-- In an abelian category, a mono is the kernel of its cokernel. More precisely:
    ...
    of `cofork.π s`. -/
def mono_is_kernel_of_cokernel {C : Type u} [category C] [abelian C] {X : C} {Y : C} {f : X ⟶ Y}
    [mono f] (s : limits.cofork f 0) (h : limits.is_colimit s) :
...
def pullback_to_biproduct {C : Type u} [category C] [abelian C] [limits.has_pullbacks C] {X : C}
    {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) : limits.pullback f g ⟶ X ⊞ Y :=
...
/-- The canonical map `pullback f g ⟶ X ⊞ Y` induces a kernel cone on the map
    ...
    `(0, g)`. -/
def pullback_to_biproduct_fork {C : Type u} [category C] [abelian C] [limits.has_pullbacks C]
    {X : C} {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) :
    limits.kernel_fork (limits.biprod.desc f (-g)) :=
...
/-- The canonical map `pullback f g ⟶ X ⊞ Y` is a kernel of the map induced by
    `(f, -g)`. -/
def is_limit_pullback_to_biproduct {C : Type u} [category C] [abelian C] [limits.has_pullbacks C]
    {X : C} {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) :
    limits.is_limit (pullback_to_biproduct_fork f g) :=
  limits.fork.is_limit.mk (pullback_to_biproduct_fork f g)
    (fun (s : limits.fork (limits.biprod.desc f (-g)) 0) =>
      limits.pullback.lift (limits.fork.ι s ≫ limits.biprod.fst)
        (limits.fork.ι s ≫ limits.biprod.snd) sorry)
    sorry sorry
...
def biproduct_to_pushout {C : Type u} [category C] [abelian C] [limits.has_pushouts C] {X : C}
    {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) : Y ⊞ Z ⟶ limits.pushout f g :=
...
def biproduct_to_pushout_cofork {C : Type u} [category C] [abelian C] [limits.has_pushouts C]
    {X : C} {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) :
    limits.cokernel_cofork (limits.biprod.lift f (-g)) :=
...
/-- The cofork induced by the canonical map `Y ⊞ Z ⟶ pushout f g` is in fact a colimit cokernel
    cofork. -/
def is_colimit_biproduct_to_pushout {C : Type u} [category C] [abelian C] [limits.has_pushouts C]
    {X : C} {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) :
    limits.is_colimit (biproduct_to_pushout_cofork f g) :=
  limits.cofork.is_colimit.mk (biproduct_to_pushout_cofork f g)
    (fun (s : limits.cofork (limits.biprod.lift f (-g)) 0) =>
      limits.pushout.desc (limits.biprod.inl ≫ limits.cofork.π s)
        (limits.biprod.inr ≫ limits.cofork.π s) sorry)
    sorry sorry
...
protected instance epi_pullback_of_epi_f {C : Type u} [category C] [abelian C]
    [limits.has_pullbacks C] {X : C} {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) [epi f] :
    epi limits.pullback.snd :=
  sorry
...
protected instance epi_pullback_of_epi_g {C : Type u} [category C] [abelian C]
    [limits.has_pullbacks C] {X : C} {Y : C} {Z : C} (f : X ⟶ Z) (g : Y ⟶ Z) [epi g] :
    epi limits.pullback.fst :=
  sorry

-- pullback.fst ≫ e = 0 and show that e = 0.

protected instance mono_pushout_of_mono_f {C : Type u} [category C] [abelian C]
    [limits.has_pushouts C] {X : C} {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) [mono f] :
    mono limits.pushout.inr :=
  sorry

protected instance mono_pushout_of_mono_g {C : Type u} [category C] [abelian C]
    [limits.has_pushouts C] {X : C} {Y : C} {Z : C} (f : X ⟶ Y) (g : X ⟶ Z) [mono g] :
    mono limits.pushout.inl :=
  sorry
