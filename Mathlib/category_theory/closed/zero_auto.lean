...
-/
def unique_homset_of_initial_iso_terminal {C : Type u} [category C] [limits.has_finite_products C]
    [cartesian_closed C] [limits.has_initial C] (i : ⊥_C ≅ ⊤_C) (X : C) (Y : C) : unique (X ⟶ Y) :=
  equiv.unique
    (equiv.trans
...
def unique_homset_of_zero {C : Type u} [category C] [limits.has_finite_products C]
    [cartesian_closed C] [limits.has_zero_object C] (X : C) (Y : C) : unique (X ⟶ Y) :=
  unique_homset_of_initial_iso_terminal
    (iso.mk Inhabited.default (Inhabited.default ≫ Inhabited.default)) X Y

/--
A cartesian closed category with a zero object is equivalent to the category with one object and
one morphism.
-/
def equiv_punit {C : Type u} [category C] [limits.has_finite_products C] [cartesian_closed C]
    [limits.has_zero_object C] : C ≌ discrete PUnit :=
