...
-/
class is_preconnected (J : Type u₁) [category J] where
  iso_constant :
    ∀ {α : Type u₁} (F : J ⥤ discrete α) (j : J),
      Nonempty (F ≅ functor.obj (functor.const J) (functor.obj F j))
...
-/
class is_connected (J : Type u₁) [category J] extends is_preconnected J where
...
-/
def iso_constant {J : Type u₁} [category J] [is_preconnected J] {α : Type u₁} (F : J ⥤ discrete α)
    (j : J) : F ≅ functor.obj (functor.const J) (functor.obj F j) :=
...
-/
theorem any_functor_const_on_obj {J : Type u₁} [category J] [is_preconnected J] {α : Type u₁}
    (F : J ⥤ discrete α) (j : J) (j' : J) : functor.obj F j = functor.obj F j' :=
...
-/
theorem is_connected.of_any_functor_const_on_obj {J : Type u₁} [category J] [Nonempty J]
    (h : ∀ {α : Type u₁} (F : J ⥤ discrete α) (j j' : J), functor.obj F j = functor.obj F j') :
    is_connected J :=
  is_connected.mk
...
-/
theorem constant_of_preserves_morphisms {J : Type u₁} [category J] [is_preconnected J] {α : Type u₁}
    (F : J → α) (h : ∀ (j₁ j₂ : J), (j₁ ⟶ j₂) → F j₁ = F j₂) (j : J) (j' : J) : F j = F j' :=
  any_functor_const_on_obj
    (functor.mk F fun (_x _x_1 : J) (f : _x ⟶ _x_1) => eq_to_hom (h _x _x_1 f)) j j'
...
-/
theorem is_connected.of_constant_of_preserves_morphisms {J : Type u₁} [category J] [Nonempty J]
    (h :
      ∀ {α : Type u₁} (F : J → α),
        (∀ {j₁ j₂ : J}, (j₁ ⟶ j₂) → F j₁ = F j₂) → ∀ (j j' : J), F j = F j') :
    is_connected J :=
  is_connected.of_any_functor_const_on_obj
    fun (_x : Type u₁) (F : J ⥤ discrete _x) =>
      h (functor.obj F)
        fun (_x_1 _x_2 : J) (f : _x_1 ⟶ _x_2) => plift.down (ulift.down (functor.map F f))
...
-/
theorem induct_on_objects {J : Type u₁} [category J] [is_preconnected J] (p : set J) {j₀ : J}
    (h0 : j₀ ∈ p) (h1 : ∀ {j₁ j₂ : J}, (j₁ ⟶ j₂) → (j₁ ∈ p ↔ j₂ ∈ p)) (j : J) : j ∈ p :=
  sorry
...
-/
theorem is_connected.of_induct {J : Type u₁} [category J] [Nonempty J] {j₀ : J}
    (h :
...
-/
theorem is_preconnected_induction {J : Type u₁} [category J] [is_preconnected J] (Z : J → Sort u_1)
    (h₁ : {j₁ j₂ : J} → (j₁ ⟶ j₂) → Z j₁ → Z j₂) (h₂ : {j₁ j₂ : J} → (j₁ ⟶ j₂) → Z j₂ → Z j₁)
    {j₀ : J} (x : Z j₀) (j : J) : Nonempty (Z j) :=
  sorry

/-- If `J` and `K` are equivalent, then if `J` is preconnected then `K` is as well. -/
theorem is_preconnected_of_equivalent {J : Type u₁} [category J] {K : Type u₁} [category K]
    [is_preconnected J] (e : J ≌ K) : is_preconnected K :=
  sorry

/-- If `J` and `K` are equivalent, then if `J` is connected then `K` is as well. -/
theorem is_connected_of_equivalent {J : Type u₁} [category J] {K : Type u₁} [category K] (e : J ≌ K)
    [is_connected J] : is_connected K :=
  is_connected.mk

/-- j₁ and j₂ are related by `zag` if there is a morphism between them. -/
def zag {J : Type u₁} [category J] (j₁ : J) (j₂ : J) := Nonempty (j₁ ⟶ j₂) ∨ Nonempty (j₂ ⟶ j₁)
...
-/
def zigzag {J : Type u₁} [category J] : J → J → Prop := relation.refl_trans_gen zag
...
theorem zigzag_equivalence {J : Type u₁} [category J] : equivalence zigzag :=
  mk_equivalence zigzag relation.reflexive_refl_trans_gen zigzag_symmetric
...
-/
def zigzag.setoid (J : Type u₂) [category J] : setoid J := setoid.mk zigzag zigzag_equivalence

/--
If there is a zigzag from `j₁` to `j₂`, then there is a zigzag from `F j₁` to
`F j₂` as long as `F` is a functor.
-/
theorem zigzag_obj_of_zigzag {J : Type u₁} [category J] {K : Type u₂} [category K] (F : J ⥤ K)
    {j₁ : J} {j₂ : J} (h : zigzag j₁ j₂) : zigzag (functor.obj F j₁) (functor.obj F j₂) :=
  sorry

-- TODO: figure out the right way to generalise this to `zigzag`.

theorem zag_of_zag_obj {J : Type u₁} [category J] {K : Type u₂} [category K] (F : J ⥤ K) [full F]
    {j₁ : J} {j₂ : J} (h : zag (functor.obj F j₁) (functor.obj F j₂)) : zag j₁ j₂ :=
  or.imp (nonempty.map (functor.preimage F)) (nonempty.map (functor.preimage F)) h

/-- Any equivalence relation containing (⟶) holds for all pairs of a connected category. -/
theorem equiv_relation {J : Type u₁} [category J] [is_connected J] (r : J → J → Prop)
    (hr : equivalence r) (h : ∀ {j₁ j₂ : J}, (j₁ ⟶ j₂) → r j₁ j₂) (j₁ : J) (j₂ : J) : r j₁ j₂ :=
  sorry

/-- In a connected category, any two objects are related by `zigzag`. -/
theorem is_connected_zigzag {J : Type u₁} [category J] [is_connected J] (j₁ : J) (j₂ : J) :
    zigzag j₁ j₂ :=
  equiv_relation zigzag zigzag_equivalence
    (fun (_x _x_1 : J) (f : _x ⟶ _x_1) =>
      relation.refl_trans_gen.single (Or.inl (Nonempty.intro f)))
    j₁ j₂

/--
If any two objects in an nonempty category are related by `zigzag`, the category is connected.
-/
theorem zigzag_is_connected {J : Type u₁} [category J] [Nonempty J]
    (h : ∀ (j₁ j₂ : J), zigzag j₁ j₂) : is_connected J :=
  sorry

theorem exists_zigzag' {J : Type u₁} [category J] [is_connected J] (j₁ : J) (j₂ : J) :
    ∃ (l : List J), list.chain zag j₁ l ∧ list.last (j₁ :: l) (list.cons_ne_nil j₁ l) = j₂ :=
...
-/
theorem is_connected_of_zigzag {J : Type u₁} [category J] [Nonempty J]
    (h :
      ∀ (j₁ j₂ : J),
        ∃ (l : List J), list.chain zag j₁ l ∧ list.last (j₁ :: l) (list.cons_ne_nil j₁ l) = j₂) :
    is_connected J :=
...
def discrete_is_connected_equiv_punit {α : Type (max u_1 u_2)} [is_connected (discrete α)] :
    α ≃ PUnit :=
  discrete.equiv_of_equivalence
    (equivalence.mk' (functor.star α)
      (discrete.functor fun (_x : PUnit) => classical.arbitrary (discrete α))
      (iso_constant 𝟭 (classical.arbitrary (discrete α)))
      (functor.punit_ext
        ((discrete.functor fun (_x : PUnit) => classical.arbitrary (discrete α)) ⋙ functor.star α)
        𝟭))
...
-/
theorem nat_trans_from_is_connected {J : Type u₁} [category J] {C : Type u₂} [category C]
    [is_preconnected J] {X : C} {Y : C}
    (α : functor.obj (functor.const J) X ⟶ functor.obj (functor.const J) Y) (j : J) (j' : J) :
    nat_trans.app α j = nat_trans.app α j' :=
  sorry
