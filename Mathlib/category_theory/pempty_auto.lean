...
def empty (C : Type u) [category C] : discrete pempty ⥤ C := discrete.functor pempty.elim

/-- Any two functors out of the empty category are isomorphic. -/
def empty_ext {C : Type u} [category C] (F : discrete pempty ⥤ C) (G : discrete pempty ⥤ C) :
    F ≅ G :=
...
-/
theorem empty_ext' {C : Type u} [category C] (F : discrete pempty ⥤ C) (G : discrete pempty ⥤ C) :
    F = G :=
  ext (fun (x : discrete pempty) => pempty.elim x)
    fun (x _x : discrete pempty) (_x_1 : x ⟶ _x) => pempty.elim x
