...
protected instance K.decidable_eq (tm : fin_tm2) : DecidableEq (K tm) := K_decidable_eq tm

protected instance σ.inhabited (tm : fin_tm2) : Inhabited (σ tm) := { default := initial_state tm }

/-- The type of statements (functions) corresponding to this TM. -/
def stmt (tm : fin_tm2) := TM2.stmt (Γ tm) (Λ tm) (σ tm)

/-- The type of configurations (functions) corresponding to this TM. -/
def cfg (tm : fin_tm2) := TM2.cfg (Γ tm) (Λ tm) (σ tm)
...
@[simp] def step (tm : fin_tm2) : cfg tm → Option (cfg tm) := TM2.step (M tm)
...
def init_list (tm : fin_tm2) (s : List (fin_tm2.Γ tm (fin_tm2.k₀ tm))) : fin_tm2.cfg tm :=
  TM2.cfg.mk (some (fin_tm2.main tm)) (fin_tm2.initial_state tm)
    fun (k : fin_tm2.K tm) =>
      dite (k = fin_tm2.k₀ tm) (fun (h : k = fin_tm2.k₀ tm) => eq.mpr sorry s)
        fun (_x : ¬k = fin_tm2.k₀ tm) => []
...
structure evals_to {σ : Type u_1} (f : σ → Option σ) (a : σ) (b : Option σ) where
  steps : ℕ
...
structure evals_to_in_time {σ : Type u_1} (f : σ → Option σ) (a : σ) (b : Option σ) (m : ℕ)
    extends evals_to f a b where
  steps_le_m : evals_to.steps _to_evals_to ≤ m

/-- Reflexivity of `evals_to` in 0 steps. -/
def evals_to.refl {σ : Type u_1} (f : σ → Option σ) (a : σ) : evals_to f a ↑a := evals_to.mk 0 sorry

/-- Transitivity of `evals_to` in the sum of the numbers of steps. -/
def evals_to.trans {σ : Type u_1} (f : σ → Option σ) (a : σ) (b : σ) (c : Option σ)
...
def evals_to_in_time.trans {σ : Type u_1} (f : σ → Option σ) (a : σ) (b : σ) (c : Option σ) (m₁ : ℕ)
    (m₂ : ℕ) (h₁ : evals_to_in_time f a (↑b) m₁) (h₂ : evals_to_in_time f b c m₂) :
    evals_to_in_time f a c (m₂ + m₁) :=
  evals_to_in_time.mk
    (evals_to.trans f a b c (evals_to_in_time.to_evals_to h₁) (evals_to_in_time.to_evals_to h₂))
    sorry

/-- A proof of tm outputting l' when given l. -/
def tm2_outputs (tm : fin_tm2) (l : List (fin_tm2.Γ tm (fin_tm2.k₀ tm)))
    (l' : Option (List (fin_tm2.Γ tm (fin_tm2.k₁ tm)))) :=
  evals_to (fin_tm2.step tm) (init_list tm l) (option.map (halt_list tm) l')

/-- A proof of tm outputting l' when given l in at most m steps. -/
def tm2_outputs_in_time (tm : fin_tm2) (l : List (fin_tm2.Γ tm (fin_tm2.k₀ tm)))
    (l' : Option (List (fin_tm2.Γ tm (fin_tm2.k₁ tm)))) (m : ℕ) :=
...
def tm2_outputs_in_time.to_tm2_outputs {tm : fin_tm2} {l : List (fin_tm2.Γ tm (fin_tm2.k₀ tm))}
    {l' : Option (List (fin_tm2.Γ tm (fin_tm2.k₁ tm)))} {m : ℕ}
...
structure tm2_computable_aux (Γ₀ : Type) (Γ₁ : Type) where
  tm : fin_tm2
...
structure tm2_computable {α : Type} {β : Type} (ea : computability.fin_encoding α)
    (eb : computability.fin_encoding β) (f : α → β)
    extends
      tm2_computable_aux (computability.encoding.Γ (computability.fin_encoding.to_encoding ea))
...
structure tm2_computable_in_time {α : Type} {β : Type} (ea : computability.fin_encoding α)
    (eb : computability.fin_encoding β) (f : α → β)
    extends
      tm2_computable_aux (computability.encoding.Γ (computability.fin_encoding.to_encoding ea))
        (computability.encoding.Γ (computability.fin_encoding.to_encoding eb))
...
steps. -/
structure tm2_computable_in_poly_time {α : Type} {β : Type} (ea : computability.fin_encoding α)
    (eb : computability.fin_encoding β) (f : α → β)
    extends
      tm2_computable_aux (computability.encoding.Γ (computability.fin_encoding.to_encoding ea))
        (computability.encoding.Γ (computability.fin_encoding.to_encoding eb))
...
def tm2_computable_in_time.to_tm2_computable {α : Type} {β : Type}
    {ea : computability.fin_encoding α} {eb : computability.fin_encoding β} {f : α → β}
...
def tm2_computable_in_poly_time.to_tm2_computable_in_time {α : Type} {β : Type}
    {ea : computability.fin_encoding α} {eb : computability.fin_encoding β} {f : α → β}
    (h : tm2_computable_in_poly_time ea eb f) : tm2_computable_in_time ea eb f :=
  tm2_computable_in_time.mk (tm2_computable_in_poly_time.to_tm2_computable_aux h)
    (fun (n : ℕ) => polynomial.eval n (tm2_computable_in_poly_time.time h))
    (tm2_computable_in_poly_time.outputs_fun h)
...
def id_computer {α : Type} (ea : computability.fin_encoding α) : fin_tm2 :=
  fin_tm2.mk PUnit.unit PUnit.unit
    (fun (_x : Unit) => computability.encoding.Γ (computability.fin_encoding.to_encoding ea)) Unit
    PUnit.unit Unit PUnit.unit fun (_x : Unit) => TM2.stmt.halt

protected instance inhabited_fin_tm2 : Inhabited fin_tm2 :=
  { default := id_computer Inhabited.default }

/-- A proof that the identity map on α is computable in polytime. -/
def id_computable_in_poly_time {α : Type} (ea : computability.fin_encoding α) :
    tm2_computable_in_poly_time ea ea id :=
  tm2_computable_in_poly_time.mk
    (tm2_computable_aux.mk (id_computer ea) (equiv.cast sorry) (equiv.cast sorry)) 1
    fun (_x : α) => evals_to_in_time.mk (evals_to.mk 1 sorry) sorry

protected instance inhabited_tm2_computable_in_poly_time :
    Inhabited (tm2_computable_in_poly_time Inhabited.default Inhabited.default id) :=
  { default := id_computable_in_poly_time Inhabited.default }

protected instance inhabited_tm2_outputs_in_time :
    Inhabited
        (tm2_outputs_in_time (id_computer computability.fin_encoding_bool_bool)
          ...
          (some
            (list.map (equiv.inv_fun (equiv.cast inhabited_tm2_outputs_in_time._proof_2)) [false]))
          (polynomial.eval
            (list.length
              (computability.encoding.encode
                (computability.fin_encoding.to_encoding computability.fin_encoding_bool_bool)
                false))
            (tm2_computable_in_poly_time.time
              (id_computable_in_poly_time computability.fin_encoding_bool_bool)))) :=
  { default :=
      tm2_computable_in_poly_time.outputs_fun
        (id_computable_in_poly_time computability.fin_encoding_bool_bool) false }

protected instance inhabited_tm2_outputs :
    Inhabited
...
protected instance inhabited_evals_to_in_time :
    Inhabited
        (evals_to_in_time (fun (_x : Unit) => some PUnit.unit) PUnit.unit (some PUnit.unit) 0) :=
  { default := evals_to_in_time.refl (fun (_x : Unit) => some PUnit.unit) PUnit.unit }

protected instance inhabited_tm2_evals_to :
    Inhabited (evals_to (fun (_x : Unit) => some PUnit.unit) PUnit.unit (some PUnit.unit)) :=
  { default := evals_to.refl (fun (_x : Unit) => some PUnit.unit) PUnit.unit }

/-- A proof that the identity map on α is computable in time. -/
def id_computable_in_time {α : Type} (ea : computability.fin_encoding α) :
    tm2_computable_in_time ea ea id :=
  tm2_computable_in_poly_time.to_tm2_computable_in_time (id_computable_in_poly_time ea)

protected instance inhabited_tm2_computable_in_time :
    Inhabited
        (tm2_computable_in_time computability.fin_encoding_bool_bool
          computability.fin_encoding_bool_bool id) :=
...
protected instance inhabited_tm2_computable :
    Inhabited
        (tm2_computable computability.fin_encoding_bool_bool computability.fin_encoding_bool_bool
          id) :=
