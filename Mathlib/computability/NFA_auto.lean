...
theorem mem_step_set {α : Type u} {σ : Type v} (M : NFA α σ) (s : σ) (S : set σ) (a : α) : s ∈ step_set M S a ↔ ∃ (t : σ), ∃ (H : t ∈ S), s ∈ step M t a := sorry
...
/-- `M.eval x` computes all possible paths though `M` with input `x` starting at an element of
  `M.start`. -/
def eval {α : Type u} {σ : Type v} (M : NFA α σ) : List α → set σ :=
  eval_from M (start M)
...
@[simp] theorem to_DFA_correct {α : Type u} {σ : Type v} (M : NFA α σ) : DFA.accepts (to_DFA M) = accepts M := sorry
...
@[simp] theorem to_NFA_eval_from_match {α : Type u} {σ : Type v} (M : DFA α σ) (start : σ) (s : List α) : NFA.eval_from (to_NFA M) (singleton start) s = singleton (eval_from M start s) := sorry

@[simp] theorem to_NFA_correct {α : Type u} {σ : Type v} (M : DFA α σ) : NFA.accepts (to_NFA M) = accepts M := sorry
