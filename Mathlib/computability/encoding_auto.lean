...
protected instance inhabited_Γ' : Inhabited Γ' := { default := Γ'.blank }
...
def section_Γ'_bool : Γ' → Bool := sorry
...
def encode_pos_num : pos_num → List Bool := sorry

/-- An encoding function of the binary numbers in bool. -/
def encode_num : num → List Bool := sorry
...
def decode_pos_num : List Bool → pos_num := sorry

/-- A decoding function from `list bool` to the binary numbers. -/
def decode_num : List Bool → num := fun (l : List Bool) => ite (l = []) num.zero ↑(decode_pos_num l)

/-- A decoding function from `list bool` to ℕ. -/
def decode_nat : List Bool → ℕ := fun (l : List Bool) => ↑(decode_num l)

theorem encode_pos_num_nonempty (n : pos_num) : encode_pos_num n ≠ [] :=
  pos_num.cases_on n (list.cons_ne_nil tt [])
    (fun (m : pos_num) => list.cons_ne_nil tt (encode_pos_num m))
...
def fin_encoding_nat_bool : fin_encoding ℕ := fin_encoding.mk encoding_nat_bool bool.fintype
...
def fin_encoding_nat_Γ' : fin_encoding ℕ := fin_encoding.mk encoding_nat_Γ' Γ'.fintype

/-- A unary encoding function of ℕ in bool. -/
def unary_encode_nat : ℕ → List Bool := sorry

/-- A unary decoding function from `list bool` to ℕ. -/
def unary_decode_nat : List Bool → ℕ := list.length

theorem unary_decode_encode_nat (n : ℕ) : unary_decode_nat (unary_encode_nat n) = n :=
  Nat.rec rfl
    (fun (m : ℕ) (hm : unary_decode_nat (unary_encode_nat m) = m) =>
      Eq.symm (congr_arg Nat.succ (Eq.symm hm)))
    n

/-- A unary fin_encoding of ℕ. -/
def unary_fin_encoding_nat : fin_encoding ℕ :=
  fin_encoding.mk
    (encoding.mk Bool unary_encode_nat (fun (n : List Bool) => some (unary_decode_nat n)) sorry)
    bool.fintype
...
def decode_bool : List Bool → Bool := sorry

theorem decode_encode_bool (b : Bool) : decode_bool (encode_bool b) = b := bool.cases_on b rfl rfl

/-- A fin_encoding of bool in bool. -/
def fin_encoding_bool_bool : fin_encoding Bool :=
  fin_encoding.mk (encoding.mk Bool encode_bool (fun (x : List Bool) => some (decode_bool x)) sorry)
    bool.fintype
