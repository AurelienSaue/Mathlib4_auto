...
coercions. -/
theorem radius_eq_liminf {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {E : Type u_2} [normed_group E] [normed_space 𝕜 E] {F : Type u_3} [normed_group F] [normed_space 𝕜 F] (p : formal_multilinear_series 𝕜 E F) : radius p = filter.liminf filter.at_top fun (n : ℕ) => 1 / ↑(nnnorm (p n) ^ (1 / ↑n)) := sorry
