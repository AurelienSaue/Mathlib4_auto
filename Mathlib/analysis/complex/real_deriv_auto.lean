...
theorem has_deriv_at.real_of_complex {e : ℂ → ℂ} {e' : ℂ} {z : ℝ} (h : has_deriv_at e e' ↑z) :
    has_deriv_at (fun (x : ℝ) => complex.re (e ↑x)) (complex.re e') z :=
  sorry

theorem times_cont_diff_at.real_of_complex {e : ℂ → ℂ} {z : ℝ} {n : with_top ℕ}
    (h : times_cont_diff_at ℂ n e ↑z) :
    times_cont_diff_at ℝ n (fun (x : ℝ) => complex.re (e ↑x)) z :=
  sorry

theorem times_cont_diff.real_of_complex {e : ℂ → ℂ} {n : with_top ℕ} (h : times_cont_diff ℂ n e) :
    times_cont_diff ℝ n fun (x : ℝ) => complex.re (e ↑x) :=
