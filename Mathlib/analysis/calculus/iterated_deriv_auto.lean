...
def iterated_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F]
    [normed_space 𝕜 F] (n : ℕ) (f : 𝕜 → F) (x : 𝕜) : F :=
...
from `𝕜` to `F`. -/
def iterated_deriv_within {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] (n : ℕ) (f : 𝕜 → F) (s : set 𝕜) (x : 𝕜) : F :=
  coe_fn (iterated_fderiv_within 𝕜 n f s x) fun (i : fin n) => 1

theorem iterated_deriv_within_univ {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} :
    iterated_deriv_within n f set.univ = iterated_deriv n f :=
  sorry

/-! ### Properties of the iterated derivative within a set -/

theorem iterated_deriv_within_eq_iterated_fderiv_within {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {s : set 𝕜} {x : 𝕜} :
...
Fréchet derivative -/
theorem iterated_deriv_within_eq_equiv_comp {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {s : set 𝕜} :
...
iterated derivative. -/
theorem iterated_fderiv_within_eq_equiv_comp {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {s : set 𝕜} :
    iterated_fderiv_within 𝕜 n f s =
...
theorem iterated_fderiv_within_apply_eq_iterated_deriv_within_mul_prod {𝕜 : Type u_1}
    [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {n : ℕ}
    {f : 𝕜 → F} {s : set 𝕜} {x : 𝕜} {m : fin n → 𝕜} :
    coe_fn (iterated_fderiv_within 𝕜 n f s x) m =
        (finset.prod finset.univ fun (i : fin n) => m i) • iterated_deriv_within n f s x :=
  sorry

@[simp] theorem iterated_deriv_within_zero {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {s : set 𝕜} :
    iterated_deriv_within 0 f s = f :=
  sorry

@[simp] theorem iterated_deriv_within_one {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {s : set 𝕜} (hs : unique_diff_on 𝕜 s) {x : 𝕜}
    (hx : x ∈ s) : iterated_deriv_within 1 f s x = deriv_within f s x :=
  sorry
...
theorem times_cont_diff_on_of_continuous_on_differentiable_on_deriv {𝕜 : Type u_1}
    [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F}
    {s : set 𝕜} {n : with_top ℕ}
    (Hcont : ∀ (m : ℕ), ↑m ≤ n → continuous_on (fun (x : 𝕜) => iterated_deriv_within m f s x) s)
    (Hdiff :
      ∀ (m : ℕ), ↑m < n → differentiable_on 𝕜 (fun (x : 𝕜) => iterated_deriv_within m f s x) s) :
...
-/
theorem times_cont_diff_on_of_differentiable_on_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {s : set 𝕜} {n : with_top ℕ}
    (h : ∀ (m : ℕ), ↑m ≤ n → differentiable_on 𝕜 (iterated_deriv_within m f s) s) :
    times_cont_diff_on 𝕜 n f s :=
  sorry

/-- On a set with unique derivatives, a `C^n` function has derivatives up to `n` which are
continuous. -/
theorem times_cont_diff_on.continuous_on_iterated_deriv_within {𝕜 : Type u_1}
    [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F}
    {s : set 𝕜} {n : with_top ℕ} {m : ℕ} (h : times_cont_diff_on 𝕜 n f s) (hmn : ↑m ≤ n)
    (hs : unique_diff_on 𝕜 s) : continuous_on (iterated_deriv_within m f s) s :=
  sorry

/-- On a set with unique derivatives, a `C^n` function has derivatives less than `n` which are
differentiable. -/
theorem times_cont_diff_on.differentiable_on_iterated_deriv_within {𝕜 : Type u_1}
    [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F}
    {s : set 𝕜} {n : with_top ℕ} {m : ℕ} (h : times_cont_diff_on 𝕜 n f s) (hmn : ↑m < n)
    (hs : unique_diff_on 𝕜 s) : differentiable_on 𝕜 (iterated_deriv_within m f s) s :=
  sorry
...
theorem times_cont_diff_on_iff_continuous_on_differentiable_on_deriv {𝕜 : Type u_1}
    [nondiscrete_normed_field 𝕜] {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F}
    {s : set 𝕜} {n : with_top ℕ} (hs : unique_diff_on 𝕜 s) :
    times_cont_diff_on 𝕜 n f s ↔
        (∀ (m : ℕ), ↑m ≤ n → continuous_on (iterated_deriv_within m f s) s) ∧
          ∀ (m : ℕ), ↑m < n → differentiable_on 𝕜 (iterated_deriv_within m f s) s :=
  sorry
...
theorem iterated_deriv_within_succ {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {s : set 𝕜} {x : 𝕜}
...
theorem iterated_deriv_within_eq_iterate {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {s : set 𝕜} {x : 𝕜}
    (hs : unique_diff_on 𝕜 s) (hx : x ∈ s) :
    iterated_deriv_within n f s x = nat.iterate (fun (g : 𝕜 → F) => deriv_within g s) n f x :=
  sorry
...
theorem iterated_deriv_eq_iterated_fderiv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {x : 𝕜} :
    iterated_deriv n f x = coe_fn (iterated_fderiv 𝕜 n f x) fun (i : fin n) => 1 :=
  rfl

/-- Write the iterated derivative as the composition of a continuous linear equiv and the iterated
Fréchet derivative -/
theorem iterated_deriv_eq_equiv_comp {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} :
    iterated_deriv n f =
        ⇑(continuous_linear_equiv.symm (continuous_multilinear_map.pi_field_equiv 𝕜 (fin n) F)) ∘
...
/-- Write the iterated Fréchet derivative as the composition of a continuous linear equiv and the
iterated derivative. -/
theorem iterated_fderiv_eq_equiv_comp {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} :
    iterated_fderiv 𝕜 n f =
...
/-- The `n`-th Fréchet derivative applied to a vector `(m 0, ..., m (n-1))` is the derivative
multiplied by the product of the `m i`s. -/
theorem iterated_fderiv_apply_eq_iterated_deriv_mul_prod {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} {x : 𝕜} {m : fin n → 𝕜} :
    coe_fn (iterated_fderiv 𝕜 n f x) m =
        (finset.prod finset.univ fun (i : fin n) => m i) • iterated_deriv n f x :=
  sorry

@[simp] theorem iterated_deriv_zero {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} : iterated_deriv 0 f = f :=
  sorry

@[simp] theorem iterated_deriv_one {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} : iterated_deriv 1 f = deriv f :=
  sorry

/-- The property of being `C^n`, initially defined in terms of the Fréchet derivative, can be
reformulated in terms of the one-dimensional derivative. -/
theorem times_cont_diff_iff_iterated_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {n : with_top ℕ} :
    times_cont_diff 𝕜 n f ↔
        (∀ (m : ℕ), ↑m ≤ n → continuous (iterated_deriv m f)) ∧
          ∀ (m : ℕ), ↑m < n → differentiable 𝕜 (iterated_deriv m f) :=
  sorry
...
advantage of avoiding the discussion of continuity in the proof (and for `n = ∞` this is optimal).
-/
theorem times_cont_diff_of_differentiable_iterated_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {n : with_top ℕ}
    (h : ∀ (m : ℕ), ↑m ≤ n → differentiable 𝕜 (iterated_deriv m f)) : times_cont_diff 𝕜 n f :=
...
theorem times_cont_diff.continuous_iterated_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {n : with_top ℕ} (m : ℕ)
    (h : times_cont_diff 𝕜 n f) (hmn : ↑m ≤ n) : continuous (iterated_deriv m f) :=
  and.left (iff.mp times_cont_diff_iff_iterated_deriv h) m hmn

theorem times_cont_diff.differentiable_iterated_deriv {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜]
    {F : Type u_2} [normed_group F] [normed_space 𝕜 F] {f : 𝕜 → F} {n : with_top ℕ} (m : ℕ)
    (h : times_cont_diff 𝕜 n f) (hmn : ↑m < n) : differentiable 𝕜 (iterated_deriv m f) :=
...
iterated derivative. -/
theorem iterated_deriv_succ {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} :
...
theorem iterated_deriv_eq_iterate {𝕜 : Type u_1} [nondiscrete_normed_field 𝕜] {F : Type u_2}
    [normed_group F] [normed_space 𝕜 F] {n : ℕ} {f : 𝕜 → F} :
