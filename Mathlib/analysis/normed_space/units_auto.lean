...
@[simp] theorem one_sub_coe {R : Type u_1} [normed_ring R] [complete_space R] (t : R)
    (h : norm t < 1) : ↑(one_sub t h) = 1 - t :=
  rfl

/-- In a complete normed ring, a perturbation of a unit `x` by an element `t` of distance less than
`∥x⁻¹∥⁻¹` from `x` is a unit.  Here we construct its `units` structure. -/
def add {R : Type u_1} [normed_ring R] [complete_space R] (x : units R) (t : R)
    (h : norm t < (norm ↑(x⁻¹)⁻¹)) : units R :=
  x * one_sub (-(↑(x⁻¹) * t)) sorry

@[simp] theorem add_coe {R : Type u_1} [normed_ring R] [complete_space R] (x : units R) (t : R)
    (h : norm t < (norm ↑(x⁻¹)⁻¹)) : ↑(add x t h) = ↑x + t :=
  sorry

/-- In a complete normed ring, an element `y` of distance less than `∥x⁻¹∥⁻¹` from `x` is a unit.
Here we construct its `units` structure. -/
def unit_of_nearby {R : Type u_1} [normed_ring R] [complete_space R] (x : units R) (y : R)
    (h : norm (y - ↑x) < (norm ↑(x⁻¹)⁻¹)) : units R :=
  add x (y - ↑x) h

@[simp] theorem unit_of_nearby_coe {R : Type u_1} [normed_ring R] [complete_space R] (x : units R)
    (y : R) (h : norm (y - ↑x) < (norm ↑(x⁻¹)⁻¹)) : ↑(unit_of_nearby x y h) = y :=
  sorry

/-- The group of units of a complete normed ring is an open subset of the ring. -/
theorem is_open {R : Type u_1} [normed_ring R] [complete_space R] :
    is_open (set_of fun (x : R) => is_unit x) :=
  sorry

theorem nhds {R : Type u_1} [normed_ring R] [complete_space R] (x : units R) :
    (set_of fun (x : R) => is_unit x) ∈ nhds ↑x :=
  mem_nhds_sets is_open
    (eq.mpr (id (Eq._oldrec (Eq.refl (↑x ∈ set_of fun (x : R) => is_unit x)) set.mem_set_of_eq))
      (is_unit_unit x))
...
theorem inverse_one_sub {R : Type u_1} [normed_ring R] [complete_space R] (t : R) (h : norm t < 1) :
    ring.inverse (1 - t) = ↑(units.one_sub t h⁻¹) :=
  sorry

/-- The formula `inverse (x + t) = inverse (1 + x⁻¹ * t) * x⁻¹` holds for `t` sufficiently small. -/
theorem inverse_add {R : Type u_1} [normed_ring R] [complete_space R] (x : units R) :
    filter.eventually
        (fun (t : R) => ring.inverse (↑x + t) = ring.inverse (1 + ↑(x⁻¹) * t) * ↑(x⁻¹)) (nhds 0) :=
  sorry

theorem inverse_one_sub_nth_order {R : Type u_1} [normed_ring R] [complete_space R] (n : ℕ) :
    filter.eventually
        (fun (t : R) =>
          ring.inverse (1 - t) =
            (finset.sum (finset.range n) fun (i : ℕ) => t ^ i) + t ^ n * ring