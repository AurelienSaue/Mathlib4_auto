...
structure submonoid (M : Type u_3) [monoid M] where
  carrier : set M
  one_mem' : 1 ∈ carrier
...
structure add_submonoid (M : Type u_3) [add_monoid M] where
  carrier : set M
  zero_mem' : 0 ∈ carrier
...
protected instance Mathlib.add_submonoid.set.has_coe {M : Type u_1} [add_monoid M] :
...
@[simp] theorem mem_carrier {M : Type u_1} [monoid M] {s : submonoid M} {x : M} :
    x ∈ carrier s ↔ x ∈ s :=
  iff.rfl
...
@[simp] theorem coe_coe {M : Type u_1} [monoid M] (s : submonoid M) : ↥↑s = ↥s := rfl

protected theorem Mathlib.add_submonoid.exists {M : Type u_1} [add_monoid M] {s : add_submonoid M}
    {p : ↥s → Prop} : (∃ (x : ↥s), p x) ↔ ∃ (x : M), ∃ (H : x ∈ s), p { val := x, property := H } :=
  set_coe.exists

protected theorem forall {M : Type u_1} [monoid M] {s : submonoid M} {p : ↥s → Prop} :
    (∀ (x : ↥s), p x) ↔ ∀ (x : M) (H : x ∈ s), p { val := x, property := H } :=
  set_coe.forall

/-- Two submonoids are equal if the underlying subsets are equal. -/
theorem ext' {M : Type u_1} [monoid M] {S : submonoid M} {T : submonoid M} (h : ↑S = ↑T) : S = T :=
  sorry

/-- Two submonoids are equal if and only if the underlying subsets are equal. -/
protected theorem ext'_iff {M : Type u_1} [monoid M] {S : submonoid M} {T : submonoid M} :
    S = T ↔ ↑S = ↑T :=
  { mp := fun (h : S = T) => h ▸ rfl, mpr := fun (h : ↑S = ↑T) => ext' h }

/-- Two submonoids are equal if they have the same elements. -/
theorem Mathlib.add_submonoid.ext {M : Type u_1} [add_monoid M] {S : add_submonoid M}
    {T : add_submonoid M} (h : ∀ (x : M), x ∈ S ↔ x ∈ T) : S = T :=
...
@[simp] theorem coe_copy {M : Type u_1} [monoid M] {S : submonoid M} {s : set M} (hs : s = ↑S) :
    ↑(copy S s hs) = s :=
  rfl

theorem copy_eq {M : Type u_1} [monoid M] {S : submonoid M} {s : set M} (hs : s = ↑S) :
    copy S s hs = S :=
  ext' hs

/-- A submonoid contains the monoid's 1. -/
theorem Mathlib.add_submonoid.zero_mem {M : Type u_1} [add_monoid M] (S : add_submonoid M) :
    0 ∈ S :=
  add_submonoid.zero_mem' S

/-- A submonoid is closed under multiplication. -/
theorem mul_mem {M : Type u_1} [monoid M] (S : submonoid M) {x : M} {y : M} :
    x ∈ S → y ∈ S → x * y ∈ S :=
  mul_mem' S
...
@[simp] theorem coe_eq_coe {M : Type u_1} [monoid M] (S : submonoid M) (x : ↥S) (y : ↥S) :
    ↑x = ↑y ↔ x = y :=
  set_coe.ext_iff
...
theorem le_def {M : Type u_1} [monoid M] {S : submonoid M} {T : submonoid M} :
    S ≤ T ↔ ∀ {x : M}, x ∈ S → x ∈ T :=
  iff.rfl

@[simp] theorem Mathlib.add_submonoid.coe_subset_coe {M : Type u_1} [add_monoid M]
    {S : add_submonoid M} {T : add_submonoid M} : ↑S ⊆ ↑T ↔ S ≤ T :=
  iff.rfl

protected instance Mathlib.add_submonoid.partial_order {M : Type u_1} [add_monoid M] :
    partial_order (add_submonoid M) :=
  partial_order.mk (fun (S T : add_submonoid M) => ∀ {x : M}, x ∈ S → x ∈ T) partial_order.lt sorry
    sorry sorry

@[simp] theorem coe_ssubset_coe {M : Type u_1} [monoid M] {S : submonoid M} {T : submonoid M} :
    ↑S ⊂ ↑T ↔ S < T :=
  iff.rfl
...
protected instance Mathlib.add_submonoid.has_bot {M : Type u_1} [add_monoid M] :
    has_bot (add_submonoid M) :=
  has_bot.mk (add_submonoid.mk (singleton 0) sorry sorry)

protected instance Mathlib.add_submonoid.inhabited {M : Type u_1} [add_monoid M] :
    Inhabited (add_submonoid M) :=
  { default := ⊥ }

@[simp] theorem mem_bot {M : Type u_1} [monoid M] {x : M} : x ∈ ⊥ ↔ x = 1 := set.mem_singleton_iff

@[simp] theorem mem_top {M : Type u_1} [monoid M] (x : M) : x ∈ ⊤ := set.mem_univ x

@[simp] theorem Mathlib.add_submonoid.coe_top {M : Type u_1} [add_monoid M] : ↑⊤ = set.univ := rfl

@[simp] theorem coe_bot {M : Type u_1} [monoid M] : ↑⊥ = singleton 1 := rfl
...
@[simp] theorem coe_inf {M : Type u_1} [monoid M] (p : submonoid M) (p' : submonoid M) :
    ↑(p ⊓ p') = ↑p ∩ ↑p' :=
  rfl

@[simp] theorem mem_inf {M : Type u_1} [monoid M] {p : submonoid M} {p' : submonoid M} {x : M} :
    x ∈ p ⊓ p' ↔ x ∈ p ∧ x ∈ p' :=
  iff.rfl

protected instance has_Inf {M : Type u_1} [monoid M] : has_Inf (submonoid M) :=
  has_Inf.mk
    fun (s : set (submonoid M)) =>
      mk (set.Inter fun (t : submonoid M) => set.Inter fun (H : t ∈ s) => ↑t) sorry sorry

@[simp] theorem coe_Inf {M : Type u_1} [monoid M] (S : set (submonoid M)) :
    ↑(Inf S) = set.Inter fun (s : submonoid M) => set.Inter fun (H : s ∈ S) => ↑s :=
  rfl

theorem Mathlib.add_submonoid.mem_Inf {M : Type u_1} [add_monoid M] {S : set (add_submonoid M)}
    {x : M} : x ∈ Inf S ↔ ∀ (p : add_submonoid M), p ∈ S → x ∈ p :=
  set.mem_bInter_iff

theorem mem_infi {M : Type u_1} [monoid M] {ι : Sort u_2} {S : ι → submonoid M} {x : M} :
    (x ∈ infi fun (i : ι) => S i) ↔ ∀ (i : ι), x ∈ S i :=
  sorry

@[simp] theorem Mathlib.add_submonoid.coe_infi {M : Type u_1} [add_monoid M] {ι : Sort u_2}
    {S : ι → add_submonoid M} : ↑(infi fun (i : ι) => S i) = set.Inter fun (i : ι) => ↑(S i) :=
  sorry

/-- Submonoids of a monoid form a complete lattice. -/
protected instance complete_lattice {M : Type u_1} [monoid M] : complete_lattice (submonoid M) :=
  complete_lattice.mk complete_lattice.sup LessEq Less sorry sorry sorry sorry sorry sorry
    has_inf.inf sorry sorry sorry ⊤ sorry ⊥ sorry complete_lattice.Sup Inf sorry sorry sorry sorry

theorem subsingleton_iff {M : Type u_1} [monoid M] : subsingleton M ↔ subsingleton (submonoid M) :=
  sorry
...
protected instance subsingleton {M : Type u_1} [monoid M] [subsingleton M] :
    subsingleton (submonoid M) :=
...
theorem mem_closure {M : Type u_1} [monoid M] {s : set M} {x : M} :
    x ∈ closure s ↔ ∀ (S : submonoid M), s ⊆ ↑S → x ∈ S :=
  mem_Inf
...
@[simp] theorem closure_le {M : Type u_1} [monoid M] {s : set M} {S : submonoid M} :
    closure s ≤ S ↔ s ⊆ ↑S :=
...
theorem Mathlib.add_submonoid.closure_mono {M : Type u_1} [add_monoid M] {s : set M} {t : set M}
    (h : s ⊆ t) : add_submonoid.closure s ≤ add_submonoid.closure t :=
  iff.mpr add_submonoid.closure_le (set.subset.trans h add_submonoid.subset_closure)

theorem Mathlib.add_submonoid.closure_eq_of_le {M : Type u_1} [add_monoid M] {s : set M}
    {S : add_submonoid M} (h₁ : s ⊆ ↑S) (h₂ : S ≤ add_submonoid.closure s) :
...
theorem closure_induction {M : Type u_1} [monoid M] {s : set M} {p : M → Prop} {x : M}
    (h : x ∈ closure s) (Hs : ∀ (x : M), x ∈ s → p x) (H1 : p 1)
    (Hmul : ∀ (x y : M), p x → p y → p (x * y)) : p x :=
...
theorem dense_induction {M : Type u_1} [monoid M] {p : M → Prop} (x : M) {s : set M}
    (hs : closure s = ⊤) (Hs : ∀ (x : M), x ∈ s → p x) (H1 : p 1)
    (Hmul : ∀ (x y : M), p x → p y → p (x * y)) : p x :=
  sorry
...
theorem Mathlib.add_submonoid.closure_union {M : Type u_1} [add_monoid M] (s : set M) (t : set M) :
...
theorem closure_Union {M : Type u_1} [monoid M] {ι : Sort u_2} (s : ι → set M) :
...
theorem is_unit.mem_submonoid_iff {M : Type u_1} [monoid M] (a : M) :
  ...
  id
    (eq.mpr (id (Eq._oldrec (Eq.refl (a ∈ set_of is_unit ↔ is_unit a)) set.mem_set_of_eq))
      (iff.refl (is_unit a)))
...
def eq_mlocus {M : Type u_1} [monoid M] {N : Type u_3} [monoid N] (f : M →* N) (g : M →* N) :
    submonoid M :=
  submonoid.mk (set_of fun (x : M) => coe_fn f x = coe_fn g x) sorry sorry

/-- If two monoid homomorphisms are equal on a set, then they are equal on its submonoid closure. -/
theorem Mathlib.add_monoid_hom.eq_on_mclosure {M : Type u_1} [add_monoid M] {N : Type u_3}
    [add_monoid N] {f : M →+ N} {g : M →+ N} {s : set M} (h : set.eq_on (⇑f) (⇑g) s) :
    set.eq_on ⇑f ⇑g ↑(add_submonoid.closure s) :=
  (fun (this : add_submonoid.closure s ≤ add_monoid_hom.eq_mlocus f g) => this)
    (iff.mpr add_submonoid.closure_le h)

theorem Mathlib.add_monoid_hom.eq_of_eq_on_mtop {M : Type u_1} [add_monoid M] {N : Type u_3}
    [add_monoid N] {f : M →+ N} {g : M →+ N} (h : set.eq_on ⇑f ⇑g ↑⊤) : f = g :=
  add_monoid_hom.ext fun (x : M) => h trivial

theorem eq_of_eq_on_mdense {M : Type u_1} [monoid M] {N : Type u_3} [monoid N] {s : set M}
    (hs : submonoid.closure s = ⊤) {f : M →* N} {g : M →* N} (h : set.eq_on (⇑f) (⇑g) s) : f = g :=
...
def of_mdense {M : Type u_1} [monoid M] {s : set M} {N : Type u_3} [monoid N] (f : M → N)
    (hs : submonoid.closure s = ⊤) (h1 : f 1 = 1)
    (hmul : ∀ (x y : M), y ∈ s → f (x * y) = f x * f y) : M →* N :=
  mk f h1 sorry
...
of `f (x + y) = f x + f y` only for `y ∈ s`. -/
@[simp] theorem coe_of_mdense {M : Type u_1} [monoid M] {s : set M} {N : Type u_3} [monoid N]
    (f : M → N) (hs : submonoid.closure s = ⊤) (h1 : f 1 = 1)
    (hmul : ∀ (x y : M), y ∈ s → f (x * y) = f x * f y) : ⇑(of_mdense f hs h1 hmul) = f :=
  rfl
