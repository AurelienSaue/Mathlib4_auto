...
-/
structure matching {V : Type u} (G : simple_graph V) where
  edges : set (sym2 V)
...
theorem matching.is_perfect_iff {V : Type u} {G : simple_graph V} (M : matching G) :
    matching.is_perfect M ↔ ∀ (v : V), ∃ (e : sym2 V), ∃ (H : e ∈ matching.edges M), v ∈ e :=
