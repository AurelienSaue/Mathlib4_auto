...
/-- An ideal on a preorder `P` is a subset of `P` that is
  ...
  - upward directed
  - downward closed. -/
structure ideal (P : Type u_2) [preorder P] where
  carrier : set P
...
theorem mem_of_mem_of_le {P : Type u_1} [preorder P] {x : P} {I : ideal P} {J : ideal P} :
    x ∈ I → I ≤ J → x ∈ J :=
  set.mem_of_mem_of_subset

@[simp] theorem principal_le_iff {P : Type u_1} [preorder P] {x : P} {I : ideal P} :
    principal x ≤ I ↔ x ∈ I :=
...
theorem sup_mem {P : Type u_1} [semilattice_sup P] {I : ideal P} (x : P) (y : P) (H : x ∈ I) :
    y ∈ I → x ⊔ y ∈ I :=
  sorry

@[simp] theorem sup_mem_iff {P : Type u_1} [semilattice_sup P] {x : P} {y : P} {I : ideal P} :
    x ⊔ y ∈ I ↔ x ∈ I ∧ y ∈ I :=
  { mp :=
      fun (h : x ⊔ y ∈ I) =>
        { left := mem_of_le I le_sup_left h, right := mem_of_le I le_sup_right h },
...
structure cofinal (P : Type u_2) [preorder P] where
  carrier : set P
...
def above {P : Type u_1} [preorder P] (D : cofinal P) (x : P) : P := classical.some (mem_gt D x)
...
def sequence_of_cofinals {P : Type u_1} [preorder P] (p : P) {ι : Type u_2} [encodable ι]
    (𝒟 : ι → cofinal P) : ℕ → P :=
  sorry

theorem sequence_of_cofinals.monotone {P : Type u_1} [preorder P] (p : P) {ι : Type u_2}
    [encodable ι] (𝒟 : ι → cofinal P) : monotone (sequence_of_cofinals p 𝒟) :=
  sorry

theorem sequence_of_cofinals.encode_mem {P : Type u_1} [preorder P] (p : P) {ι : Type u_2}
    [encodable ι] (𝒟 : ι → cofinal P) (i : ι) :
...
def ideal_of_cofinals {P : Type u_1} [preorder P] (p : P) {ι : Type u_2} [encodable ι]
    (𝒟 : ι → cofinal P) : ideal P :=
  ideal.mk (set_of fun (x : P) => ∃ (n : ℕ), x ≤ sequence_of_cofinals p 𝒟 n) sorry sorry sorry

theorem mem_ideal_of_cofinals {P : Type u_1} [preorder P] (p : P) {ι : Type u_2} [encodable ι]
    (𝒟 : ι → cofinal P) : p ∈ ideal_of_cofinals p 𝒟 :=
  Exists.intro 0 (le_refl p)

/-- `ideal_of_cofinals p 𝒟` is `𝒟`-generic. -/
theorem cofinal_meets_ideal_of_cofinals {P : Type u_1} [preorder P] (p : P) {ι : Type u_2}
    [encodable ι] (𝒟 : ι → cofinal P) (i : ι) : ∃ (x : P), x ∈ 𝒟 i ∧ x ∈ ideal_of_cofinals p 𝒟 :=
  Exists.intro (sequence_of_cofinals p 𝒟 (encodable.encode i + 1))
    { left := sequence_of_cofinals.encode_mem p 𝒟 i,
      right :=
        Exists.intro (encodable.encode i + 1)
          (le_refl (sequence_of_cofinals p 𝒟 (encodable.encode i + 1))) }
