...
theorem is_compact_element_iff_le_of_directed_Sup_le (α : Type u_1) [complete_lattice α] (k : α) :
    is_compact_element k ↔
        ∀ (s : set α),
          set.nonempty s → directed_on LessEq s → k ≤ Sup s → ∃ (x : α), x ∈ s ∧ k ≤ x :=
  sorry

theorem finset_sup_compact_of_compact {α : Type u_1} {β : Type u_2} [complete_lattice α] {f : β → α}
    (s : finset β) (h : ∀ (x : β), x ∈ s → is_compact_element (f x)) :
    is_compact_element (finset.sup s f) :=
  sorry

theorem well_founded.is_Sup_finite_compact (α : Type u_1) [complete_lattice α]
    (h : well_founded gt) : is_Sup_finite_compact α :=
  sorry

theorem is_Sup_finite_compact.is_sup_closed_compact (α : Type u_1) [complete_lattice α]
    (h : is_Sup_finite_compact α) : is_sup_closed_compact α :=
  sorry

theorem is_sup_closed_compact.well_founded (α : Type u_1) [complete_lattice α]
    (h : is_sup_closed_compact α) : well_founded gt :=
  sorry

theorem is_Sup_finite_compact_iff_all_elements_compact (α : Type u_1) [complete_lattice α] :
    is_Sup_finite_compact α ↔ ∀ (k : α), is_compact_element k :=
  sorry

theorem well_founded_characterisations (α : Type u_1) [complete_lattice α] :
    tfae
        [well_founded gt, is_Sup_finite_compact α, is_sup_closed_compact α,
          ∀ (k : α), is_compact_element k] :=
  sorry

theorem well_founded_iff_is_Sup_finite_compact (α : Type u_1) [complete_lattice α] :
    well_founded gt ↔ is_Sup_finite_compact α :=
  list.tfae.out (well_founded_characterisations α) 0 1

theorem is_Sup_finite_compact_iff_is_sup_closed_compact (α : Type u_1) [complete_lattice α] :
    is_Sup_finite_compact α ↔ is_sup_closed_compact α :=
  list.tfae.out (well_founded_characterisations α) 1 (bit0 1)

theorem is_sup_closed_compact_iff_well_founded (α : Type u_1) [complete_lattice α] :
    is_sup_closed_compact α ↔ well_founded gt :=
  list.tfae.out (well_founded_characterisations α) (bit0 1) 0

theorem Mathlib.is_Sup_finite_compact.well_founded (α : Type u_1) [complete_lattice α] :
    is_Sup_finite_compact α → well_founded gt :=
  iff.mpr (well_founded_iff_is_Sup_finite_compact α)

theorem Mathlib.is_sup_closed_compact.is_Sup_finite_compact (α : Type u_1) [complete_lattice α] :
    is_sup_closed_compact α → is_Sup_finite_compact α :=
  iff.mpr (is_Sup_finite_compact_iff_is_sup_closed_compact α)

theorem Mathlib.well_founded.is_sup_closed_compact (α : Type u_1) [complete_lattice α] :
    well_founded gt → is_sup_closed_compact α :=
  iff.mpr (is_sup_closed_compact_iff_well_founded α)

theorem compactly_generated_of_well_founded (α : Type u_1) [complete_lattice α]
    (h : well_founded gt) : is_compactly_generated α :=
  sorry
