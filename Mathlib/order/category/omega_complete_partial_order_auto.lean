...
def ωCPO := category_theory.bundled omega_complete_partial_order

namespace ωCPO


protected instance omega_complete_partial_order.continuous_hom.category_theory.bundled_hom :
...
def of (α : Type u_1) [omega_complete_partial_order α] : ωCPO := category_theory.bundled.of α

protected instance inhabited : Inhabited ωCPO := { default := of PUnit }
...
protected instance category_theory.limits.has_product (J : Type v) (f : J → ωCPO) :
    category_theory.limits.has_product f :=
  category_theory.limits.has_limit.mk
    (category_theory.limits.limit_cone.mk (product f) (is_product J f))

end has_products


protected instance omega_complete_partial_order_equalizer {α : Type u_1} {β : Type u_2}
    [omega_complete_partial_order α] [omega_complete_partial_order β] (f : α →𝒄 β) (g : α →𝒄 β) :
    omega_complete_partial_order (Subtype fun (a : α) => coe_fn f a = coe_fn g a) :=
...
def equalizer_ι {α : Type u_1} {β : Type u_2} [omega_complete_partial_order α]
    [omega_complete_partial_order β] (f : α →𝒄 β) (g : α →𝒄 β) :
    (Subtype fun (a : α) => coe_fn f a = coe_fn g a) →𝒄 α :=
  omega_complete_partial_order.continuous_hom.of_mono
    (preorder_hom.subtype.val fun (a : α) => coe_fn f a = coe_fn g a) sorry
...
def is_equalizer {X : ωCPO} {Y : ωCPO} (f : X ⟶ Y) (g : X ⟶ Y) :
    category_theory.limits.is_limit (equalizer f g) :=
    ...
    category_theory.limits.has_limits_of_shape.mk
      fun (F : category_theory.discrete J ⥤ ωCPO) =>
        category_theory.limits.has_limit_of_iso
          (category_theory.iso.symm category_theory.discrete.nat_iso_functor)

protected instance category_theory.limits.parallel_pair.category_theory.limits.has_limit {X : ωCPO}
    {Y : ωCPO} (f : X ⟶ Y) (g : X ⟶ Y) :
    category_theory.limits.has_limit (category_theory.limits.parallel_pair f g) :=
  category_theory.limits.has_limit.mk
    (category_theory.limits.limit_cone.mk (has_equalizers.equalizer f g)
      (has_equalizers.is_equalizer f g))

protected instance category_theory.limits.has_equalizers :
