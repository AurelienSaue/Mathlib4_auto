...
def padic_seq (p : ℕ) := cau_seq ℚ (padic_norm p)
...
constant. -/
theorem stationary {p : ℕ} [fact (nat.prime p)] {f : cau_seq ℚ (padic_norm p)} (hf : ¬f ≈ 0) :
    ∃ (N : ℕ), ∀ (m n : ℕ), N ≤ m → N ≤ n → padic_norm p (coe_fn f n) = padic_norm p (coe_fn f m) :=
  sorry
...
theorem stationary_point_spec {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) {m : ℕ}
    {n : ℕ} :
    stationary_point hf ≤ m →
        stationary_point hf ≤ n → padic_norm p (coe_fn f n) = padic_norm p (coe_fn f m) :=
...
def norm {p : ℕ} [fact (nat.prime p)] (f : padic_seq p) : ℚ :=
  dite (f ≈ 0) (fun (hf : f ≈ 0) => 0)
    fun (hf : ¬f ≈ 0) => padic_norm p (coe_fn f (stationary_point hf))

theorem norm_zero_iff {p : ℕ} [fact (nat.prime p)] (f : padic_seq p) : norm f = 0 ↔ f ≈ 0 := sorry

theorem equiv_zero_of_val_eq_of_equiv_zero {p : ℕ} [fact (nat.prime p)] {f : padic_seq p}
    {g : padic_seq p} (h : ∀ (k : ℕ), padic_norm p (coe_fn f k) = padic_norm p (coe_fn g k))
    (hf : f ≈ 0) : g ≈ 0 :=
  sorry

theorem norm_nonzero_of_not_equiv_zero {p : ℕ} [fact (nat.prime p)] {f : padic_seq p}
    (hf : ¬f ≈ 0) : norm f ≠ 0 :=
  hf ∘ iff.mp (norm_zero_iff f)

theorem norm_eq_norm_app_of_nonzero {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) :
    ∃ (k : ℚ), norm f = padic_norm p k ∧ k ≠ 0 :=
  sorry

theorem not_lim_zero_const_of_nonzero {p : ℕ} [fact (nat.prime p)] {q : ℚ} (hq : q ≠ 0) :
    ¬cau_seq.lim_zero (cau_seq.const (padic_norm p) q) :=
  fun (h' : cau_seq.lim_zero (cau_seq.const (padic_norm p) q)) =>
    hq (iff.mp cau_seq.const_lim_zero h')

theorem not_equiv_zero_const_of_nonzero {p : ℕ} [fact (nat.prime p)] {q : ℚ} (hq : q ≠ 0) :
...
theorem lift_index_left_left {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) (v2 : ℕ)
    (v3 : ℕ) :
    padic_norm p (coe_fn f (stationary_point hf)) =
        padic_norm p (coe_fn f (max (stationary_point hf) (max v2 v3))) :=
  stationary_point_spec hf (le_max_left (stationary_point hf) (max v2 v3))
    (le_refl (stationary_point hf))

/-- An auxiliary lemma for manipulating sequence indices. -/
theorem lift_index_left {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) (v1 : ℕ)
    (v3 : ℕ) :
    padic_norm p (coe_fn f (stationary_point hf)) =
        padic_norm p (coe_fn f (max v1 (max (stationary_point hf) v3))) :=
  stationary_point_spec hf
    (le_trans (le_max_left (stationary_point hf) v3)
      (le_max_right v1 (max (stationary_point hf) v3)))
    (le_refl (stationary_point hf))

/-- An auxiliary lemma for manipulating sequence indices. -/
theorem lift_index_right {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) (v1 : ℕ)
    (v2 : ℕ) :
    padic_norm p (coe_fn f (stationary_point hf)) =
        padic_norm p (coe_fn f (max v1 (max v2 (stationary_point hf)))) :=
  stationary_point_spec hf
    (le_trans (le_max_right v2 (stationary_point hf))
      (le_max_right v1 (max v2 (stationary_point hf))))
    (le_refl (stationary_point hf))
...
def valuation {p : ℕ} [fact (nat.prime p)] (f : padic_seq p) : ℤ :=
  dite (f ≈ 0) (fun (hf : f ≈ 0) => 0)
    fun (hf : ¬f ≈ 0) => padic_val_rat p (coe_fn f (stationary_point hf))

theorem norm_eq_pow_val {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} (hf : ¬f ≈ 0) :
    norm f = ↑p ^ (-valuation f) :=
  sorry

theorem val_eq_iff_norm_eq {p : ℕ} [fact (nat.prime p)] {f : padic_seq p} {g : padic_seq p}
    (hf : ¬f ≈ 0) (hg : ¬g ≈ 0) : valuation f = valuation g ↔ norm f = norm g :=
  sorry
...
theorem norm_mul {p : ℕ} [hp : fact (nat.prime p)] (f : padic_seq p) (g : padic_seq p) :
    norm (f * g) = norm f * norm g :=
  sorry

theorem eq_zero_iff_equiv_zero {p : ℕ} [hp : fact (nat.prime p)] (f : padic_seq p) :
    cau_seq.completion.mk f = 0 ↔ f ≈ 0 :=
  cau_seq.completion.mk_eq

theorem ne_zero_iff_nequiv_zero {p : ℕ} [hp : fact (nat.prime p)] (f : padic_seq p) :
    cau_seq.completion.mk f ≠ 0 ↔ ¬f ≈ 0 :=
  iff.mpr not_iff_not (eq_zero_iff_equiv_zero f)

theorem norm_const {p : ℕ} [hp : fact (nat.prime p)] (q : ℚ) :
    norm (cau_seq.const (padic_norm p) q) = padic_norm p q :=
  sorry

theorem norm_values_discrete {p : ℕ} [hp : fact (nat.prime p)] (a : padic_seq p) (ha : ¬a ≈ 0) :
    ∃ (z : ℤ), norm a = ↑p ^ (-z) :=
  sorry

theorem norm_one {p : ℕ} [hp : fact (nat.prime p)] : norm 1 = 1 := sorry

theorem norm_equiv {p : ℕ} [hp : fact (nat.prime p)] {f : padic_seq p} {g : padic_seq p}
    (hfg : f ≈ g) : norm f = norm g :=
  sorry

theorem norm_nonarchimedean {p : ℕ} [hp : fact (nat.prime p)] (f : padic_seq p) (g : padic_seq p) :
    norm (f + g) ≤ max (norm f) (norm g) :=
  sorry

theorem norm_eq {p : ℕ} [hp : fact (nat.prime p)] {f : padic_seq p} {g : padic_seq p}
    (h : ∀ (k : ℕ), padic_norm p (coe_fn f k) = padic_norm p (coe_fn g k)) : norm f = norm g :=
  sorry

theorem norm_neg {p : ℕ} [hp : fact (nat.prime p)] (a : padic_seq p) : norm (-a) = norm a := sorry

theorem norm_eq_of_add_equiv_zero {p : ℕ} [hp : fact (nat.prime p)] {f : padic_seq p}
    {g : padic_seq p} (h : f + g ≈ 0) : norm f = norm g :=
  sorry

theorem add_eq_max_of_ne {p : ℕ} [hp : fact (nat.prime p)] {f : padic_seq p} {g : padic_seq p}
    (hfgne : norm f ≠ norm g) : norm (f + g) = max (norm f) (norm g) :=
  sorry
...
def padic (p : ℕ) [fact (nat.prime p)] := cau_seq.completion.Cauchy

namespace padic


/-- The discrete field structure on `ℚ_p` is inherited from the Cauchy completion construction. -/
protected instance field {p : ℕ} [fact (nat.prime p)] : field (padic p) := cau_seq.completion.field

protected instance inhabited {p : ℕ} [fact (nat.prime p)] : Inhabited (padic p) := { default := 0 }
...
def mk {p : ℕ} [fact (nat.prime p)] : padic_seq p → padic p := quotient.mk

theorem mk_eq (p : ℕ) [fact (nat.prime p)] {f : padic_seq p} {g : padic_seq p} :
    mk f = mk g ↔ f ≈ g :=
  quotient.eq

/-- Embeds the rational numbers in the p-adic numbers. -/
def of_rat (p : ℕ) [fact (nat.prime p)] : ℚ → padic p := cau_seq.completion.of_rat

@[simp] theorem of_rat_add (p : ℕ) [fact (nat.prime p)] (x : ℚ) (y : ℚ) :
    of_rat p (x + y) = of_rat p x + of_rat p y :=
...
@[simp] theorem of_rat_mul (p : ℕ) [fact (nat.prime p)] (x : ℚ) (y : ℚ) :
    of_rat p (x * y) = of_rat p x * of_rat p y :=
  cau_seq.completion.of_rat_mul

@[simp] theorem of_rat_sub (p : ℕ) [fact (nat.prime p)] (x : ℚ) (y : ℚ) :
    of_rat p (x - y) = of_rat p x - of_rat p y :=
  cau_seq.completion.of_rat_sub

@[simp] theorem of_rat_div (p : ℕ) [fact (nat.prime p)] (x : ℚ) (y : ℚ) :
    of_rat p (x / y) = of_rat p x / of_rat p y :=
  cau_seq.completion.of_rat_div

@[simp] theorem of_rat_one (p : ℕ) [fact (nat.prime p)] : of_rat p 1 = 1 := rfl

@[simp] theorem of_rat_zero (p : ℕ) [fact (nat.prime p)] : of_rat p 0 = 0 := rfl
...
theorem coe_one (p : ℕ) [fact (nat.prime p)] : ↑1 = 1 := rfl

theorem coe_zero (p : ℕ) [fact (nat.prime p)] : ↑0 = 0 := rfl

theorem const_equiv (p : ℕ) [fact (nat.prime p)] {q : ℚ} {r : ℚ} :
...
theorem of_rat_eq (p : ℕ) [fact (nat.prime p)] {q : ℚ} {r : ℚ} : of_rat p q = of_rat p r ↔ q = r :=
  { mp := iff.mp (const_equiv p) ∘ iff.mp quotient.eq,
    mpr :=
      fun (h : q = r) =>
        eq.mpr (id (Eq._oldrec (Eq.refl (of_rat p q = of_rat p r)) h)) (Eq.refl (of_rat p r)) }
      ...
      eq.mpr (id (Eq._oldrec (Eq.refl (↑m = ↑n → m = n)) (Eq.symm (rat.cast_coe_nat m))))
        (eq.mpr
          (id
            (imp_congr_eq
              (Eq.trans
                (Eq.trans (congr_arg (Eq ↑↑m) (Eq.symm (rat.cast_coe_nat n))) (propext (coe_inj p)))
...
theorem defn {p : ℕ} [fact (nat.prime p)] (f : padic_seq p) {ε : ℚ} (hε : 0 < ε) :
    ∃ (N : ℕ), ∀ (i : ℕ), i ≥ N → padic_norm_e (quotient.mk f - ↑(coe_fn f i)) < ε :=
  sorry

protected theorem nonneg {p : ℕ} [fact (nat.prime p)] (q : padic p) : 0 ≤ padic_norm_e q :=
  quotient.induction_on q padic_seq.norm_nonneg

theorem zero_def {p : ℕ} [fact (nat.prime p)] : 0 = quotient.mk 0 := rfl
...
@[simp] protected theorem neg {p : ℕ} [fact (nat.prime p)] (q : padic p) :
    padic_norm_e (-q) = padic_norm_e q :=
  quotient.induction_on q padic_seq.norm_neg

/-- Theorems about `padic_norm_e` are named with a `'` so the names do not conflict with the
equivalent theorems about `norm` (`∥ ∥`). -/
theorem nonarchimedean' {p : ℕ} [fact (nat.prime p)] (q : padic p) (r : padic p) :
    padic_norm_e (q + r) ≤ max (padic_norm_e q) (padic_norm_e r) :=
  quotient.induction_on₂ q r padic_seq.norm_nonarchimedean

/-- Theorems about `padic_norm_e` are named with a `'` so the names do not conflict with the
equivalent theorems about `norm` (`∥ ∥`). -/
theorem add_eq_max_of_ne' {p : ℕ} [fact (nat.prime p)] {q : padic p} {r : padic p} :
    padic_norm_e q ≠ padic_norm_e r →
        padic_norm_e (q + r) = max (padic_norm_e q) (padic_norm_e r) :=
  quotient.induction_on₂ q r fun (_x _x_1 : cau_seq ℚ (padic_norm p)) => padic_seq.add_eq_max_of_ne

theorem triangle_ineq {p : ℕ} [fact (nat.prime p)] (x : padic p) (y : padic p) (z : padic p) :
    padic_norm_e (x - z) ≤ padic_norm_e (x - y) + padic_norm_e (y - z) :=
  sorry

protected theorem add {p : ℕ} [fact (nat.prime p)] (q : padic p) (r : padic p) :
    padic_norm_e (q + r) ≤ padic_norm_e q + padic_norm_e r :=
  le_trans (nonarchimedean' q r)
    (max_le_add_of_nonneg (padic_norm_e.nonneg q) (padic_norm_e.nonneg r))

protected theorem mul' {p : ℕ} [fact (nat.prime p)] (q : padic p) (r : padic p) :
    padic_norm_e (q * r) = padic_norm_e q * padic_norm_e r :=
  quotient.induction_on₂ q r padic_seq.norm_mul

protected instance is_absolute_value {p : ℕ} [fact (nat.prime p)] :
    is_absolute_value padic_norm_e :=
  is_absolute_value.mk padic_norm_e.nonneg zero_iff padic_norm_e.add padic_norm_e.mul'

@[simp] theorem eq_padic_norm' {p : ℕ} [fact (nat.prime p)] (q : ℚ) :
    padic_norm_e (padic.of_rat p q) = padic_norm p q :=
  padic_seq.norm_const q

protected theorem image' {p : ℕ} [fact (nat.prime p)] {q : padic p} :
    q ≠ 0 → ∃ (n : ℤ), padic_norm_e q = ↑p ^ (-n) :=
  quotient.induction_on q
    fun (f : cau_seq ℚ (padic_norm p)) (hf : quotient.mk f ≠ 0) =>
      (fun (this : ¬f ≈ 0) => padic_seq.norm_values_discrete f this)
        (iff.mp (padic_seq.ne_zero_iff_nequiv_zero f) hf)

theorem sub_rev {p : ℕ} [fact (nat.prime p)] (q : padic p) (r : padic p) :
    padic_norm_e (q - r) = padic_norm_e (r - q) :=
  sorry
...
theorem rat_dense' {p : ℕ} [fact (nat.prime p)] (q : padic p) {ε : ℚ} (hε : 0 < ε) :
...
theorem exi_rat_seq_conv {p : ℕ} [fact (nat.prime p)] (f : cau_seq (padic p) padic_norm_e) {ε : ℚ}
    (hε : 0 < ε) : ∃ (N : ℕ), ∀ (i : ℕ), i ≥ N → padic_norm_e (coe_fn f i - ↑(lim_seq f i)) < ε :=
  sorry

theorem exi_rat_seq_conv_cauchy {p : ℕ} [fact (nat.prime p)] (f : cau_seq (padic p) padic_norm_e) :
    is_cau_seq (padic_norm p) (lim_seq f) :=
  sorry

theorem complete' {p : ℕ} [fact (nat.prime p)] (f : cau_seq (padic p) padic_norm_e) :
    ∃ (q : padic p),
        ∀ (ε : ℚ), ε > 0 → ∃ (N : ℕ), ∀ (i : ℕ), i ≥ N → padic_norm_e (q - coe_fn f i) < ε :=
  sorry
...
protected instance is_absolute_value (p : ℕ) [fact (nat.prime p)] :
    is_absolute_value fun (a : padic p) => norm a :=
  sorry

theorem rat_dense {p : ℕ} {hp : fact (nat.prime p)} (q : padic p) {ε : ℝ} (hε : 0 < ε) :
    ∃ (r : ℚ), norm (q - ↑r) < ε :=
  sorry
...
@[simp] protected theorem mul {p : ℕ} [hp : fact (nat.prime p)] (q : padic p) (r : padic p) :
    norm (q * r) = norm q * norm r :=
  sorry

protected theorem is_norm {p : ℕ} [hp : fact (nat.prime p)] (q : padic p) :
    ↑(padic_norm_e q) = norm q :=
  rfl

theorem nonarchimedean {p : ℕ} [hp : fact (nat.prime p)] (q : padic p) (r : padic p) :
    norm (q + r) ≤ max (norm q) (norm r) :=
  sorry

theorem add_eq_max_of_ne {p : ℕ} [hp : fact (nat.prime p)] {q : padic p} {r : padic p}
    (h : norm q ≠ norm r) : norm (q + r) = max (norm q) (norm r) :=
  sorry

@[simp] theorem eq_padic_norm {p : ℕ} [hp : fact (nat.prime p)] (q : ℚ) :
    norm ↑q = ↑(padic_norm p q) :=
  sorry

protected instance padic.nondiscrete_normed_field {p : ℕ} [hp : fact (nat.prime p)] :
    nondiscrete_normed_field (padic p) :=
...
@[simp] theorem norm_p_pow {p : ℕ} [hp : fact (nat.prime p)] (n : ℤ) : norm (↑p ^ n) = ↑p ^ (-n) :=
  sorry

protected theorem image {p : ℕ} [hp : fact (nat.prime p)] {q : padic p} :
    q ≠ 0 → ∃ (n : ℤ), norm q = ↑(↑p ^ (-n)) :=
  sorry

protected theorem is_rat {p : ℕ} [hp : fact (nat.prime p)] (q : padic p) :
    ∃ (q' : ℚ), norm q = ↑q' :=
  sorry
...
theorem norm_rat_le_one {p : ℕ} [hp : fact (nat.prime p)] {q : ℚ} (hq : ¬p ∣ rat.denom q) :
    norm ↑q ≤ 1 :=
  sorry

theorem norm_int_le_one {p : ℕ} [hp : fact (nat.prime p)] (z : ℤ) : norm ↑z ≤ 1 := sorry

theorem norm_int_lt_one_iff_dvd {p : ℕ} [hp : fact (nat.prime p)] (k : ℤ) : norm ↑k < 1 ↔ ↑p ∣ k :=
  sorry

theorem norm_int_le_pow_iff_dvd {p : ℕ} [hp : fact (nat.prime p)] (k : ℤ) (n : ℕ) :
    norm ↑k ≤ ↑p ^ (-↑n) ↔ ↑(p ^ n) ∣ k :=
  sorry

theorem eq_of_norm_add_lt_right {p : ℕ} {hp : fact (nat.prime p)} {z1 : padic p} {z2 : padic p}
    (h : norm (z1 + z2) < norm z2) : norm z1 = norm z2 :=
  sorry

theorem eq_of_norm_add_lt_left {p : ℕ} {hp : fact (nat.prime p)} {z1 : padic p} {z2 : padic p}
    (h : norm (z1 + z2) < norm z1) : norm z1 = norm z2 :=
  sorry
...
theorem padic_norm_e_lim_le {p : ℕ} [fact (nat.prime p)] {f : cau_seq (padic p) norm} {a : ℝ}
    (ha : 0 < a) (hf : ∀ (i : ℕ), norm (coe_fn f i) ≤ a) : norm (cau_seq.lim f) ≤ a :=
  sorry
...
-/
def valuation {p : ℕ} [fact (nat.prime p)] : padic p → ℤ := quotient.lift padic_seq.valuation sorry
...
theorem norm_eq_pow_val {p : ℕ} [fact (nat.prime p)] {x : padic p} :
    x ≠ 0 → norm x = ↑p ^ (-valuation x) :=
  sorry
