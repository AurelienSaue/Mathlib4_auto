...
-/
def numeral (α : Type) [HasZero α] [HasOne α] [Add α] : parser α := nat.bin_cast <$> nat
...
at "1", so `"1"` is parsed in as `nat.cast 0`. Providing `"0"` to the parser causes a failure.
-/
def numeral.from_one.of_fintype (α : Type) [HasZero α] [HasOne α] [Add α] [fintype α] : parser α :=
  sorry
...
-/
def numeral.char (α : Type) [HasZero α] [HasOne α] [Add α] (fromc : char) (toc : char) : parser α :=
  sorry
...
-/
def numeral.char.of_fintype (α : Type) [HasZero α] [HasOne α] [Add α] [fintype α] (fromc : char) :
    parser α :=
  sorry
