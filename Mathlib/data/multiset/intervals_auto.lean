...
theorem map_sub (n : ℕ) (m : ℕ) (k : ℕ) (h : k ≤ n) : map (fun (x : ℕ) => x - k) (Ico n m) = Ico (n - k) (m - k) :=
  congr_arg coe (list.Ico.map_sub n m k h)

theorem zero_bot (n : ℕ) : Ico 0 n = range n :=
  congr_arg coe (list.Ico.zero_bot n)

@[simp] theorem card (n : ℕ) (m : ℕ) : coe_fn card (Ico n m) = m - n :=
  list.Ico.length n m
...
theorem add_consecutive {n : ℕ} {m : ℕ} {l : ℕ} (hnm : n ≤ m) (hml : m ≤ l) : Ico n m + Ico m l = Ico n l :=
...
theorem filter_le_of_bot {n : ℕ} {m : ℕ} (hnm : n < m) : filter (fun (x : ℕ) => x ≤ n) (Ico n m) = singleton n :=
