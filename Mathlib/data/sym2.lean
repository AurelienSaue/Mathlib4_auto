...
theorem rel.trans {α : Type u} {x : α × α} {y : α × α} {z : α × α} : rel α x y → rel α y z → rel α x z := sorry

theorem rel.is_equivalence {α : Type u} : equivalence (rel α) := sorry

protected instance rel.setoid (α : Type u) : setoid (α × α) :=
  setoid.mk (rel α) rel.is_equivalence
...
theorem eq_swap {α : Type u} {a : α} {b : α} : quotient.mk (a, b) = quotient.mk (b, a) :=
  eq.mpr (id (Eq._oldrec (Eq.refl (quotient.mk (a, b) = quotient.mk (b, a))) (propext quotient.eq))) (rel.swap a b)

theorem congr_right {α : Type u} {a : α} {b : α} {c : α} : quotient.mk (a, b) = quotient.mk (a, c) ↔ b = c := sorry

theorem congr_left {α : Type u} {a : α} {b : α} {c : α} : quotient.mk (b, a) = quotient.mk (c, a) ↔ b = c := sorry
...
theorem map_comp {α : Type u_1} {β : Type u_2} {γ : Type u_3} {g : β → γ} {f : α → β} : map (g ∘ f) = map g ∘ map f := sorry

@[simp] theorem map_pair_eq {α : Type u_1} {β : Type u_2} (f : α → β) (x : α) (y : α) : map f (quotient.mk (x, y)) = quotient.mk (f x, f y) := sorry
...
theorem mk_has_mem {α : Type u} (x : α) (y : α) : x ∈ quotient.mk (x, y) :=
  Exists.intro y rfl
...
-/
def mem.other {α : Type u} {a : α} {z : sym2 α} (h : a ∈ z) : α :=
  classical.some h

@[simp] theorem mem_other_spec {α : Type u} {a : α} {z : sym2 α} (h : a ∈ z) : quotient.mk (a, mem.other h) = z :=
  eq.mpr (id (Eq._oldrec (Eq.refl (quotient.mk (a, mem.other h) = z)) (Eq.symm (classical.some_spec h)))) (Eq.refl z)

theorem eq_iff {α : Type u} {x : α} {y : α} {z : α} {w : α} : quotient.mk (x, y) = quotient.mk (z, w) ↔ x = z ∧ y = w ∨ x = w ∧ y = z := sorry

@[simp] theorem mem_iff {α : Type u} {a : α} {b : α} {c : α} : a ∈ quotient.mk (b, c) ↔ a = b ∨ a = c := sorry

theorem mem_other_mem {α : Type u} {a : α} {z : sym2 α} (h : a ∈ z) : mem.other h ∈ z := sorry

theorem elems_iff_eq {α : Type u} {x : α} {y : α} {z : sym2 α} (hne : x ≠ y) : x ∈ z ∧ y ∈ z ↔ z = quotient.mk (x, y) := sorry

theorem sym2_ext {α : Type u} (z : sym2 α) (z' : sym2 α) (h : ∀ (x : α), x ∈ z ↔ x ∈ z') : z = z' := sorry

protected instance mem.decidable {α : Type u} [DecidableEq α] (x : α) (z : sym2 α) : Decidable (x ∈ z) :=
...
@[simp] theorem is_diag_iff_proj_eq {α : Type u} (z : α × α) : is_diag (quotient.mk z) ↔ prod.fst z = prod.snd z := sorry

protected instance is_diag.decidable_pred (α : Type u) [DecidableEq α] : decidable_pred is_diag :=
  fun (z : sym2 α) => quotient.rec_on_subsingleton z fun (a : α × α) => eq.mpr sorry (_inst_1 (prod.fst a) (prod.snd a))

theorem mem_other_ne {α : Type u} {a : α} {z : sym2 α} (hd : ¬is_diag z) (h : a ∈ z) : mem.other h ≠ a := sorry
...
@[simp] theorem from_rel_proj_prop {α : Type u} {r : α → α → Prop} {sym : symmetric r} {z : α × α} : quotient.mk z ∈ from_rel sym ↔ r (prod.fst z) (prod.snd z) :=
  iff.rfl

@[simp] theorem from_rel_prop {α : Type u} {r : α → α → Prop} {sym : symmetric r} {a : α} {b : α} : quotient.mk (a, b) ∈ from_rel sym ↔ r a b := sorry

theorem from_rel_irreflexive {α : Type u} {r : α → α → Prop} {sym : symmetric r} : irreflexive r ↔ ∀ {z : sym2 α}, z ∈ from_rel sym → ¬is_diag z := sorry

theorem mem_from_rel_irrefl_other_ne {α : Type u} {r : α → α → Prop} {sym : symmetric r} (irrefl : irreflexive r) {a : α} {z : sym2 α} (hz : z ∈ from_rel sym) (h : a ∈ z) : mem.other h ≠ a :=
  mem_other_ne (iff.mp from_rel_irreflexive irrefl z hz) h

protected instance from_rel.decidable_pred {α : Type u} {r : α → α → Prop} (sym : symmetric r) [h : DecidableRel r] : decidable_pred (from_rel sym) :=
...
def sym2_equiv_sym' {α : Type u_1} : sym2 α ≃ sym.sym' α (bit0 1) :=
  equiv.mk (quotient.map (fun (x : α × α) => { val := [prod.fst x, prod.snd x], property := sorry }) sorry)
...
-/
def equiv_multiset (α : Type u_1) : sym2 α ≃ Subtype fun (s : multiset α) => coe_fn multiset.card s = bit0 1 :=
  equiv_sym α
...
theorem rel_bool_spec {α : Type u} [DecidableEq α] (x : α × α) (y : α × α) : ↥(rel_bool x y) ↔ rel α x y := sorry
...
@[simp] theorem mem_other_spec' {α : Type u} [DecidableEq α] {a : α} {z : sym2 α} (h : a ∈ z) : quotient.mk (a, mem.other' h) = z := sorry

@[simp] theorem other_eq_other' {α : Type u} [DecidableEq α] {a : α} {z : sym2 α} (h : a ∈ z) : mem.other h = mem.other' h :=
  eq.mpr (id (Eq._oldrec (Eq.refl (mem.other h = mem.other' h)) (Eq.symm (propext congr_right))))
    (eq.mpr (id (Eq._oldrec (Eq.refl (quotient.mk (a, mem.other h) = quotient.mk (a, mem.other' h))) (mem_other_spec' h)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (quotient.mk (a, mem.other h) = z)) (mem_other_spec h))) (Eq.refl z)))

theorem mem_other_mem' {α : Type u} [DecidableEq α] {a : α} {z : sym2 α} (h : a ∈ z) : mem.other' h ∈ z :=
  eq.mpr (id (Eq._oldrec (Eq.refl (mem.other' h ∈ z)) (Eq.symm (other_eq_other' h)))) (mem_other_mem h)

theorem other_invol' {α : Type u} [DecidableEq α] {a : α} {z : sym2 α} (ha : a ∈ z) (hb : mem.other' ha ∈ z) : mem.other' hb = a := sorry

theorem other_invol {α : Type u} {a : α} {z : sym2 α} (ha : a ∈ z) (hb : mem.other ha ∈ z) : mem.other hb = a := sorry
