...
protected instance infinite : infinite ℚ :=
  infinite.of_injective coe nat.cast_injective

protected instance denumerable : denumerable ℚ :=
  let T : Type := Subtype fun (x : ℤ × ℕ) => 0 < prod.snd x ∧ nat.coprime (int.nat_abs (prod.fst x)) (prod.snd x);
...
theorem mk_rat : mk ℚ = omega :=
  iff.mp denumerable_iff (Nonempty.intro rat.denumerable)
