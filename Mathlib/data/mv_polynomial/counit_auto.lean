...
def acounit (A : Type u_1) (B : Type u_2) [comm_semiring A] [comm_semiring B] [algebra A B] : alg_hom A (mv_polynomial B A) B :=
  aeval id

@[simp] theorem acounit_X (A : Type u_1) {B : Type u_2} [comm_semiring A] [comm_semiring B] [algebra A B] (b : B) : coe_fn (acounit A B) (X b) = b :=
  aeval_X id b

@[simp] theorem acounit_C {A : Type u_1} (B : Type u_2) [comm_semiring A] [comm_semiring B] [algebra A B] (a : A) : coe_fn (acounit A B) (coe_fn C a) = coe_fn (algebra_map A B) a :=
  aeval_C id a

theorem acounit_surjective (A : Type u_1) (B : Type u_2) [comm_semiring A] [comm_semiring B] [algebra A B] : function.surjective ⇑(acounit A B) :=
...
def counit (R : Type u_3) [comm_ring R] : mv_polynomial R ℤ →+* R :=
  ↑(acounit ℤ R)
...
def counit_nat (A : Type u_1) [comm_semiring A] : mv_polynomial A ℕ →+* A :=
  ↑(acounit ℕ A)

theorem counit_surjective (R : Type u_3) [comm_ring R] : function.surjective ⇑(counit R) :=
  acounit_surjective ℤ R

theorem counit_nat_surjective (A : Type u_1) [comm_semiring A] : function.surjective ⇑(counit_nat A) :=
  acounit_surjective ℕ A

theorem counit_C (R : Type u_3) [comm_ring R] (n : ℤ) : coe_fn (counit R) (coe_fn C n) = ↑n :=
  acounit_C R n

theorem counit_nat_C (A : Type u_1) [comm_semiring A] (n : ℕ) : coe_fn (counit_nat A) (coe_fn C n) = ↑n :=
  acounit_C A n

@[simp] theorem counit_X {R : Type u_3} [comm_ring R] (r : R) : coe_fn (counit R) (X r) = r :=
  acounit_X ℤ r

@[simp] theorem counit_nat_X {A : Type u_1} [comm_semiring A] (a : A) : coe_fn (counit_nat A) (X a) = a :=
  acounit_X ℕ a
