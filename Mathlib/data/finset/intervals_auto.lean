...
def Ico (n : ℕ) (m : ℕ) : finset ℕ := mk (multiset.Ico n m) (multiset.Ico.nodup n m)

namespace Ico


@[simp] theorem val (n : ℕ) (m : ℕ) : val (Ico n m) = multiset.Ico n m := rfl
...
theorem image_add (n : ℕ) (m : ℕ) (k : ℕ) : image (Add.add k) (Ico n m) = Ico (n + k) (m + k) :=
  sorry

theorem image_sub (n : ℕ) (m : ℕ) (k : ℕ) (h : k ≤ n) :
    image (fun (x : ℕ) => x - k) (Ico n m) = Ico (n - k) (m - k) :=
  sorry

theorem zero_bot (n : ℕ) : Ico 0 n = range n := eq_of_veq (multiset.Ico.zero_bot n)

@[simp] theorem card (n : ℕ) (m : ℕ) : card (Ico n m) = m - n := multiset.Ico.card n m

@[simp] theorem mem {n : ℕ} {m : ℕ} {l : ℕ} : l ∈ Ico n m ↔ n ≤ l ∧ l < m := multiset.Ico.mem
...
@[simp] theorem self_eq_empty (n : ℕ) : Ico n n = ∅ := eq_empty_of_le (le_refl n)

@[simp] theorem eq_empty_iff {n : ℕ} {m : ℕ} : Ico n m = ∅ ↔ m ≤ n :=
  iff.trans (iff.symm val_eq_zero) multiset.Ico.eq_zero_iff

theorem subset_iff {m₁ : ℕ} {n₁ : ℕ} {m₂ : ℕ} {n₂ : ℕ} (hmn : m₁ < n₁) :
    Ico m₁ n₁ ⊆ Ico m₂ n₂ ↔ m₂ ≤ m₁ ∧ n₁ ≤ n₂ :=
  sorry

protected theorem subset {m₁ : ℕ} {n₁ : ℕ} {m₂ : ℕ} {n₂ : ℕ} (hmm : m₂ ≤ m₁) (hnn : n₁ ≤ n₂) :
    Ico m₁ n₁ ⊆ Ico m₂ n₂ :=
  eq.mpr
    (id
      (Eq.trans (propext subset_iff)
        (forall_congr_eq fun (x : ℕ) => imp_congr_eq (propext mem) (propext mem))))
    fun (x : ℕ) (hx : m₁ ≤ x ∧ x < n₁) =>
      { left := le_trans hmm (and.left hx), right := lt_of_lt_of_le (and.right hx) hnn }

theorem union_consecutive {n : ℕ} {m : ℕ} {l : ℕ} (hnm : n ≤ m) (hml : m ≤ l) :
    Ico n m ∪ Ico m l = Ico n l :=
  sorry

theorem union' {n : ℕ} {m : ℕ} {l : ℕ} {k : ℕ} (hlm : l ≤ m) (hnk : n ≤ k) :
    Ico n m ∪ Ico l k = Ico (min n l) (max m k) :=
  sorry

theorem union {n : ℕ} {m : ℕ} {l : ℕ} {k : ℕ} (h₁ : min n m ≤ max l k) (h₂ : min l k ≤ max n m) :
    Ico n m ∪ Ico l k = Ico (min n l) (max m k) :=
  sorry
...
@[simp] theorem not_mem_top {n : ℕ} {m : ℕ} : ¬m ∈ Ico n m := multiset.Ico.not_mem_top
...
theorem filter_Ico_bot {n : ℕ} {m : ℕ} (hnm : n < m) :
...
theorem image_const_sub {k : ℕ} {m : ℕ} {n : ℕ} (hkn : k ≤ n) :
    image (fun (j : ℕ) => n - j) (Ico k m) = Ico (n + 1 - m) (n + 1 - k) :=
  sorry
...
theorem range_image_pred_top_sub (n : ℕ) : image (fun (j : ℕ) => n - 1 - j) (range n) = range n :=
  sorry
