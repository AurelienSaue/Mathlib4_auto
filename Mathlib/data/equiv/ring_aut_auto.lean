...
def ring_aut (R : Type u_1) [Mul R] [Add R] := R ≃+* R
...
protected instance group (R : Type u_1) [Mul R] [Add R] : group (ring_aut R) :=
  group.mk (fun (g h : R ≃+* R) => ring_equiv.trans h g) sorry (ring_equiv.refl R) sorry sorry
    ring_equiv.symm (fun (a b : R ≃+* R) => a * ring_equiv.symm b) sorry
