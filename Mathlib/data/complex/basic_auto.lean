...
protected instance decidable_eq : DecidableEq ℂ := classical.dec_eq ℂ

/-- The equivalence between the complex numbers and `ℝ × ℝ`. -/
def equiv_real_prod : ℂ ≃ ℝ × ℝ :=
  equiv.mk (fun (z : ℂ) => (re z, im z)) (fun (p : ℝ × ℝ) => mk (prod.fst p) (prod.snd p)) sorry
    sorry

@[simp] theorem equiv_real_prod_apply (z : ℂ) : coe_fn equiv_real_prod z = (re z, im z) := rfl

theorem equiv_real_prod_symm_re (x : ℝ) (y : ℝ) :
    re (coe_fn (equiv.symm equiv_real_prod) (x, y)) = x :=
  rfl

theorem equiv_real_prod_symm_im (x : ℝ) (y : ℝ) :
    im (coe_fn (equiv.symm equiv_real_prod) (x, y)) = y :=
  rfl
...
@[simp] theorem eta (z : ℂ) : mk (re z) (im z) = z :=
  cases_on z
    fun (z_re z_im : ℝ) =>
      idRhs
        (mk (re (mk z_re z_im)) (im (mk z_re z_im)) = mk (re (mk z_re z_im)) (im (mk z_re z_im)))
        rfl
...
protected instance has_coe : has_coe ℝ ℂ := has_coe.mk fun (r : ℝ) => mk r 0

@[simp] theorem of_real_re (r : ℝ) : re ↑r = r := rfl

@[simp] theorem of_real_im (r : ℝ) : im ↑r = 0 := rfl

@[simp] theorem of_real_inj {z : ℝ} {w : ℝ} : ↑z = ↑w ↔ z = w :=
  { mp := congr_arg re, mpr := congr_arg fun {z : ℝ} => ↑z }

protected instance has_zero : HasZero ℂ := { zero := ↑0 }

protected instance inhabited : Inhabited ℂ := { default := 0 }

@[simp] theorem zero_re : re 0 = 0 := rfl

@[simp] theorem zero_im : im 0 = 0 := rfl

@[simp] theorem of_real_zero : ↑0 = 0 := rfl

@[simp] theorem of_real_eq_zero {z : ℝ} : ↑z = 0 ↔ z = 0 := of_real_inj

theorem of_real_ne_zero {z : ℝ} : ↑z ≠ 0 ↔ z ≠ 0 := not_congr of_real_eq_zero

protected instance has_one : HasOne ℂ := { one := ↑1 }

@[simp] theorem one_re : re 1 = 1 := rfl

@[simp] theorem one_im : im 1 = 0 := rfl

@[simp] theorem of_real_one : ↑1 = 1 := rfl

protected instance has_add : Add ℂ := { add := fun (z w : ℂ) => mk (re z + re w) (im z + im w) }

@[simp] theorem add_re (z : ℂ) (w : ℂ) : re (z + w) = re z + re w := rfl

@[simp] theorem add_im (z : ℂ) (w : ℂ) : im (z + w) = im z + im w := rfl

@[simp] theorem bit0_re (z : ℂ) : re (bit0 z) = bit0 (re z) := rfl

@[simp] theorem bit1_re (z : ℂ) : re (bit1 z) = bit1 (re z) := rfl

@[simp] theorem bit0_im (z : ℂ) : im (bit0 z) = bit0 (im z) := Eq.refl (im (bit0 z))

@[simp] theorem bit1_im (z : ℂ) : im (bit1 z) = bit0 (im z) := add_zero (im (bit0 z))
...
protected instance has_neg : Neg ℂ := { neg := fun (z : ℂ) => mk (-re z) (-im z) }

@[simp] theorem neg_re (z : ℂ) : re (-z) = -re z := rfl

@[simp] theorem neg_im (z : ℂ) : im (-z) = -im z := rfl

@[simp] theorem of_real_neg (r : ℝ) : ↑(-r) = -↑r := sorry

protected instance has_sub : Sub ℂ := { sub := fun (z w : ℂ) => mk (re z - re w) (im z - im w) }

protected instance has_mul : Mul ℂ :=
  { mul := fun (z w : ℂ) => mk (re z * re w - im z * im w) (re z * im w + im z * re w) }

@[simp] theorem mul_re (z : ℂ) (w : ℂ) : re (z * w) = re z * re w - im z * im w := rfl

@[simp] theorem mul_im (z : ℂ) (w : ℂ) : im (z * w) = re z * im w + im z * re w := rfl
...
@[simp] theorem I_re : re I = 0 := rfl

@[simp] theorem I_im : im I = 1 := rfl

@[simp] theorem I_mul_I : I * I = -1 := sorry

theorem I_mul (z : ℂ) : I * z = mk (-im z) (re z) := sorry

theorem I_ne_zero : I ≠ 0 := mt (congr_arg im) (ne.symm zero_ne_one)
...
protected instance re.is_add_group_hom : is_add_group_hom re := is_add_group_hom.mk

protected instance im.is_add_group_hom : is_add_group_hom im := is_add_group_hom.mk
...
@[simp] theorem I_pow_bit1 (n : ℕ) : I ^ bit1 n = (-1) ^ n * I :=
  eq.mpr (id (Eq._oldrec (Eq.refl (I ^ bit1 n = (-1) ^ n * I)) (pow_bit1' I n)))
    (eq.mpr (id (Eq._oldrec (Eq.refl ((I * I) ^ n * I = (-1) ^ n * I)) I_mul_I))
      (Eq.refl ((-1) ^ n * I)))

/-! ### Complex conjugation -/

/-- The complex conjugate. -/
def conj : ℂ →+* ℂ := ring_hom.mk (fun (z : ℂ) => mk (re z) (-im z)) sorry sorry sorry sorry

@[simp] theorem conj_re (z : ℂ) : re (coe_fn conj z) = re z := rfl

@[simp] theorem conj_im (z : ℂ) : im (coe_fn conj z) = -im z := rfl
...
theorem conj_involutive : function.involutive ⇑conj := conj_conj

theorem conj_bijective : function.bijective ⇑conj := function.involutive.bijective conj_involutive
...
protected instance star_ring : star_ring ℂ := star_ring.mk sorry
...
theorem norm_sq_apply (z : ℂ) : coe_fn norm_sq z = re z * re z + im z * im z := rfl

@[simp] theorem norm_sq_of_real (r : ℝ) : coe_fn norm_sq ↑r = r * r := sorry

theorem norm_sq_zero : coe_fn norm_sq 0 = 0 := monoid_with_zero_hom.map_zero norm_sq

theorem norm_sq_one : coe_fn norm_sq 1 = 1 := monoid_with_zero_hom.map_one norm_sq
...
theorem norm_sq_mul (z : ℂ) (w : ℂ) :
    coe_fn norm_sq (z * w) = coe_fn norm_sq z * coe_fn norm_sq w :=
  monoid_with_zero_hom.map_mul norm_sq z w

theorem norm_sq_add (z : ℂ) (w : ℂ) :
    coe_fn norm_sq (z + w) =
        coe_fn norm_sq z + coe_fn norm_sq w + bit0 1 * re (z * coe_fn conj w) :=
  sorry
...
def of_real : ℝ →+* ℂ := ring_hom.mk coe of_real_one of_real_mul of_real_zero of_real_add

@[simp] theorem of_real_eq_coe (r : ℝ) : coe_fn of_real r = ↑r := rfl
...
@[simp] theorem sub_re (z : ℂ) (w : ℂ) : re (z - w) = re z - re w := rfl

@[simp] theorem sub_im (z : ℂ) (w : ℂ) : im (z - w) = im z - im w := rfl
...
theorem inv_def (z : ℂ) : z⁻¹ = coe_fn conj z * ↑(coe_fn norm_sq z⁻¹) := rfl
...
protected instance field : field ℂ :=
  field.mk comm_ring.add comm_ring.add_assoc comm_ring.zero comm_ring.zero_add comm_ring.add_zero
    ...
    comm_ring.right_distrib comm_ring.mul_comm has_inv.inv sorry complex.mul_inv_cancel
    complex.inv_zero
...
theorem div_re (z : ℂ) (w : ℂ) :
    re (z / w) = re z * re w / coe_fn norm_sq w + im z * im w / coe_fn norm_sq w :=
  sorry

theorem div_im (z : ℂ) (w : ℂ) :
    im (z / w) = im z * re w / coe_fn norm_sq w - re z * im w / coe_fn norm_sq w :=
  sorry

@[simp] theorem of_real_div (r : ℝ) (s : ℝ) : ↑(r / s) = ↑r / ↑s := ring_hom.map_div of_real r s

@[simp] theorem of_real_fpow (r : ℝ) (n : ℤ) : ↑(r ^ n) = ↑r ^ n := ring_hom.map_fpow of_real r n
...
@[simp] theorem norm_sq_div (z : ℂ) (w : ℂ) :
    coe_fn norm_sq (z / w) = coe_fn norm_sq z / coe_fn norm_sq w :=
  monoid_with_zero_hom.map_div norm_sq z w

/-! ### Cast lemmas -/

@[simp] theorem of_real_nat_cast (n : ℕ) : ↑↑n = ↑n := ring_hom.map_nat_cast of_real n
...
@[simp] theorem of_real_int_cast (n : ℤ) : ↑↑n = ↑n := ring_hom.map_int_cast of_real n
...
@[simp] theorem of_real_rat_cast (n : ℚ) : ↑↑n = ↑n := ring_hom.map_rat_cast of_real n
...
def abs (z : ℂ) : ℝ := real.sqrt (coe_fn norm_sq z)

@[simp] theorem abs_of_real (r : ℝ) : abs ↑r = abs r := sorry

theorem abs_of_nonneg {r : ℝ} (h : 0 ≤ r) : abs ↑r = r := Eq.trans (abs_of_real r) (abs_of_nonneg h)

theorem abs_of_nat (n : ℕ) : abs ↑n = ↑n :=
  Eq.trans
    (eq.mpr (id (Eq._oldrec (Eq.refl (abs ↑n = abs ↑↑n)) (of_real_nat_cast n))) (Eq.refl (abs ↑n)))
...
theorem abs_nonneg (z : ℂ) : 0 ≤ abs z := real.sqrt_nonneg (coe_fn norm_sq z)

@[simp] theorem abs_eq_zero {z : ℂ} : abs z = 0 ↔ z = 0 :=
  iff.trans (real.sqrt_eq_zero (norm_sq_nonneg z)) norm_sq_eq_zero

theorem abs_ne_zero {z : ℂ} : abs z ≠ 0 ↔ z ≠ 0 := not_congr abs_eq_zero
...
theorem re_le_abs (z : ℂ) : re z ≤ abs z := and.right (iff.mp abs_le (abs_re_le_abs z))

theorem im_le_abs (z : ℂ) : im z ≤ abs z := and.right (iff.mp abs_le (abs_im_le_abs z))
...
@[simp] theorem abs_abs (z : ℂ) : abs (abs z) = abs z := abs_of_nonneg (abs_nonneg z)

@[simp] theorem abs_pos {z : ℂ} : 0 < abs z ↔ z ≠ 0 := is_absolute_value.abv_pos abs

@[simp] theorem abs_neg (z : ℂ) : abs (-z) = abs z := is_absolute_value.abv_neg abs

theorem abs_sub (z : ℂ) (w : ℂ) : abs (z - w) = abs (w - z) := is_absolute_value.abv_sub abs

theorem abs_sub_le (a : ℂ) (b : ℂ) (c : ℂ) : abs (a - c) ≤ abs (a - b) + abs (b - c) :=
  is_absolute_value.abv_sub_le abs

@[simp] theorem abs_inv (z : ℂ) : abs (z⁻¹) = (abs z⁻¹) := is_absolute_value.abv_inv abs
...
def lim_aux (f : cau_seq ℂ abs) : ℂ := mk (cau_seq.lim (cau_seq_re f)) (cau_seq.lim (cau_seq_im f))
...
theorem lim_eq_lim_im_add_lim_re (f : cau_seq ℂ abs) :
    cau_seq.lim f = ↑(cau_seq.lim (cau_seq_re f)) + ↑(cau_seq.lim (cau_seq_im f)) * I :=
  sorry
...
theorem is_cau_seq_conj (f : cau_seq ℂ abs) :
    is_cau_seq abs fun (n : ℕ) => coe_fn conj (coe_fn f n) :=
  sorry
...
theorem lim_conj (f : cau_seq ℂ abs) : cau_seq.lim (cau_seq_conj f) = coe_fn conj (cau_seq.lim f) :=
  sorry
...
@[simp] theorem of_real_prod {α : Type u_1} (s : finset α) (f : α → ℝ) :
    ↑(finset.prod s fun (i : α) => f i) = finset.prod s fun (i : α) => ↑(f i) :=
  ring_hom.map_prod of_real (fun (x : α) => f x) s

@[simp] theorem of_real_sum {α : Type u_1} (s : finset α) (f : α → ℝ) :
