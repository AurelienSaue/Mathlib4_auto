...
class is_R_or_C (K : Type u_1)
    extends nondiscrete_normed_field K, normed_algebra ℝ K, complete_space K where
  re : K →+ ℝ
  im : K →+ ℝ
  conj : K →+* K
  I : K
  ...
  re_add_im_ax :
    ∀ (z : K),
      coe_fn (algebra_map ℝ K) (coe_fn re z) + coe_fn (algebra_map ℝ K) (coe_fn im z) * I = z
  ...
  norm_sq_eq_def_ax :
    ∀ (z : K), norm z ^ bit0 1 = coe_fn re z * coe_fn re z + coe_fn im z * coe_fn im z
...
theorem algebra_map_eq_of_real {K : Type u_1} [is_R_or_C K] : ⇑(algebra_map ℝ K) = coe := rfl

@[simp] theorem re_add_im {K : Type u_1} [is_R_or_C K] (z : K) :
    ↑(coe_fn re z) + ↑(coe_fn im z) * I = z :=
  re_add_im_ax z

@[simp] theorem of_real_re {K : Type u_1} [is_R_or_C K] (r : ℝ) : coe_fn re ↑r = r := of_real_re_ax

@[simp] theorem of_real_im {K : Type u_1} [is_R_or_C K] (r : ℝ) : coe_fn im ↑r = 0 := of_real_im_ax

@[simp] theorem mul_re {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn re (z * w) = coe_fn re z * coe_fn re w - coe_fn im z * coe_fn im w :=
  mul_re_ax

@[simp] theorem mul_im {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn im (z * w) = coe_fn re z * coe_fn im w + coe_fn im z * coe_fn re w :=
  mul_im_ax

theorem inv_def {K : Type u_1} [is_R_or_C K] (z : K) : z⁻¹ = coe_fn conj z * ↑(norm z ^ bit0 1⁻¹) :=
  inv_def_ax z

theorem ext_iff {K : Type u_1} [is_R_or_C K] {z : K} {w : K} :
    z = w ↔ coe_fn re z = coe_fn re w ∧ coe_fn im z = coe_fn im w :=
  sorry

theorem ext {K : Type u_1} [is_R_or_C K] {z : K} {w : K} :
    coe_fn re z = coe_fn re w → coe_fn im z = coe_fn im w → z = w :=
  sorry
...
@[simp] theorem bit0_re {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn re (bit0 z) = bit0 (coe_fn re z) :=
  sorry

@[simp] theorem bit1_re {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn re (bit1 z) = bit1 (coe_fn re z) :=
  sorry

@[simp] theorem bit0_im {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im (bit0 z) = bit0 (coe_fn im z) :=
  sorry

@[simp] theorem bit1_im {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im (bit1 z) = bit0 (coe_fn im z) :=
  sorry
...
@[simp] theorem of_real_add {K : Type u_1} [is_R_or_C K] {r : ℝ} {s : ℝ} : ↑(r + s) = ↑r + ↑s :=
  sorry
...
@[simp] theorem of_real_mul {K : Type u_1} [is_R_or_C K] (r : ℝ) (s : ℝ) : ↑(r * s) = ↑r * ↑s :=
  sorry

theorem of_real_mul_re {K : Type u_1} [is_R_or_C K] (r : ℝ) (z : K) :
    coe_fn re (↑r * z) = r * coe_fn re z :=
  sorry

theorem smul_re {K : Type u_1} [is_R_or_C K] (r : ℝ) (z : K) :
    coe_fn re (↑r * z) = r * coe_fn re z :=
  sorry

theorem smul_im {K : Type u_1} [is_R_or_C K] (r : ℝ) (z : K) :
    coe_fn im (↑r * z) = r * coe_fn im z :=
  sorry

theorem smul_re' {K : Type u_1} [is_R_or_C K] (r : ℝ) (z : K) :
    coe_fn re (r • z) = r * coe_fn re z :=
  eq.mpr (id (Eq._oldrec (Eq.refl (coe_fn re (r • z) = r * coe_fn re z)) (algebra.smul_def r z)))
    (smul_re r z)

theorem smul_im' {K : Type u_1} [is_R_or_C K] (r : ℝ) (z : K) :
    coe_fn im (r • z) = r * coe_fn im z :=
  eq.mpr (id (Eq._oldrec (Eq.refl (coe_fn im (r • z) = r * coe_fn im z)) (algebra.smul_def r z)))
    (smul_im r z)
...
@[simp] theorem re_lm_coe {K : Type u_1} [is_R_or_C K] : ⇑re_lm = ⇑re := rfl

/-! ### The imaginary unit, `I` -/

/-- The imaginary unit. -/
@[simp] theorem I_re {K : Type u_1} [is_R_or_C K] : coe_fn re I = 0 := I_re_ax

@[simp] theorem I_im {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im z * coe_fn im I = coe_fn im z :=
  mul_im_I_ax z

@[simp] theorem I_im' {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im I * coe_fn im z = coe_fn im z :=
  eq.mpr
    (id
      (Eq._oldrec (Eq.refl (coe_fn im I * coe_fn im z = coe_fn im z))
        (mul_comm (coe_fn im I) (coe_fn im z))))
    (eq.mpr (id (Eq._oldrec (Eq.refl (coe_fn im z * coe_fn im I = coe_fn im z)) (I_im z)))
      (Eq.refl (coe_fn im z)))

theorem I_mul_re {K : Type u_1} [is_R_or_C K] (z : K) : coe_fn re (I * z) = -coe_fn im z := sorry

theorem I_mul_I {K : Type u_1} [is_R_or_C K] : I = 0 ∨ I * I = -1 := I_mul_I_ax

@[simp] theorem conj_re {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn re (coe_fn conj z) = coe_fn re z :=
  conj_re_ax z

@[simp] theorem conj_im {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im (coe_fn conj z) = -coe_fn im z :=
  conj_im_ax z

@[simp] theorem conj_of_real {K : Type u_1} [is_R_or_C K] (r : ℝ) : coe_fn conj ↑r = ↑r := sorry

@[simp] theorem conj_bit0 {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn conj (bit0 z) = bit0 (coe_fn conj z) :=
  sorry

@[simp] theorem conj_bit1 {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn conj (bit1 z) = bit1 (coe_fn conj z) :=
  sorry

@[simp] theorem conj_neg_I {K : Type u_1} [is_R_or_C K] : coe_fn conj (-I) = I := sorry

@[simp] theorem conj_conj {K : Type u_1} [is_R_or_C K] (z : K) : coe_fn conj (coe_fn conj z) = z :=
  sorry

theorem conj_involutive {K : Type u_1} [is_R_or_C K] : function.involutive ⇑conj := conj_conj

theorem conj_bijective {K : Type u_1} [is_R_or_C K] : function.bijective ⇑conj :=
  function.involutive.bijective conj_involutive

theorem conj_inj {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn conj z = coe_fn conj w ↔ z = w :=
...
theorem eq_conj_iff_real {K : Type u_1} [is_R_or_C K] {z : K} :
    coe_fn conj z = z ↔ ∃ (r : ℝ), z = ↑r :=
  sorry
...
@[simp] theorem ring_equiv_apply {K : Type u_1} [is_R_or_C K] {x : K} :
    opposite.unop (coe_fn (conj_to_ring_equiv K) x) = coe_fn conj x :=
  rfl

theorem eq_conj_iff_re {K : Type u_1} [is_R_or_C K] {z : K} :
    coe_fn conj z = z ↔ ↑(coe_fn re z) = z :=
  sorry

/-- The norm squared function. -/
def norm_sq {K : Type u_1} [is_R_or_C K] : monoid_with_zero_hom K ℝ :=
  monoid_with_zero_hom.mk (fun (z : K) => coe_fn re z * coe_fn re z + coe_fn im z * coe_fn im z)
    sorry sorry sorry

theorem norm_sq_eq_def {K : Type u_1} [is_R_or_C K] {z : K} :
    norm z ^ bit0 1 = coe_fn re z * coe_fn re z + coe_fn im z * coe_fn im z :=
  norm_sq_eq_def_ax z

theorem norm_sq_eq_def' {K : Type u_1} [is_R_or_C K] (z : K) : coe_fn norm_sq z = norm z ^ bit0 1 :=
  eq.mpr (id (Eq._oldrec (Eq.refl (coe_fn norm_sq z = norm z ^ bit0 1)) norm_sq_eq_def))
    (Eq.refl (coe_fn norm_sq z))

@[simp] theorem norm_sq_of_real {K : Type u_1} [is_R_or_C K] (r : ℝ) : norm ↑r ^ bit0 1 = r * r :=
  sorry
...
@[simp] theorem norm_sq_eq_zero {K : Type u_1} [is_R_or_C K] {z : K} :
    coe_fn norm_sq z = 0 ↔ z = 0 :=
  sorry

@[simp] theorem norm_sq_pos {K : Type u_1} [is_R_or_C K] {z : K} : 0 < coe_fn norm_sq z ↔ z ≠ 0 :=
  sorry

@[simp] theorem norm_sq_neg {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn norm_sq (-z) = coe_fn norm_sq z :=
  sorry

@[simp] theorem norm_sq_conj {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn norm_sq (coe_fn conj z) = coe_fn norm_sq z :=
  sorry

@[simp] theorem norm_sq_mul {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn norm_sq (z * w) = coe_fn norm_sq z * coe_fn norm_sq w :=
  monoid_with_zero_hom.map_mul norm_sq z w

theorem norm_sq_add {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn norm_sq (z + w) =
        coe_fn norm_sq z + coe_fn norm_sq w + bit0 1 * coe_fn re (z * coe_fn conj w) :=
  sorry

theorem re_sq_le_norm_sq {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn re z * coe_fn re z ≤ coe_fn norm_sq z :=
  le_add_of_nonneg_right (mul_self_nonneg (coe_fn im z))

theorem im_sq_le_norm_sq {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im z * coe_fn im z ≤ coe_fn norm_sq z :=
  le_add_of_nonneg_left (mul_self_nonneg (coe_fn re z))

theorem mul_conj {K : Type u_1} [is_R_or_C K] (z : K) : z * coe_fn conj z = ↑(coe_fn norm_sq z) :=
  sorry

theorem add_conj {K : Type u_1} [is_R_or_C K] (z : K) :
    z + coe_fn conj z = bit0 1 * ↑(coe_fn re z) :=
  sorry

/-- The pseudo-coercion `of_real` as a `ring_hom`. -/
def of_real_hom {K : Type u_1} [is_R_or_C K] : ℝ →+* K := algebra_map ℝ K
...
@[simp] theorem of_real_sub {K : Type u_1} [is_R_or_C K] (r : ℝ) (s : ℝ) : ↑(r - s) = ↑r - ↑s :=
  sorry

@[simp] theorem of_real_pow {K : Type u_1} [is_R_or_C K] (r : ℝ) (n : ℕ) : ↑(r ^ n) = ↑r ^ n :=
  sorry

theorem sub_conj {K : Type u_1} [is_R_or_C K] (z : K) :
    z - coe_fn conj z = bit0 1 * ↑(coe_fn im z) * I :=
  sorry

theorem norm_sq_sub {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn norm_sq (z - w) =
        coe_fn norm_sq z + coe_fn norm_sq w - bit0 1 * coe_fn re (z * coe_fn conj w) :=
  sorry

theorem sqrt_norm_sq_eq_norm {K : Type u_1} [is_R_or_C K] {z : K} :
    real.sqrt (coe_fn norm_sq z) = norm z :=
  eq.mpr
    (id
      (Eq._oldrec (Eq.refl (real.sqrt (coe_fn norm_sq z) = norm z))
        (Eq.symm (real.sqrt_sqr (norm_nonneg z)))))
    (congr_arg real.sqrt (norm_sq_eq_def' z))

/-! ### Inversion -/

@[simp] theorem inv_re {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn re (z⁻¹) = coe_fn re z / coe_fn norm_sq z :=
  sorry

@[simp] theorem inv_im {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn im (z⁻¹) = coe_fn im (-z) / coe_fn norm_sq z :=
  sorry
...
protected theorem mul_inv_cancel {K : Type u_1} [is_R_or_C K] {z : K} (h : z ≠ 0) : z * (z⁻¹) = 1 :=
  sorry

theorem div_re {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn re (z / w) =
        coe_fn re z * coe_fn re w / coe_fn norm_sq w +
          coe_fn im z * coe_fn im w / coe_fn norm_sq w :=
  sorry

theorem div_im {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn im (z / w) =
        coe_fn im z * coe_fn re w / coe_fn norm_sq w -
          coe_fn re z * coe_fn im w / coe_fn norm_sq w :=
  sorry

@[simp] theorem of_real_div {K : Type u_1} [is_R_or_C K] (r : ℝ) (s : ℝ) : ↑(r / s) = ↑r / ↑s :=
  ring_hom.map_div coe_hom r s

theorem div_re_of_real {K : Type u_1} [is_R_or_C K] {z : K} {r : ℝ} :
    coe_fn re (z / ↑r) = coe_fn re z / r :=
  sorry
...
theorem I_mul_I_of_nonzero {K : Type u_1} [is_R_or_C K] : I ≠ 0 → I * I = -1 :=
  fun (ᾰ : I ≠ 0) =>
    or.dcases_on I_mul_I_ax (fun (this : I = 0) => absurd this ᾰ)
      fun (this : I * I = -1) => eq.mpr rfl this

@[simp] theorem div_I {K : Type u_1} [is_R_or_C K] (z : K) : z / I = -(z * I) := sorry

@[simp] theorem inv_I {K : Type u_1} [is_R_or_C K] : I⁻¹ = -I := sorry

@[simp] theorem norm_sq_inv {K : Type u_1} [is_R_or_C K] (z : K) :
    coe_fn norm_sq (z⁻¹) = (coe_fn norm_sq z⁻¹) :=
  monoid_with_zero_hom.map_inv' norm_sq z

@[simp] theorem norm_sq_div {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    coe_fn norm_sq (z / w) = coe_fn norm_sq z / coe_fn norm_sq w :=
  monoid_with_zero_hom.map_div norm_sq z w

theorem norm_conj {K : Type u_1} [is_R_or_C K] {z : K} : norm (coe_fn conj z) = norm z := sorry

theorem conj_inv {K : Type u_1} [is_R_or_C K] {z : K} : coe_fn conj (z⁻¹) = (coe_fn conj z⁻¹) :=
  sorry

theorem conj_div {K : Type u_1} [is_R_or_C K] {z : K} {w : K} :
    coe_fn conj (z / w) = coe_fn conj z / coe_fn conj w :=
  sorry
...
theorem re_eq_add_conj {K : Type u_1} [is_R_or_C K] (z : K) :
    ↑(coe_fn re z) = (z + coe_fn conj z) / bit0 1 :=
  sorry

/-! ### Absolute value -/

/-- The complex absolute value function, defined as the square root of the norm squared. -/
def abs {K : Type u_1} [is_R_or_C K] (z : K) : ℝ := real.sqrt (coe_fn norm_sq z)
...
@[simp] theorem abs_mul {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    abs (z * w) = abs z * abs w :=
  sorry
...
theorem im_eq_zero_of_le {K : Type u_1} [is_R_or_C K] {a : K} (h : abs a ≤ coe_fn re a) :
    coe_fn im a = 0 :=
  sorry

theorem re_eq_self_of_le {K : Type u_1} [is_R_or_C K] {a : K} (h : abs a ≤ coe_fn re a) :
    ↑(coe_fn re a) = a :=
  sorry
...
theorem abs_sub_le {K : Type u_1} [is_R_or_C K] (a : K) (b : K) (c : K) :
...
@[simp] theorem abs_div {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    abs (z / w) = abs z / abs w :=
  is_absolute_value.abv_div abs

theorem abs_abs_sub_le_abs_sub {K : Type u_1} [is_R_or_C K] (z : K) (w : K) :
    abs (abs z - abs w) ≤ abs (z - w) :=
  is_absolute_value.abs_abv_sub_le_abv_sub abs

theorem abs_re_div_abs_le_one {K : Type u_1} [is_R_or_C K] (z : K) :
    abs (coe_fn re z / abs z) ≤ 1 :=
  sorry

theorem abs_im_div_abs_le_one {K : Type u_1} [is_R_or_C K] (z : K) :
    abs (coe_fn im z / abs z) ≤ 1 :=
  sorry

@[simp] theorem abs_cast_nat {K : Type u_1} [is_R_or_C K] (n : ℕ) : abs ↑n = ↑n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (abs ↑n = ↑n)) (Eq.symm (of_real_nat_cast n))))
    (eq.mpr (id (Eq._oldrec (Eq.refl (abs ↑↑n = ↑n)) (abs_of_nonneg (nat.cast_nonneg n))))
      (Eq.refl ↑n))

theorem norm_sq_eq_abs {K : Type u_1} [is_R_or_C K] (x : K) : coe_fn norm_sq x = abs x ^ bit0 1 :=
  sorry

theorem re_eq_abs_of_mul_conj {K : Type u_1} [is_R_or_C K] (x : K) :
    coe_fn re (x * coe_fn conj x) = abs (x * coe_fn conj x) :=
  sorry

theorem abs_sqr_re_add_conj {K : Type u_1} [is_R_or_C K] (x : K) :
    abs (x + coe_fn conj x) ^ bit0 1 = coe_fn re (x + coe_fn conj x) ^ bit0 1 :=
  sorry

theorem abs_sqr_re_add_conj' {K : Type u_1} [is_R_or_C K] (x : K) :
    abs (coe_fn conj x + x) ^ bit0 1 = coe_fn re (coe_fn conj x + x) ^ bit0 1 :=
  sorry

theorem conj_mul_eq_norm_sq_left {K : Type u_1} [is_R_or_C K] (x : K) :
    coe_fn conj x * x = ↑(coe_fn norm_sq x) :=
  sorry
...
@[simp] theorem re_clm_coe {K : Type u_1} [is_R_or_C K] : ↑re_clm = re_lm := rfl

@[simp] theorem re_clm_apply {K : Type u_1} [is_R_or_C K] : ⇑re_clm = ⇑re := rfl

/-! ### Cauchy sequences -/

theorem is_cau_seq_re {K : Type u_1} [is_R_or_C K] (f : cau_seq K abs) :
    is_cau_seq abs fun (n : ℕ) => coe_fn re (coe_fn f n) :=
  sorry

theorem is_cau_seq_im {K : Type u_1} [is_R_or_C K] (f : cau_seq K abs) :
    is_cau_seq abs fun (n : ℕ) => coe_fn im (coe_fn f n) :=
  sorry
...
theorem is_cau_seq_abs {K : Type u_1} [is_R_or_C K] {f : ℕ → K} (hf : is_cau_seq abs f) :
    is_cau_seq abs (abs ∘ f) :=
  sorry

@[simp] theorem of_real_prod {K : Type u_1} [is_R_or_C K] {α : Type u_2} (s : finset α)
    (f : α → ℝ) : ↑(finset.prod s fun (i : α) => f i) = finset.prod s fun (i : α) => ↑(f i) :=
  ring_hom.map_prod (algebra_map ℝ K) (fun (i : α) => f i) s

@[simp] theorem of_real_sum {K : Type u_1} [is_R_or_C K] {α : Type u_2} (s : finset α) (f : α → ℝ) :
    ↑(finset.sum s fun (i : α) => f i) = finset.sum s fun (i : α) => ↑(f i) :=
  ring_hom.map_sum (algebra_map ℝ K) (fun (i : α) => f i) s

@[simp] theorem of_real_finsupp_sum {K : Type u_1} [is_R_or_C K] {α : Type u_2} {M : Type u_3}
    [HasZero M] (f : α →₀ M) (g : α → M → ℝ) :
    ↑(finsupp.sum f fun (a : α) (b : M) => g a b) = finsupp.sum f fun (a : α) (b : M) => ↑(g a b) :=
  ring_hom.map_finsupp_sum (algebra_map ℝ K) f g

@[simp] theorem of_real_finsupp_prod {K : Type u_1} [is_R_or_C K] {α : Type u_2} {M : Type u_3}
    [HasZero M] (f : α →₀ M) (g : α → M → ℝ) :
    ↑(finsupp.prod f fun (a : α) (b : M) => g a b) =
        finsupp.prod f fun (a : α) (b : M) => ↑(g a b) :=
...
an instance. -/
protected instance proper_is_R_or_C {K : Type u_1} [is_R_or_C K] {E : Type u_2} [normed_group E]
    [normed_space K E] [finite_dimensional K E] : proper_space E :=
...
protected instance real.is_R_or_C : is_R_or_C ℝ :=
  is_R_or_C.mk (add_monoid_hom.id ℝ) 0 (ring_hom.id ℝ) 0 sorry sorry sorry sorry sorry sorry sorry
    sorry sorry sorry sorry sorry sorry sorry

namespace is_R_or_C


@[simp] theorem re_to_real {x : ℝ} : coe_fn re x = x := rfl

@[simp] theorem im_to_real {x : ℝ} : coe_fn im x = 0 := rfl

@[simp] theorem conj_to_real {x : ℝ} : coe_fn conj x = x := rfl

@[simp] theorem I_to_real : I = 0 := rfl

@[simp] theorem norm_sq_to_real {x : ℝ} : coe_fn norm_sq x = x * x := sorry

@[simp] theorem abs_to_real {x : ℝ} : abs x = abs x := sorry

@[simp] theorem coe_real_eq_id : coe = id := rfl
...
@[simp] theorem norm_smul_inv_norm {K : Type u_1} [is_R_or_C K] {E : Type u_2} [normed_group E]
    [normed_space K E] {x : E} (hx : x ≠ 0) : norm (↑(norm x)⁻¹ • x) = 1 :=
  sorry
