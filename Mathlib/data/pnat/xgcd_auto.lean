...
protected instance has_sizeof : SizeOf xgcd_type := { sizeOf := fun (u : xgcd_type) => bp u }
...
def mk' (w : ℕ+) (x : ℕ) (y : ℕ) (z : ℕ+) (a : ℕ+) (b : ℕ+) : xgcd_type :=
  mk (Nat.pred (subtype.val w)) x y (Nat.pred (subtype.val z)) (Nat.pred (subtype.val a))
    (Nat.pred (subtype.val b))

def w (u : xgcd_type) : ℕ+ := nat.succ_pnat (wp u)

def z (u : xgcd_type) : ℕ+ := nat.succ_pnat (zp u)

def a (u : xgcd_type) : ℕ+ := nat.succ_pnat (ap u)

def b (u : xgcd_type) : ℕ+ := nat.succ_pnat (bp u)

def r (u : xgcd_type) : ℕ := (ap u + 1) % (bp u + 1)

def q (u : xgcd_type) : ℕ := (ap u + 1) / (bp u + 1)

def qp (u : xgcd_type) : ℕ := q u - 1
...
def v (u : xgcd_type) : ℕ × ℕ := (↑(w u) * ↑(a u) + x u * ↑(b u), y u * ↑(a u) + ↑(z u) * ↑(b u))

def succ₂ (t : ℕ × ℕ) : ℕ × ℕ := (Nat.succ (prod.fst t), Nat.succ (prod.snd t))

theorem v_eq_succ_vp (u : xgcd_type) : v u = succ₂ (vp u) := sorry

/-- is_special holds if the matrix has determinant one. -/
def is_special (u : xgcd_type) := wp u + zp u + wp u * zp u = x u * y u

def is_special' (u : xgcd_type) := w u * z u = nat.succ_pnat (x u * y u)
...
/-- is_reduced holds if the two entries in the vector are the
 ...
 system. -/
def is_reduced (u : xgcd_type) := ap u = bp u

def is_reduced' (u : xgcd_type) := a u = b u

theorem is_reduced_iff (u : xgcd_type) : is_reduced u ↔ is_reduced' u :=
  { mp := congr_arg nat.succ_pnat, mpr := nat.succ_pnat_inj }

def flip (u : xgcd_type) : xgcd_type := mk (zp u) (y u) (x u) (wp u) (bp u) (ap u)

@[simp] theorem flip_w (u : xgcd_type) : w (flip u) = z u := rfl

@[simp] theorem flip_x (u : xgcd_type) : x (flip u) = y u := rfl

@[simp] theorem flip_y (u : xgcd_type) : y (flip u) = x u := rfl

@[simp] theorem flip_z (u : xgcd_type) : z (flip u) = w u := rfl

@[simp] theorem flip_a (u : xgcd_type) : a (flip u) = b u := rfl

@[simp] theorem flip_b (u : xgcd_type) : b (flip u) = a u := rfl
...
-/
def start (a : ℕ+) (b : ℕ+) : xgcd_type := mk 0 0 0 0 (↑a - 1) (↑b - 1)

theorem start_is_special (a : ℕ+) (b : ℕ+) : is_special (start a b) := id (Eq.refl (0 + 0 + 0 * 0))
...
theorem finish_is_reduced (u : xgcd_type) : is_reduced (finish u) := id (Eq.refl (ap (finish u)))
...
/-- We can now define the full reduction function, which applies
 ...
 given below. -/
def reduce : xgcd_type → xgcd_type := sorry
...
def xgcd (a : ℕ+) (b : ℕ+) : xgcd_type := xgcd_type.reduce (xgcd_type.start a b)

def gcd_d (a : ℕ+) (b : ℕ+) : ℕ+ := xgcd_type.a (xgcd a b)

def gcd_w (a : ℕ+) (b : ℕ+) : ℕ+ := xgcd_type.w (xgcd a b)

def gcd_x (a : ℕ+) (b : ℕ+) : ℕ := xgcd_type.x (xgcd a b)

def gcd_y (a : ℕ+) (b : ℕ+) : ℕ := xgcd_type.y (xgcd a b)

def gcd_z (a : ℕ+) (b : ℕ+) : ℕ+ := xgcd_type.z (xgcd a b)
        ...
        a = a' * d ∧
          b = b' * d ∧
            z * a' = nat.succ_pnat (x * ↑b') ∧
              w * b' = nat.succ_pnat (y * ↑a') ∧ ↑z * ↑a = x * ↑b + ↑d ∧ ↑w * ↑b = y * ↑a + ↑d :=
  sorry

theorem gcd_eq (a : ℕ+) (b : ℕ+) : gcd_d a b = gcd a b := sorry

theorem gcd_det_eq (a : ℕ+) (b : ℕ+) :
    gcd_w a b * gcd_z a b = nat.succ_pnat (gcd_x a b * gcd_y a b) :=
...
theorem gcd_rel_left' (a : ℕ+) (b : ℕ+) :
    gcd_z a b * gcd_a' a b = nat.succ_pnat (gcd_x a b * ↑(gcd_b' a b)) :=
  and.left (and.right (and.right (and.right (gcd_props a b))))

theorem gcd_rel_right' (a : ℕ+) (b : ℕ+) :
    gcd_w a b * gcd_b' a b = nat.succ_pnat (gcd_y a b * ↑(gcd_a' a b)) :=
