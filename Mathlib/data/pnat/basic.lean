...
protected instance coe_pnat_nat : has_coe ℕ+ ℕ :=
  has_coe.mk subtype.val

protected instance pnat.has_repr : has_repr ℕ+ :=
  has_repr.mk fun (n : ℕ+) => repr (subtype.val n)

/-- Predecessor of a `ℕ+`, as a `ℕ`. -/
def pnat.nat_pred (i : ℕ+) : ℕ :=
  ↑i - 1
...
def to_pnat (n : ℕ) (h : autoParam (0 < n)
  (Lean.Syntax.ident Lean.SourceInfo.none (String.toSubstring "Mathlib.tactic.exact_dec_trivial")
    (Lean.Name.mkStr (Lean.Name.mkStr (Lean.Name.mkStr Lean.Name.anonymous "Mathlib") "tactic") "exact_dec_trivial") [])) : ℕ+ :=
  { val := n, property := h }

/-- Write a successor as an element of `ℕ+`. -/
def succ_pnat (n : ℕ) : ℕ+ :=
  { val := Nat.succ n, property := succ_pos n }

@[simp] theorem succ_pnat_coe (n : ℕ) : ↑(succ_pnat n) = Nat.succ n :=
  rfl
...
protected instance decidable_eq : DecidableEq ℕ+ :=
  fun (a b : ℕ+) => subtype.decidable_eq a b

protected instance linear_order : linear_order ℕ+ :=
  subtype.linear_order fun (n : ℕ) => 0 < n

@[simp] theorem mk_le_mk (n : ℕ) (k : ℕ) (hn : 0 < n) (hk : 0 < k) : { val := n, property := hn } ≤ { val := k, property := hk } ↔ n ≤ k :=
  iff.rfl

@[simp] theorem mk_lt_mk (n : ℕ) (k : ℕ) (hn : 0 < n) (hk : 0 < k) : { val := n, property := hn } < { val := k, property := hk } ↔ n < k :=
  iff.rfl

@[simp] theorem coe_le_coe (n : ℕ+) (k : ℕ+) : ↑n ≤ ↑k ↔ n ≤ k :=
  iff.rfl

@[simp] theorem coe_lt_coe (n : ℕ+) (k : ℕ+) : ↑n < ↑k ↔ n < k :=
  iff.rfl

@[simp] theorem pos (n : ℕ+) : 0 < ↑n :=
  subtype.property n

theorem eq {m : ℕ+} {n : ℕ+} : ↑m = ↑n → m = n :=
  subtype.eq

@[simp] theorem coe_inj {m : ℕ+} {n : ℕ+} : ↑m = ↑n ↔ m = n :=
  set_coe.ext_iff

@[simp] theorem mk_coe (n : ℕ) (h : 0 < n) : ↑{ val := n, property := h } = n :=
  rfl
...
@[simp] theorem add_coe (m : ℕ+) (n : ℕ+) : ↑(m + n) = ↑m + ↑n :=
  rfl

protected instance coe_add_hom : is_add_hom coe :=
  is_add_hom.mk add_coe
...
@[simp] theorem ne_zero (n : ℕ+) : ↑n ≠ 0 :=
  ne_of_gt (subtype.property n)

theorem to_pnat'_coe {n : ℕ} : 0 < n → ↑(nat.to_pnat' n) = n :=
  nat.succ_pred_eq_of_pos

@[simp] theorem coe_to_pnat' (n : ℕ+) : nat.to_pnat' ↑n = n :=
  eq (to_pnat'_coe (pos n))

protected instance comm_monoid : comm_monoid ℕ+ :=
  comm_monoid.mk (fun (m n : ℕ+) => { val := subtype.val m * subtype.val n, property := sorry }) sorry (nat.succ_pnat 0)
    sorry sorry sorry
...
@[simp] theorem mk_one {h : 0 < 1} : { val := 1, property := h } = 1 :=
  rfl

@[simp] theorem mk_bit0 (n : ℕ) {h : 0 < bit0 n} : { val := bit0 n, property := h } = bit0 { val := n, property := nat.pos_of_bit0_pos h } :=
  rfl

@[simp] theorem mk_bit1 (n : ℕ) {h : 0 < bit1 n} {k : 0 < n} : { val := bit1 n, property := h } = bit1 { val := n, property := k } :=
  rfl
...
@[simp] theorem bit0_le_bit0 (n : ℕ+) (m : ℕ+) : bit0 n ≤ bit0 m ↔ bit0 ↑n ≤ bit0 ↑m :=
  iff.rfl

@[simp] theorem bit0_le_bit1 (n : ℕ+) (m : ℕ+) : bit0 n ≤ bit1 m ↔ bit0 ↑n ≤ bit1 ↑m :=
  iff.rfl

@[simp] theorem bit1_le_bit0 (n : ℕ+) (m : ℕ+) : bit1 n ≤ bit0 m ↔ bit1 ↑n ≤ bit0 ↑m :=
  iff.rfl

@[simp] theorem bit1_le_bit1 (n : ℕ+) (m : ℕ+) : bit1 n ≤ bit1 m ↔ bit1 ↑n ≤ bit1 ↑m :=
  iff.rfl

@[simp] theorem one_coe : ↑1 = 1 :=
  rfl

@[simp] theorem mul_coe (m : ℕ+) (n : ℕ+) : ↑(m * n) = ↑m * ↑n :=
  rfl

protected instance coe_mul_hom : is_monoid_hom coe :=
  is_monoid_hom.mk one_coe

@[simp] theorem coe_eq_one_iff {m : ℕ+} : ↑m = 1 ↔ m = 1 := sorry

@[simp] theorem coe_bit0 (a : ℕ+) : ↑(bit0 a) = bit0 ↑a :=
  rfl

@[simp] theorem coe_bit1 (a : ℕ+) : ↑(bit1 a) = bit1 ↑a :=
  rfl
...
protected instance ordered_cancel_comm_monoid : ordered_cancel_comm_monoid ℕ+ :=
  ordered_cancel_comm_monoid.mk left_cancel_semigroup.mul left_cancel_semigroup.mul_assoc
    left_cancel_semigroup.mul_left_cancel comm_monoid.one comm_monoid.one_mul comm_monoid.mul_one comm_monoid.mul_comm
    right_cancel_semigroup.mul_right_cancel linear_order.le linear_order.lt linear_order.le_refl linear_order.le_trans
    linear_order.le_antisymm sorry sorry
...
protected instance has_sub : Sub ℕ+ :=
  { sub := fun (a b : ℕ+) => nat.to_pnat' (↑a - ↑b) }

theorem sub_coe (a : ℕ+) (b : ℕ+) : ↑(a - b) = ite (b < a) (↑a - ↑b) 1 := sorry

theorem add_sub_of_lt {a : ℕ+} {b : ℕ+} : a < b → a + (b - a) = b := sorry

protected instance has_well_founded : has_well_founded ℕ+ :=
  has_well_founded.mk Less sorry

/-- Strong induction on `pnat`. -/
theorem strong_induction_on {p : ℕ+ → Prop} (n : ℕ+) (h : ∀ (k : ℕ+), (∀ (m : ℕ+), m < k → p m) → p k) : p n := sorry
...
theorem case_strong_induction_on {p : ℕ+ → Prop} (a : ℕ+) (hz : p 1) (hi : ∀ (n : ℕ+), (∀ (m : ℕ+), m ≤ n → p m) → p (n + 1)) : p a := sorry
...
theorem mod_div_aux_spec (k : ℕ+) (r : ℕ) (q : ℕ) (h : ¬(r = 0 ∧ q = 0)) : ↑(prod.fst (mod_div_aux k r q)) + ↑k * prod.snd (mod_div_aux k r q) = r + ↑k * q := sorry
...
theorem div_coe (m : ℕ+) (k : ℕ+) : div m k = ite (↑m % ↑k = 0) (Nat.pred (↑m / ↑k)) (↑m / ↑k) := sorry
...
theorem le_of_dvd {m : ℕ+} {n : ℕ+} : m ∣ n → m ≤ n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (m ∣ n → m ≤ n)) (propext dvd_iff')))
    fun (h : mod n m = m) => eq.mpr (id (Eq._oldrec (Eq.refl (m ≤ n)) (Eq.symm h))) (and.left (mod_le n m))

/-- If `h : k | m`, then `k * (div_exact m k) = m`. Note that this is not equal to `m / k`. -/
def div_exact (m : ℕ+) (k : ℕ+) : ℕ+ :=
  { val := Nat.succ (div m k), property := sorry }
...
theorem dvd_one_iff (n : ℕ+) : n ∣ 1 ↔ n = 1 :=
  { mp := fun (h : n ∣ 1) => dvd_antisymm h (one_dvd n), mpr := fun (h : n = 1) => Eq.symm h ▸ dvd_refl 1 }

theorem pos_of_div_pos {n : ℕ+} {a : ℕ} (h : a ∣ ↑n) : 0 < a :=
  iff.mpr pos_iff_ne_zero
    (id fun (hzero : a = 0) => ne_zero n (eq_zero_of_zero_dvd (eq.mp (Eq._oldrec (Eq.refl (a ∣ ↑n)) hzero) h)))
