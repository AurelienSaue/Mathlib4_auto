...
inductive ordnode (α : Type u) where
| nil : ordnode α
...
protected instance has_emptyc {α : Type u} : has_emptyc (ordnode α) := has_emptyc.mk nil

protected instance inhabited {α : Type u} : Inhabited (ordnode α) := { default := nil }
...
/-- O(1). Construct a singleton set containing value `a`.
     ...
     singleton 3 = {3} -/
protected def singleton {α : Type u} (a : α) : ordnode α := node 1 nil a nil
...
/-- O(1). Get the size of the set.
     ...
     size {2, 1, 1, 4} = 3  -/
@[simp] def size {α : Type u} : ordnode α → ℕ := sorry

/-- O(1). Is the set empty?

     empty ∅ = tt
     empty {1, 2, 3} = ff -/
def empty {α : Type u} : ordnode α → Bool := sorry
...
@[simp] def dual {α : Type u} : ordnode α → ordnode α := sorry
...
def repr {α : Type u_1} [has_repr α] : ordnode α → string := sorry

protected instance has_repr {α : Type u_1} [has_repr α] : has_repr (ordnode α) := has_repr.mk repr
...
def all {α : Type u} (P : α → Prop) : ordnode α → Prop := sorry

protected instance all.decidable {α : Type u} {P : α → Prop} [decidable_pred P] (t : ordnode α) :
    Decidable (all P t) :=
...
def any {α : Type u} (P : α → Prop) : ordnode α → Prop := sorry

protected instance any.decidable {α : Type u} {P : α → Prop} [decidable_pred P] (t : ordnode α) :
    Decidable (any P t) :=
...
of the tree.
    ...
    emem 2 {1, 2, 3} = true
    emem 4 {1, 2, 3} = false -/
def emem {α : Type u} (x : α) : ordnode α → Prop := any (Eq x)

protected instance emem.decidable {α : Type u} [DecidableEq α] (x : α) (t : ordnode α) :
    Decidable (emem x t) :=
  any.decidable
...
def amem {α : Type u} [HasLessEq α] (x : α) : ordnode α → Prop := any fun (y : α) => x ≤ y ∧ y ≤ x

protected instance amem.decidable {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α)
...
def merge {α : Type u} (l : ordnode α) : ordnode α → ordnode α :=
  ordnode.rec_on l (fun (r : ordnode α) => r)
    fun (ls : ℕ) (ll : ordnode α) (lx : α) (lr : ordnode α) (IHll IHlr : ordnode α → ordnode α)
      (r : ordnode α) =>
...
def insert_max {α : Type u} : ordnode α → α → ordnode α := sorry
...
def insert_min {α : Type u} (x : α) : ordnode α → ordnode α := sorry
  ...
  ordnode.rec_on l (insert_min x)
    fun (ls : ℕ) (ll : ordnode α) (lx : α) (lr : ordnode α) (IHll IHlr : ordnode α → ordnode α)
      ...
      ordnode.rec_on r (insert_max l x)
        fun (rs : ℕ) (rl : ordnode α) (rx : α) (rr IHrl IHrr : ordnode α) =>
          ite (delta * ls < rs) (balance_l IHrl rx rr)
            (ite (delta * rs < ls) (balance_r ll lx (IHlr r)) (node' l x r))

/-- O(n). Filter the elements of a tree satisfying a predicate.

     filter (λ x, x < 3) {1, 2, 4} = {1, 2}
     filter (λ x, x > 5) {1, 2, 4} = ∅ -/
def filter {α : Type u} (p : α → Prop) [decidable_pred p] : ordnode α → ordnode α := sorry
...
def map {α : Type u} {β : Type u_1} (f : α → β) : ordnode α → ordnode β := sorry
...
def powerset {α : Type u} (t : ordnode α) : ordnode (ordnode α) :=
  insert_min nil
    (foldr
      (fun (x : α) (ts : ordnode (ordnode α)) =>
        glue (insert_min (ordnode.singleton x) (map (insert_min x) ts)) ts)
      t nil)

/-- O(m*n). The cartesian product of two sets: `(a, b) ∈ s.prod t` iff `a ∈ s` and `b ∈ t`.

     prod {1, 2} {2, 3} = {(1, 2), (1, 3), (2, 2), (2, 3)} -/
protected def prod {α : Type u} {β : Type u_1} (t₁ : ordnode α) (t₂ : ordnode β) :
    ordnode (α × β) :=
  fold nil
    (fun (s₁ : ordnode (α × β)) (a : α) (s₂ : ordnode (α × β)) =>
      merge s₁ (merge (map (Prod.mk a) t₂) s₂))
    t₁
...
protected def copair {α : Type u} {β : Type u_1} (t₁ : ordnode α) (t₂ : ordnode β) :
    ordnode (α ⊕ β) :=
...
def pmap {α : Type u} {P : α → Prop} {β : Type u_1} (f : (a : α) → P a → β) (t : ordnode α) :
    all P t → ordnode β :=
  sorry
...
def attach' {α : Type u} {P : α → Prop} (t : ordnode α) :
    all P t → ordnode (Subtype fun (a : α) => P a) :=
  pmap Subtype.mk
...
/-- O(log n). Get the `i`th element of the set, by its index from left to right.
     ...
     nth {a, b, c, d} 5 = none -/
def nth {α : Type u} : ordnode α → ℕ → Option α := sorry
...
def remove_nth {α : Type u} : ordnode α → ℕ → ordnode α := sorry
...
def take_aux {α : Type u} : ordnode α → ℕ → ordnode α := sorry

/-- O(log n). Get the first `i` elements of the set, counted from the left.

     take 2 {a, b, c, d} = {a, b}
     take 5 {a, b, c, d} = {a, b, c, d} -/
def take {α : Type u} (i : ℕ) (t : ordnode α) : ordnode α := ite (size t ≤ i) t (take_aux t i)
...
index is within the range of the data structure.)
    ...
    drop_aux {a, b, c, d} 5 = ∅ -/
def drop_aux {α : Type u} : ordnode α → ℕ → ordnode α := sorry

/-- O(log n). Remove the first `i` elements of the set, counted from the left.

     drop 2 {a, b, c, d} = {c, d}
     drop 5 {a, b, c, d} = ∅ -/
def drop {α : Type u} (i : ℕ) (t : ordnode α) : ordnode α := ite (size t ≤ i) nil (drop_aux t i)
...
def split_at_aux {α : Type u} : ordnode α → ℕ → ordnode α × ordnode α := sorry
...
def take_while {α : Type u} (p : α → Prop) [decidable_pred p] : ordnode α → ordnode α := sorry
...
def drop_while {α : Type u} (p : α → Prop) [decidable_pred p] : ordnode α → ordnode α := sorry
...
def span {α : Type u} (p : α → Prop) [decidable_pred p] : ordnode α → ordnode α × ordnode α := sorry
...
def of_asc_list_aux₁ {α : Type u} (l : List α) :
    ℕ → ordnode α × Subtype fun (l' : List α) => list.length l' ≤ list.length l :=
  sorry

/-- Auxiliary definition for `of_asc_list`. -/
def of_asc_list_aux₂ {α : Type u} : List α → ordnode α → ℕ → ordnode α := sorry
...
def of_asc_list {α : Type u} : List α → ordnode α := sorry
...
Using a preorder on `ℕ × ℕ` that only compares the first coordinate:
    ...
    (1, 1) ∈ {(0, 1), (1, 2)} = true
    (3, 1) ∈ {(0, 1), (1, 2)} = false -/
def mem {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Bool := sorry
...
def find {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Option α := sorry

protected instance has_mem {α : Type u} [HasLessEq α] [DecidableRel LessEq] :
    has_mem α (ordnode α) :=
  has_mem.mk fun (x : α) (t : ordnode α) => ↥(mem x t)

protected instance mem.decidable {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α)
    (t : ordnode α) : Decidable (x ∈ t) :=
...
def insert_with {α : Type u} [HasLessEq α] [DecidableRel LessEq] (f : α → α) (x : α) :
    ordnode α → ordnode α :=
  sorry
...
def adjust_with {α : Type u} [HasLessEq α] [DecidableRel LessEq] (f : α → α) (x : α) :
    ordnode α → ordnode α :=
  sorry
...
Note that the element returned by `f` must be equivalent to `x`.
    ...
    update_with f 1 {1, 2, 3} = {2, 3}     if f 1 = none
                              = {a, 2, 3}  if f 1 = some a -/
def update_with {α : Type u} [HasLessEq α] [DecidableRel LessEq] (f : α → Option α) (x : α) :
    ordnode α → ordnode α :=
  sorry
...
Note that the element returned by `f` must be equivalent to `x`.
    ...
    alter f 1 {1, 2, 3} = {2, 3}     if f 1 = none
                        = {a, 2, 3}  if f 1 = some a -/
def alter {α : Type u} [HasLessEq α] [DecidableRel LessEq] (f : Option α → Option α) (x : α) :
    ordnode α → ordnode α :=
  sorry
...
protected def insert {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) :
    ordnode α → ordnode α :=
  sorry

protected instance has_insert {α : Type u} [HasLessEq α] [DecidableRel LessEq] :
...
def split {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) :
...
def split3 {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) :
...
def erase {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → ordnode α := sorry
...
def find_lt {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Option α := sorry
...
def find_gt {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Option α := sorry
...
def find_le {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Option α := sorry
...
def find_ge {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) : ordnode α → Option α := sorry

/-- Auxiliary definition for `find_index`. -/
def find_index_aux {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) :
...
def find_index {α : Type u} [HasLessEq α] [DecidableRel LessEq] (x : α) (t : ordnode α) :
    Option ℕ :=
...
def is_subset {α : Type u} [HasLessEq α] [DecidableRel LessEq] (t₁ : ordnode α) (t₂ : ordnode α) :
    Bool :=
...
/-- O(n * log n). Adaptively chooses between the linear and log-linear algorithm depending
  on whether the input list is already sorted.
    ...
    of_list' [1, 2, 3] = {1, 2, 3}
    of_list' [2, 1, 1, 3] = {1, 2, 3} -/
def of_list' {α : Type u} [HasLessEq α] [DecidableRel LessEq] : List α → ordnode α := sorry
...
def image {α : Type u_1} {β : Type u_2} [HasLessEq β] [DecidableRel LessEq] (f : α → β)
    (t : ordnode α) : ordnode β :=
