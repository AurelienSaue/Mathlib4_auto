...
@[simp] def factorial : ℕ → ℕ := sorry

@[simp] theorem factorial_zero : factorial 0 = factorial 1 := rfl

@[simp] theorem factorial_succ (n : ℕ) : factorial (Nat.succ n) = Nat.succ n * factorial n := rfl

@[simp] theorem factorial_one : factorial 1 = 1 := rfl
...
theorem factorial_ne_zero (n : ℕ) : factorial n ≠ 0 := ne_of_gt (factorial_pos n)
...
theorem factorial_mul_pow_le_factorial {m : ℕ} {n : ℕ} :
    factorial m * Nat.succ m ^ n ≤ factorial (m + n) :=
  sorry

theorem monotone_factorial : monotone factorial := fun (n m : ℕ) => factorial_le
...
theorem factorial_inj {m : ℕ} {n : ℕ} (h0 : 1 < factorial n) : factorial n = factorial m ↔ n = m :=
  sorry
