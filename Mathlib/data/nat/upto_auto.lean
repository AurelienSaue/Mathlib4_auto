...
def upto (p : ℕ → Prop) := Subtype fun (i : ℕ) => ∀ (j : ℕ), j < i → ¬p j

namespace upto


/-- Lift the "greater than" relation on natural numbers to `nat.upto`. -/
protected def gt (p : ℕ → Prop) (x : upto p) (y : upto p) := subtype.val x > subtype.val y
...
def zero {p : ℕ → Prop} : upto p := { val := 0, property := sorry }
