...
protected def mk {n : ℕ} {A : Type u} {α : typevec n} (x : A) : const n A α :=
  x

/-- Destructor for constant functor -/
protected def get {n : ℕ} {A : Type u} {α : typevec n} (x : const n A α) : A :=
  x

@[simp] protected theorem mk_get {n : ℕ} {A : Type u} {α : typevec n} (x : const n A α) : const.mk (const.get x) = x :=
  rfl

@[simp] protected theorem get_mk {n : ℕ} {A : Type u} {α : typevec n} (x : A) : const.get (const.mk x) = x :=
  rfl

/-- `map` for constant functor -/
protected def map {n : ℕ} {A : Type u} {α : typevec n} {β : typevec n} : const n A α → const n A β :=
  fun (x : const n A α) => x
...
theorem map_mk {n : ℕ} {A : Type u} {α : typevec n} {β : typevec n} (f : typevec.arrow α β) (x : A) : mvfunctor.map f (const.mk x) = const.mk x :=
  rfl

theorem get_map {n : ℕ} {A : Type u} {α : typevec n} {β : typevec n} (f : typevec.arrow α β) (x : const n A α) : const.get (mvfunctor.map f x) = const.get x :=
  rfl

protected instance mvqpf {n : ℕ} {A : Type u} : mvqpf (const n A) :=
  mk (mvpfunctor.const n A) (fun (α : typevec n) (x : mvpfunctor.obj (mvpfunctor.const n A) α) => mvpfunctor.const.get x)
