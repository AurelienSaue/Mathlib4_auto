...
def prj {n : ℕ} (i : fin2 n) (v : typevec n) := v i

protected instance prj.inhabited {n : ℕ} (i : fin2 n) {v : typevec n} [Inhabited (v i)] :
    Inhabited (prj i v) :=
  { default := Inhabited.default }

/-- `map` on functor `prj i` -/
def prj.map {n : ℕ} (i : fin2 n) {α : typevec n} {β : typevec n} (f : typevec.arrow α β) :
    prj i α → prj i β :=
  f i
...
def prj.abs {n : ℕ} (i : fin2 n) {α : typevec n} : mvpfunctor.obj (prj.P i) α → prj i α := sorry

/-- Representation function of the `qpf` instance -/
def prj.repr {n : ℕ} (i : fin2 n) {α : typevec n} : prj i α → mvpfunctor.obj (prj.P i) α :=
  fun (x : α i) =>
    sigma.mk PUnit.unit fun (j : fin2 n) (_x : mvpfunctor.B (prj.P i) PUnit.unit j) => sorry
