...
Multivariate quotients of polynomial functors.
-/
class mvqpf {n : ℕ} (F : typevec n → Type u_1) [mvfunctor F] 
where
  P : mvpfunctor n
  abs : {α : typevec n} → mvpfunctor.obj P α → F α
  repr : {α : typevec n} → F α → mvpfunctor.obj P α
  abs_repr : ∀ {α : typevec n} (x : F α), abs (repr x) = x
  abs_map : ∀ {α β : typevec n} (f : typevec.arrow α β) (p : mvpfunctor.obj P α), abs (mvfunctor.map f p) = mvfunctor.map f (abs p)
...
protected theorem id_map {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} (x : F α) : mvfunctor.map typevec.id x = x := sorry

@[simp] theorem comp_map {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} {β : typevec n} {γ : typevec n} (f : typevec.arrow α β) (g : typevec.arrow β γ) (x : F α) : mvfunctor.map (typevec.comp g f) x = mvfunctor.map g (mvfunctor.map f x) := sorry

protected instance is_lawful_mvfunctor {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] : is_lawful_mvfunctor F :=
  is_lawful_mvfunctor.mk mvqpf.id_map comp_map

/- Lifting predicates and relations -/

theorem liftp_iff {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} (p : {i : fin2 n} → α i → Prop) (x : F α) : mvfunctor.liftp p x ↔
  ∃ (a : mvpfunctor.A (P F)),
    ∃ (f : typevec.arrow (mvpfunctor.B (P F) a) α),
      x = abs (sigma.mk a f) ∧ ∀ (i : fin2 n) (j : mvpfunctor.B (P F) a i), p (f i j) := sorry

theorem liftr_iff {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} (r : {i : fin2 n} → α i → α i → Prop) (x : F α) (y : F α) : mvfunctor.liftr r x y ↔
  ∃ (a : mvpfunctor.A (P F)),
    ∃ (f₀ : typevec.arrow (mvpfunctor.B (P F) a) α),
      ∃ (f₁ : typevec.arrow (mvpfunctor.B (P F) a) α),
        x = abs (sigma.mk a f₀) ∧
...
theorem mem_supp {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} (x : F α) (i : fin2 n) (u : α i) : u ∈ mvfunctor.supp x i ↔
  ∀ (a : mvpfunctor.A (P F)) (f : typevec.arrow (mvpfunctor.B (P F) a) α), abs (sigma.mk a f) = x → u ∈ f i '' set.univ := sorry

theorem supp_eq {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} {i : fin2 n} (x : F α) : mvfunctor.supp x i =
  set_of
    fun (u : α i) =>
...
theorem has_good_supp_iff {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] {α : typevec n} (x : F α) : (∀ (p : (i : fin2 n) → α i → Prop), mvfunctor.liftp p x ↔ ∀ (i : fin2 n) (u : α i), u ∈ mvfunctor.supp x i → p i u) ↔
  ∃ (a : mvpfunctor.A (P F)),
    ∃ (f : typevec.arrow (mvpfunctor.B (P F) a) α),
      abs (sigma.mk a f) = x ∧
        ∀ (i : fin2 n) (a' : mvpfunctor.A (P F)) (f' : typevec.arrow (mvpfunctor.B (P F) a') α),
...
theorem liftp_iff_of_is_uniform {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] (h : is_uniform q) {α : typevec n} (x : F α) (p : (i : fin2 n) → α i → Prop) : mvfunctor.liftp p x ↔ ∀ (i : fin2 n) (u : α i), u ∈ mvfunctor.supp x i → p i u := sorry

theorem supp_map {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] (h : is_uniform q) {α : typevec n} {β : typevec n} (g : typevec.arrow α β) (x : F α) (i : fin2 n) : mvfunctor.supp (mvfunctor.map g x) i = g i '' mvfunctor.supp x i := sorry

theorem supp_preservation_iff_uniform {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] : supp_preservation q ↔ is_uniform q := sorry

theorem supp_preservation_iff_liftp_preservation {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] : supp_preservation q ↔ liftp_preservation q := sorry

theorem liftp_preservation_iff_uniform {n : ℕ} {F : typevec n → Type u_1} [mvfunctor F] [q : mvqpf F] : liftp_preservation q ↔ is_uniform q := sorry
