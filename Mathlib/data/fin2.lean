...
protected def cases' {n : ℕ} {C : fin2 (Nat.succ n) → Sort u} (H1 : C fz) (H2 : (n_1 : fin2 n) → C (fs n_1)) (i : fin2 (Nat.succ n)) : C i :=
  sorry
...
def to_nat {n : ℕ} : fin2 n → ℕ :=
  sorry

/-- convert a `nat` into a `fin2` if it is in range -/
def opt_of_nat {n : ℕ} (k : ℕ) : Option (fin2 n) :=
  sorry

/-- `i + k : fin2 (n + k)` when `i : fin2 n` and `k : ℕ` -/
def add {n : ℕ} (i : fin2 n) (k : ℕ) : fin2 (n + k) :=
  sorry

/-- `left k` is the embedding `fin2 n → fin2 (k + n)` -/
def left (k : ℕ) {n : ℕ} : fin2 n → fin2 (k + n) :=
  sorry
...
def insert_perm {n : ℕ} : fin2 n → fin2 n → fin2 n :=
  sorry
...
def remap_left {m : ℕ} {n : ℕ} (f : fin2 m → fin2 n) (k : ℕ) : fin2 (m + k) → fin2 (n + k) :=
  sorry
...
protected instance is_lt.zero (n : ℕ) : is_lt 0 (Nat.succ n) :=
  is_lt.mk (nat.succ_pos n)
...
def of_nat' {n : ℕ} (m : ℕ) [is_lt m n] : fin2 n :=
  sorry

protected instance inhabited : Inhabited (fin2 1) :=
  { default := fz }
