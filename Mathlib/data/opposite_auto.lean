...
def op {α : Sort u} : α → (αᵒᵖ) := id

def unop {α : Sort u} : αᵒᵖ → α := id

theorem op_injective {α : Sort u} : function.injective op := fun (_x _x_1 : α) => id

theorem unop_injective {α : Sort u} : function.injective unop := fun (_x _x_1 : αᵒᵖ) => id

@[simp] theorem op_inj_iff {α : Sort u} (x : α) (y : α) : op x = op y ↔ x = y := iff.rfl

@[simp] theorem unop_inj_iff {α : Sort u} (x : αᵒᵖ) (y : αᵒᵖ) : unop x = unop y ↔ x = y := iff.rfl

@[simp] theorem op_unop {α : Sort u} (x : αᵒᵖ) : op (unop x) = x := rfl

@[simp] theorem unop_op {α : Sort u} (x : α) : unop (op x) = x := rfl

/-- The type-level equivalence between a type and its opposite. -/
def equiv_to_opposite {α : Sort u} : α ≃ (αᵒᵖ) := equiv.mk op unop unop_op op_unop

@[simp] theorem equiv_to_opposite_apply {α : Sort u} (a : α) : coe_fn equiv_to_opposite a = op a :=
  rfl

@[simp] theorem equiv_to_opposite_symm_apply {α : Sort u} (a : αᵒᵖ) :
    coe_fn (equiv.symm equiv_to_opposite) a = unop a :=
  rfl
