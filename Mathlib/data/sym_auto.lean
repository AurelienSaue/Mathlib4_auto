...
-/
def sym (α : Type u) (n : ℕ) := Subtype fun (s : multiset α) => coe_fn multiset.card s = n
...
-/
def nil {α : Type u} : sym α 0 := { val := 0, property := sorry }

/--
Inserts an element into the term of `sym α n`, increasing the length by one.
-/
def cons {α : Type u} {n : ℕ} : α → sym α n → sym α (Nat.succ n) := sorry

infixr:67 " :: " => Mathlib.sym.cons

@[simp] theorem cons_inj_right {α : Type u} {n : ℕ} (a : α) (s : sym α n) (s' : sym α n) :
    a :: s = a :: s' ↔ s = s' :=
  sorry

@[simp] theorem cons_inj_left {α : Type u} {n : ℕ} (a : α) (a' : α) (s : sym α n) :
    a :: s = a' :: s ↔ a = a' :=
  sorry

theorem cons_swap {α : Type u} {n : ℕ} (a : α) (b : α) (s : sym α n) : a :: b :: s = b :: a :: s :=
  sorry

/--
`α ∈ s` means that `a` appears as one of the factors in `s`.
-/
def mem {α : Type u} {n : ℕ} (a : α) (s : sym α n) := a ∈ subtype.val s

protected instance has_mem {α : Type u} {n : ℕ} : has_mem α (sym α n) := has_mem.mk mem

protected instance decidable_mem {α : Type u} {n : ℕ} [DecidableEq α] (a : α) (s : sym α n) :
    Decidable (a ∈ s) :=
  subtype.cases_on s
    fun (s_val : multiset α) (s_property : coe_fn multiset.card s_val = n) =>
      id (multiset.decidable_mem a s_val)

@[simp] theorem mem_cons {α : Type u} {n : ℕ} {a : α} {b : α} {s : sym α n} :
    a ∈ b :: s ↔ a = b ∨ a ∈ s :=
  sorry

theorem mem_cons_of_mem {α : Type u} {n : ℕ} {a : α} {b : α} {s : sym α n} (h : a ∈ s) :
    a ∈ b :: s :=
...
theorem sound {α : Type u} {n : ℕ} {a : vector α n} {b : vector α n}
    (h : subtype.val a ~ subtype.val b) : ↑a = ↑b :=
  sorry
...
theorem cons_equiv_eq_equiv_cons (α : Type u) (n : ℕ) (a : α) (s : sym α n) :
    a :: coe_fn sym_equiv_sym' s = coe_fn sym_equiv_sym' (a :: s) :=
  sorry
