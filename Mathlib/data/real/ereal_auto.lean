...
protected instance has_coe : has_coe ℝ ereal := has_coe.mk (some ∘ some)
...
protected instance has_zero : HasZero ereal := { zero := ↑0 }

protected instance inhabited : Inhabited ereal := { default := 0 }

/-! ### Negation -/

/-- negation on ereal -/
protected def neg : ereal → ereal := sorry

protected instance has_neg : Neg ereal := { neg := ereal.neg }

protected theorem neg_def (x : ℝ) : ↑(-x) = -↑x := rfl
...
theorem neg_eq_iff_neg_eq {a : ereal} {b : ereal} : -a = b ↔ -b = a :=
  { mp :=
      fun (h : -a = b) => eq.mpr (id (Eq._oldrec (Eq.refl (-b = a)) (Eq.symm h))) (ereal.neg_neg a),
    mpr :=
      fun (h : -b = a) =>
        eq.mpr (id (Eq._oldrec (Eq.refl (-a = b)) (Eq.symm h))) (ereal.neg_neg b) }
