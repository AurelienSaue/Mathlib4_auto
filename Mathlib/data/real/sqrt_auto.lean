...
@[simp] theorem sqrt_zero : coe_fn sqrt 0 = 0 := iff.mpr sqrt_eq_zero rfl

@[simp] theorem sqrt_one : coe_fn sqrt 1 = 1 := iff.mpr sqrt_eq_iff_sqr_eq (mul_one 1)
...
theorem sqrt_mul (x : nnreal) (y : nnreal) : coe_fn sqrt (x * y) = coe_fn sqrt x * coe_fn sqrt y :=
  sorry
...
theorem continuous_sqrt : continuous ⇑sqrt := order_iso.continuous sqrt
...
def sqrt_aux (f : cau_seq ℚ abs) : ℕ → ℚ := sorry
...
def sqrt (x : ℝ) : ℝ := ↑(coe_fn nnreal.sqrt (nnreal.of_real x))
...
theorem continuous_sqrt : continuous sqrt :=
  continuous.comp nnreal.continuous_coe
    (continuous.comp (order_iso.continuous nnreal.sqrt) nnreal.continuous_of_real)
...
theorem sqrt_eq_iff_mul_self_eq {x : ℝ} {y : ℝ} (hx : 0 ≤ x) (hy : 0 ≤ y) :
    sqrt x = y ↔ y * y = x :=
  sorry
...
@[simp] theorem sqrt_inj {x : ℝ} {y : ℝ} (hx : 0 ≤ x) (hy : 0 ≤ y) : sqrt x = sqrt y ↔ x = y :=
  sorry
...
theorem filter.tendsto.sqrt {α : Type u_1} {f : α → ℝ} {l : filter α} {x : ℝ}
...
theorem continuous_within_at.sqrt {α : Type u_1} [topological_space α] {f : α → ℝ} {s : set α}
    {x : α} (h : continuous_within_at f s x) :
    continuous_within_at (fun (x : α) => real.sqrt (f x)) s x :=
  filter.tendsto.sqrt h

theorem continuous_at.sqrt {α : Type u_1} [topological_space α] {f : α → ℝ} {x : α}
    (h : continuous_at f x) : continuous_at (fun (x : α) => real.sqrt (f x)) x :=
  filter.tendsto.sqrt h

theorem continuous_on.sqrt {α : Type u_1} [topological_space α] {f : α → ℝ} {s : set α}
    (h : continuous_on f s) : continuous_on (fun (x : α) => real.sqrt (f x)) s :=
