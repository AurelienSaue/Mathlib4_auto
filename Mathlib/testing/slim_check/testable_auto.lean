...
-/
inductive test_result (p : Prop) where
...
protected def test_result.to_string {p : Prop} : test_result p → string := sorry
...
protected instance default_printable_prop {p : Prop} : printable_prop p := printable_prop.mk none
...
def combine {p : Prop} {q : Prop} : psum Unit (p → q) → psum Unit p → psum Unit q := sorry

/-- Combine the test result for properties `p` and `q` to create a test for their conjunction. -/
def and_counter_example {p : Prop} {q : Prop} :
    test_result p → test_result q → test_result (p ∧ q) :=
  sorry

/-- Combine the test result for properties `p` and `q` to create a test for their disjunction -/
def or_counter_example {p : Prop} {q : Prop} :
    test_result p → test_result q → test_result (p ∨ q) :=
  sorry

/-- If `q → p`, then `¬ p → ¬ q` which means that testing `p` can allow us
to find counter-examples to `q`. -/
def convert_counter_example {p : Prop} {q : Prop} (h : q → p) :
    test_result p → optParam (psum Unit (p → q)) (psum.inl Unit.unit) → test_result q :=
  sorry

/-- Test `q` by testing `p` and proving the equivalence between the two. -/
def convert_counter_example' {p : Prop} {q : Prop} (h : p ↔ q) (r : test_result p) :
    test_result q :=
...
can be informative. -/
def add_to_counter_example (x : string) {p : Prop} {q : Prop} (h : q → p) :
    test_result p → optParam (psum Unit (p → q)) (psum.inl Unit.unit) → test_result q :=
  sorry

/-- Add some formatting to the information recorded by `add_to_counter_example`. -/
def add_var_to_counter_example {γ : Type v} [has_repr γ] (var : string) (x : γ) {p : Prop}
    {q : Prop} (h : q → p) :
    test_result p → optParam (psum Unit (p → q)) (psum.inl Unit.unit) → test_result q :=
...
```
 -/
@[simp] def named_binder (n : string) (p : Prop) := p

/-- Is the given test result a failure? -/
def is_failure {p : Prop} : test_result p → Bool := sorry

protected instance and_testable (p : Prop) (q : Prop) [testable p] [testable q] :
    testable (p ∧ q) :=
  testable.mk
...
protected instance iff_testable (p : Prop) (q : Prop) [testable (p ∧ q ∨ ¬p ∧ ¬q)] :
    testable (p ↔ q) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
      do 
...
protected instance dec_guard_testable (var : string) (p : Prop) [printable_prop p] [Decidable p]
    (β : p → Prop) [(h : p) → testable (β h)] : testable (named_binder var (∀ (h : p), β h)) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
...
def use_has_to_string (α : Type u_1) := α

protected instance use_has_to_string.inhabited (α : Type u) [I : Inhabited α] :
    Inhabited (use_has_to_string α) :=
  I

/-- Add the type tag `use_has_to_string` to an expression's type. -/
def use_has_to_string.mk {α : Type u_1} (x : α) : use_has_to_string α := x

protected instance use_has_to_string.has_repr (α : Type u) [has_to_string α] :
    has_repr (use_has_to_string α) :=
  has_repr.mk to_string

protected instance all_types_testable (var : string) (f : Type → Prop) [testable (f ℤ)] :
    testable (named_binder var (∀ (x : Type), f x)) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
      do 
                  ...
                  (char.of_nat
                    (bit0
                      (bit0
                        (bit1
                          (bit0
                            (bit0 (bit1 (bit0 (bit0 (bit1 (bit0 (bit0 (bit0 (bit0 1))))))))))))))))
              sorry r (psum.inl Unit.unit))

/-- Trace the value of sampled variables if the sample is discarded. -/
def trace_if_giveup {p : Prop} {α : Type u_1} {β : Type u_2} [has_repr α] (tracing_enabled : Bool)
    (var : string) (val : α) : test_result p → thunk β → β :=
  sorry

/-- testable instance for a property iterating over the element of a list -/
protected instance test_forall_in_list (var : string) (var' : string) (α : Type u) (β : α → Prop)
    [(x : α) → testable (β x)] [has_repr α] (xs : List α) :
    testable (named_binder var (∀ (x : α), named_binder var' (x ∈ xs → β x))) :=
  sorry
...
Format the counter-examples found in a test failure.
-/
def format_failure' (s : string) {p : Prop} : test_result p → string := sorry

/--
Increase the number of shrinking steps in a test result.
-/
def add_shrinks {p : Prop} (n : ℕ) : test_result p → test_result p := sorry
...
than `x`. -/
def minimize_aux (α : Type u) (β : α → Prop) [sampleable_ext α] [(x : α) → testable (β x)]
    (cfg : slim_check_cfg) (var : string) :
    sampleable_ext.proxy_repr α →
        ℕ →
          option_t gen
            (sigma
              fun (x : sampleable_ext.proxy_repr α) =>
                test_result (β (sampleable_ext.interp α x))) :=
  sorry

/-- Once a property fails to hold on an example, look for smaller counter-examples
to show the user. -/
def minimize (α : Type u) (β : α → Prop) [sampleable_ext α] [(x : α) → testable (β x)]
    (cfg : slim_check_cfg) (var : string) (x : sampleable_ext.proxy_repr α)
    (r : test_result (β (sampleable_ext.interp α x))) :
    gen
        (sigma
          fun (x : sampleable_ext.proxy_repr α) => test_result (β (sampleable_ext.interp α x))) :=
  sorry

protected instance exists_testable (var : string) (var' : string) (α : Type u) (β : α → Prop)
    (p : Prop) [testable (named_binder var (∀ (x : α), named_binder var' (β x → p)))] :
    testable (named_binder var' (named_binder var (∃ (x : α), β x) → p)) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
      do 
...
protected instance var_testable (var : string) (α : Type u) (β : α → Prop) [sampleable_ext α]
    [(x : α) → testable (β x)] : testable (named_binder var (∀ (x : α), β x)) :=
  sorry

/-- Test a universal property about propositions -/
protected instance prop_var_testable (var : string) (β : Prop → Prop)
    [I : (b : Bool) → testable (β ↥b)] : testable (named_binder var (∀ (p : Prop), β p)) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
      (fun (ᾰ : test_result (∀ (b : Bool), β ↥b)) =>
          convert_counter_example sorry ᾰ (psum.inl Unit.unit)) <$>
        testable.run (named_binder var (∀ (b : Bool), β ↥b)) cfg min

protected instance unused_var_testable (var : string) (α : Type u) (β : Prop) [Inhabited α]
    [testable β] : testable (named_binder var (α → β)) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
      do 
...
protected instance subtype_var_testable (var : string) (var' : string) (α : Type u) (β : α → Prop)
    {p : α → Prop} [(x : α) → printable_prop (p x)] [(x : α) → testable (β x)]
    [I : sampleable_ext (Subtype p)] :
    testable (named_binder var (∀ (x : α), named_binder var' (p x → β x))) :=
  testable.mk
    fun (cfg : slim_check_cfg) (min : Bool) =>
...
protected instance eq.printable_prop {α : Type u_1} [has_repr α] (x : α) (y : α) :
    printable_prop (x = y) :=
  printable_prop.mk
    (some
...
protected instance ne.printable_prop {α : Type u_1} [has_repr α] (x : α) (y : α) :
    printable_prop (x ≠ y) :=
  printable_prop.mk
    (some
      (string.empty ++ to_string (repr x) ++
              ...
              (string.str
                ...
                (char.of_nat
                  (bit0
                    (bit0
                      (bit0
                        (bit0 (bit0 (bit1 (bit1 (bit0 (bit0 (bit1 (bit0 (bit0 (bit0 1)))))))))))))))
              (char.of_nat (bit0 (bit0 (bit0 (bit0 (bit0 1)))))) ++
            to_string (repr y) ++
          string.empty)))

protected instance le.printable_prop {α : Type u_1} [HasLessEq α] [has_repr α] (x : α) (y : α) :
    printable_prop (x ≤ y) :=
  printable_prop.mk
    (some
      (string.empty ++ to_string (repr x) ++
...
protected instance lt.printable_prop {α : Type u_1} [HasLess α] [has_repr α] (x : α) (y : α) :
    printable_prop (x < y) :=
  printable_prop.mk
    (some
      (string.empty ++ to_string (repr x) ++
...
protected instance perm.printable_prop {α : Type u_1} [has_repr α] (xs : List α) (ys : List α) :
    printable_prop (xs ~ ys) :=
  printable_prop.mk
    (some
...
protected instance and.printable_prop (x : Prop) (y : Prop) [printable_prop x] [printable_prop y] :
    printable_prop (x ∧ y) :=
  printable_prop.mk
    (do 
      ...
      some
          (string.str string.empty (char.of_nat (bit0 (bit0 (bit0 (bit1 (bit0 1)))))) ++
              to_string x' ++
            (string.str
                  (string.str
                    (string.str string.empty (char.of_nat (bit0 (bit0 (bit0 (bit0 (bit0 1)))))))
                    (char.of_nat
                      (bit1
                        (bit1
                          (bit1
                            (bit0
                              (bit0 (bit1 (bit0 (bit0 (bit0 (bit1 (bit0 (bit0 (bit0 1)))))))))))))))
                  (char.of_nat (bit0 (bit0 (bit0 (bit0 (bit0 1)))))) ++
                to_string y' ++
              string.str string.empty (char.of_nat (bit1 (bit0 (bit0 (bit1 (bit0 1)))))))))

protected instance or.printable_prop (x : Prop) (y : Prop) [printable_prop x] [printable_prop y] :
    printable_prop (x ∨ y) :=
  printable_prop.mk
    (do 
      let x' ← printable_prop.print_prop x 
...
protected instance iff.printable_prop (x : Prop) (y : Prop) [printable_prop x] [printable_prop y] :
    printable_prop (x ↔ y) :=
  printable_prop.mk
    (do 
      let x' ← printable_prop.print_prop x 
                    ...
                    (char.of_nat
                      (bit0
                        (bit0
                          (bit1
                            (bit0
                              (bit1 (bit0 (bit0 (bit1 (bit1 (bit0 (bit0 (bit0 (bit0 1)))))))))))))))
                  (char.of_nat (bit0 (bit0 (bit0 (bit0 (bit0 1)))))) ++
                to_string y' ++
              string.str string.empty (char.of_nat (bit1 (bit0 (bit0 (bit1 (bit0 1)))))))))

protected instance imp.printable_prop (x : Prop) (y : Prop) [printable_prop x] [printable_prop y] :
    printable_prop (x → y) :=
  printable_prop.mk
    (do 
      let x' ← printable_prop.print_prop x 
  ...
  printable_prop.mk
    (do 
      ...
      some
          (string.str
                (string.str string.empty
                  (char.of_nat (bit0 (bit0 (bit1 (bit1 (bit0 (bit1 (bit0 1)))))))))
                (char.of_nat (bit0 (bit0 (bit0 (bit0 (bit0 1)))))) ++
              to_string x' ++
            string.empty))
...
`n` times). -/
def retry {p : Prop} (cmd : rand (test_result p)) : ℕ → rand (test_result p) := sorry

/-- Count the number of times the test procedure gave up. -/
def give_up {p : Prop} (x : ℕ) : test_result p → test_result p := sorry

/-- Try `n` times to find a counter-example for `p`. -/
def testable.run_suite_aux (p : Prop) [testable p] (cfg : slim_check_cfg) :
    test_result p → ℕ → rand (test_result p) :=
  sorry

/-- Try to find a counter-example of `p`. -/
def testable.run_suite (p : Prop) [testable p]
    (cfg :
      optParam slim_check_cfg
        (slim_check_cfg.mk (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1))))))
          (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1)))))) false false false false none false)) :
    rand (test_result p) :=
  testable.run_suite_aux p cfg (test_result.success (psum.inl Unit.unit))
    (slim_check_cfg.num_inst cfg)

/-- Run a test suite for `p` in `io`. -/
def testable.check' (p : Prop) [testable p]
    (cfg :
      optParam slim_check_cfg
        (slim_check_cfg.mk (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1))))))
          (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1)))))) false false false false none false)) :
...
def testable.check (p : Prop)
    (cfg :
      optParam slim_check_cfg
        (slim_check_cfg.mk (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1))))))
          (bit0 (bit0 (bit1 (bit0 (bit0 (bit1 1)))))) false false false false none false))
    (p' :
      autoParam (tactic.decorations_of p)
        (Lean.Syntax.ident Lean.SourceInfo.none
          (String.toSubstring "Mathlib.slim_check.tactic.mk_decorations")
          (Lean.Name.mkStr
            (Lean.Name.mkStr
