...
structure linear_recurrence (α : Type u_1) [comm_semiring α] where
  order : ℕ
  coeffs : fin order → α

protected instance linear_recurrence.inhabited (α : Type u_1) [comm_semiring α] :
...
def is_solution {α : Type u_1} [comm_semiring α] (E : linear_recurrence α) (u : ℕ → α) :=
  ∀ (n : ℕ),
    u (n + order E) = finset.sum finset.univ fun (i : fin (order E)) => coeffs E i * u (n + ↑i)

/-- A solution of a `linear_recurrence` which satisfies certain initial conditions.
  We will prove this is the only such solution. -/
def mk_sol {α : Type u_1} [comm_semiring α] (E : linear_recurrence α) (init : fin (order E) → α) :
    ℕ → α :=
  sorry

/-- `E.mk_sol` indeed gives solutions to `E`. -/
theorem is_sol_mk_sol {α : Type u_1} [comm_semiring α] (E : linear_recurrence α)
    (init : fin (order E) → α) : is_solution E (mk_sol α _inst_1 E init) :=
  sorry

/-- `E.mk_sol init`'s first `E.order` terms are `init`. -/
theorem mk_sol_eq_init {α : Type u_1} [comm_semiring α] (E : linear_recurrence α)
    (init : fin (order E) → α) (n : fin (order E)) : mk_sol α _inst_1 E init ↑n = init n :=
  sorry

/-- If `u` is a solution to `E` and `init` designates its first `E.order` values,
  then `∀ n, u n = E.mk_sol init n`. -/
theorem eq_mk_of_is_sol_of_eq_init {α : Type u_1} [comm_semiring α] (E : linear_recurrence α)
    {u : ℕ → α} {init : fin (order E) → α} (h : is_solution E u)
    (heq : ∀ (n : fin (order E)), u ↑n = init n) (n : ℕ) : u n = mk_sol α _inst_1 E init n :=
  sorry
...
theorem eq_mk_of_is_sol_of_eq_init' {α : Type u_1} [comm_semiring α] (E : linear_recurrence α)
    {u : ℕ → α} {init : fin (order E) → α} (h : is_solution E u)
    (heq : ∀ (n : fin (order E)), u ↑n = init n) : u = mk_sol α _inst_1 E init :=
...
theorem is_sol_iff_mem_sol_space {α : Type u_1} [comm_semiring α] (E : linear_recurrence α)
    (u : ℕ → α) : is_solution E u ↔ u ∈ sol_space E :=
  iff.rfl

/-- The function that maps a solution `u` of `E` to its first
  `E.order` terms as a `linear_equiv`. -/
def to_init {α : Type u_1} [comm_semiring α] (E : linear_recurrence α) :
    linear_equiv α (↥(sol_space E)) (fin (order E) → α) :=
  linear_equiv.mk (fun (u : ↥(sol_space E)) (x : fin (order E)) => coe u ↑x) sorry sorry
    (fun (u : fin (order E) → α) =>
      { val := mk_sol α _inst_1 E u, property := is_sol_mk_sol α _inst_1 E u })
    sorry sorry

/-- Two solutions are equal iff they are equal on `range E.order`. -/
theorem sol_eq_of_eq_init {α : Type u_1} [comm_semiring α] (E : linear_recurrence α) (u : ℕ → α)
    (v : ℕ → α) (hu : is_solution E u) (hv : is_solution E v) :
    u = v ↔ set.eq_on u v ↑(finset.range (order E)) :=
  sorry
...
/-- `E.tuple_succ` maps `![s₀, s₁, ..., sₙ]` to `![s₁, ..., sₙ, ∑ (E.coeffs i) * sᵢ]`,
  where `n := E.order`. -/
def tuple_succ {α : Type u_1} [comm_semiring α] (E : linear_recurrence α) :
    linear_map α (fin (order E) → α) (fin (order E) → α) :=
  linear_map.mk
    (fun (X : fin (order E) → α) (i : fin (order E)) =>
      dite (↑i + 1 < order E) (fun (h : ↑i + 1 < order E) => X { val := ↑i + 1, property := h })
        fun (h : ¬↑i + 1 < order E) =>
          finset.sum finset.univ fun (i : fin (order E)) => coeffs E i * X i)
    sorry sorry

/-- The dimension of `E.sol_space` is `E.order`. -/
theorem sol_space_dim {α : Type u_1} [field α] (E : linear_recurrence α) :
    vector_space.dim α ↥(sol_space E) = ↑(order E) :=
...
theorem geom_sol_iff_root_char_poly {α : Type u_1} [comm_ring α] (E : linear_recurrence α) (q : α) :
    (is_solution E fun (n : ℕ) => q ^ n) ↔ polynomial.is_root (char_poly E) q :=
  sorry
