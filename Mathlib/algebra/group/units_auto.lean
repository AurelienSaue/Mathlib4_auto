...
structure add_units (α : Type u) [add_monoid α] where
  val : α
  neg : α
  val_neg : val + neg = 0
  neg_val : neg + val = 0
...
protected instance Mathlib.add_units.has_coe {α : Type u} [add_monoid α] :
    has_coe (add_units α) α :=
  has_coe.mk add_units.val

@[simp] theorem Mathlib.add_units.coe_mk {α : Type u} [add_monoid α] (a : α) (b : α)
    (h₁ : a + b = 0) (h₂ : b + a = 0) : ↑(add_units.mk a b h₁ h₂) = a :=
  rfl

theorem ext {α : Type u} [monoid α] : function.injective coe := sorry

theorem Mathlib.add_units.eq_iff {α : Type u} [add_monoid α] {a : add_units α} {b : add_units α} :
    ↑a = ↑b ↔ a = b :=
  function.injective.eq_iff add_units.ext

theorem Mathlib.add_units.ext_iff {α : Type u} [add_monoid α] {a : add_units α} {b : add_units α} :
    a = b ↔ ↑a = ↑b :=
  iff.symm add_units.eq_iff

protected instance Mathlib.add_units.decidable_eq {α : Type u} [add_monoid α] [DecidableEq α] :
    DecidableEq (add_units α) :=
  fun (a b : add_units α) => decidable_of_iff' (↑a = ↑b) add_units.ext_iff

@[simp] theorem mk_coe {α : Type u} [monoid α] (u : units α) (y : α) (h₁ : ↑u * y = 1)
    (h₂ : y * ↑u = 1) : mk (↑u) y h₁ h₂ = u :=
  ext rfl

/-- Units of a monoid form a group. -/
protected instance group {α : Type u} [monoid α] : group (units α) :=
  group.mk (fun (u₁ u₂ : units α) => mk (val u₁ * val u₂) (inv u₂ * inv u₁) sorry sorry) sorry
    (mk 1 1 sorry sorry) sorry sorry
    (fun (u : units α) => mk (inv u) (val u) (inv_val u) (val_inv u))
    (div_inv_monoid.div._default
      (fun (u₁ u₂ : units α) => mk (val u₁ * val u₂) (inv u₂ * inv u₁) sorry sorry) sorry
      (mk 1 1 sorry sorry) sorry sorry
      fun (u : units α) => mk (inv u) (val u) (inv_val u) (val_inv u))
    sorry

@[simp] theorem Mathlib.add_units.coe_add {α : Type u} [add_monoid α] (a : add_units α)
    (b : add_units α) : ↑(a + b) = ↑a + ↑b :=
  rfl

@[simp] theorem coe_one {α : Type u} [monoid α] : ↑1 = 1 := rfl

@[simp] theorem Mathlib.add_units.coe_eq_zero {α : Type u} [add_monoid α] {a : add_units α} :
    ↑a = 0 ↔ a = 0 :=
  eq.mpr (id (Eq._oldrec (Eq.refl (↑a = 0 ↔ a = 0)) (Eq.symm add_units.coe_zero)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (↑a = ↑0 ↔ a = 0)) (propext add_units.eq_iff)))
      (iff.refl (a = 0)))

@[simp] theorem inv_mk {α : Type u} [monoid α] (x : α) (y : α) (h₁ : x * y = 1) (h₂ : y * x = 1) :
    mk x y h₁ h₂⁻¹ = mk y x h₂ h₁ :=
  rfl

theorem val_coe {α : Type u} [monoid α] (a : units α) : ↑a = val a := rfl

theorem coe_inv {α : Type u} [monoid α] (a : units α) : ↑(a⁻¹) = inv a := rfl

@[simp] theorem inv_mul {α : Type u} [monoid α] (a : units α) : ↑(a⁻¹) * ↑a = 1 := inv_val a

@[simp] theorem Mathlib.add_units.add_neg {α : Type u} [add_monoid α] (a : add_units α) :
    ↑a + ↑(-a) = 0 :=
  add_units.val_neg a
...
theorem Mathlib.add_units.add_neg_of_eq {α : Type u} [add_monoid α] {u : add_units α} {a : α}
    (h : ↑u = a) : a + ↑(-u) = 0 :=
  eq.mpr (id (Eq._oldrec (Eq.refl (a + ↑(-u) = 0)) (Eq.symm h)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (↑u + ↑(-u) = 0)) (add_units.add_neg u))) (Eq.refl 0))

@[simp] theorem mul_inv_cancel_left {α : Type u} [monoid α] (a : units α) (b : α) :
    ↑a * (↑(a⁻¹) * b) = b :=
...
@[simp] theorem Mathlib.add_units.neg_add_cancel_left {α : Type u} [add_monoid α] (a : add_units α)
    (b : α) : ↑(-a) + (↑a + b) = b :=
...
@[simp] theorem mul_inv_cancel_right {α : Type u} [monoid α] (a : α) (b : units α) :
    a * ↑b * ↑(b⁻¹) = a :=
...
@[simp] theorem Mathlib.add_units.neg_add_cancel_right {α : Type u} [add_monoid α] (a : α)
    (b : add_units α) : a + ↑(-b) + ↑b = a :=
...
protected instance Mathlib.add_units.inhabited {α : Type u} [add_monoid α] :
...
protected instance Mathlib.add_units.has_repr {α : Type u} [add_monoid α] [has_repr α] :
    has_repr (add_units α) :=
  has_repr.mk (repr ∘ add_units.val)

@[simp] theorem Mathlib.add_units.add_right_inj {α : Type u} [add_monoid α] (a : add_units α)
    {b : α} {c : α} : ↑a + b = ↑a + c ↔ b = c :=
  sorry

@[simp] theorem Mathlib.add_units.add_left_inj {α : Type u} [add_monoid α] (a : add_units α) {b : α}
    {c : α} : b + ↑a = c + ↑a ↔ b = c :=
  sorry

theorem Mathlib.add_units.eq_add_neg_iff_add_eq {α : Type u} [add_monoid α] {c : add_units α}
    {a : α} {b : α} : a = b + ↑(-c) ↔ a + ↑c = b :=
  sorry

theorem eq_inv_mul_iff_mul_eq {α : Type u} [monoid α] (b : units α) {a : α} {c : α} :
    a = ↑(b⁻¹) * c ↔ ↑b * a = c :=
  sorry

theorem inv_mul_eq_iff_eq_mul {α : Type u} [monoid α] (a : units α) {b : α} {c : α} :
    ↑(a⁻¹) * b = c ↔ b = ↑a * c :=
  sorry

theorem mul_inv_eq_iff_eq_mul {α : Type u} [monoid α] (b : units α) {a : α} {c : α} :
    a * ↑(b⁻¹) = c ↔ a = c * ↑b :=
  sorry

theorem inv_eq_of_mul_eq_one {α : Type u} [monoid α] {u : units α} {a : α} (h : ↑u * a = 1) :
    ↑(u⁻¹) = a :=
  sorry

theorem inv_unique {α : Type u} [monoid α] {u₁ : units α} {u₂ : units α} (h : ↑u₁ = ↑u₂) :
    ↑(u₁⁻¹) = ↑(u₂⁻¹) :=
...
def add_units.mk_of_add_eq_zero {α : Type u} [add_comm_monoid α] (a : α) (b : α) (hab : a + b = 0) :
    add_units α :=
  add_units.mk a b hab sorry

@[simp] theorem units.coe_mk_of_mul_eq_one {α : Type u} [comm_monoid α] {a : α} {b : α}
    (h : a * b = 1) : ↑(units.mk_of_mul_eq_one a b h) = a :=
  rfl

/-- Partial division. It is defined when the
  second argument is invertible, and unlike the division operator
  in `division_ring` it is not totalized at zero. -/
def divp {α : Type u} [monoid α] (a : α) (u : units α) : α := a * ↑(u⁻¹)

infixl:70 " /ₚ " => Mathlib.divp

@[simp] theorem divp_self {α : Type u} [monoid α] (u : units α) : ↑u /ₚ u = 1 := units.mul_inv u

@[simp] theorem divp_one {α : Type u} [monoid α] (a : α) : a /ₚ 1 = a := mul_one a

theorem divp_assoc {α : Type u} [monoid α] (a : α) (b : α) (u : units α) :
    a * b /ₚ u = a * (b /ₚ u) :=
  mul_assoc a b ↑(u⁻¹)

@[simp] theorem divp_inv {α : Type u} [monoid α] {a : α} (u : units α) : a /ₚ (u⁻¹) = a * ↑u := rfl
...
@[simp] theorem divp_left_inj {α : Type u} [monoid α] (u : units α) {a : α} {b : α} :
    a /ₚ u = b /ₚ u ↔ a = b :=
  units.mul_left_inj (u⁻¹)

theorem divp_divp_eq_divp_mul {α : Type u} [monoid α] (x : α) (u₁ : units α) (u₂ : units α) :
    x /ₚ u₁ /ₚ u₂ = x /ₚ (u₂ * u₁) :=
  sorry
...
@[simp] theorem one_divp {α : Type u} [monoid α] (u : units α) : 1 /ₚ u = ↑(u⁻¹) := one_mul ↑(u⁻¹)

theorem divp_eq_divp_iff {α : Type u} [comm_monoid α] {x : α} {y : α} {ux : units α}
    {uy : units α} : x /ₚ ux = y /ₚ uy ↔ x * ↑uy = y * ↑ux :=
  sorry

theorem divp_mul_divp {α : Type u} [comm_monoid α] (x : α) (y : α) (ux : units α) (uy : units α) :
...
def is_unit {M : Type u_1} [monoid M] (a : M) := ∃ (u : units M), ↑u = a
