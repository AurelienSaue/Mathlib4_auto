...
-/
class has_star (R : Type u) where
  star : R → R

/--
A star operation (e.g. complex conjugate).
-/
def star {R : Type u} [has_star R] (r : R) : R := has_star.star r

/--
Typeclass for a star operation with is involutive.
-/
class has_involutive_star (R : Type u) extends has_star R where
...
-/
class star_monoid (R : Type u) [monoid R] extends has_involutive_star R where
  star_mul : ∀ (r s : R), has_star.star (r * s) = has_star.star s * has_star.star r

@[simp] theorem star_mul {R : Type u} [monoid R] [star_monoid R] (r : R) (s : R) :
    star (r * s) = star s * star r :=
  star_monoid.star_mul r s

/-- `star` as an `mul_equiv` from `R` to `Rᵒᵖ` -/
@[simp] theorem star_mul_equiv_apply {R : Type u} [monoid R] [star_monoid R] (x : R) :
    coe_fn star_mul_equiv x = opposite.op (star x) :=
...
-/
class star_ring (R : Type u) [semiring R] extends star_monoid R where
  star_add : ∀ (r s : R), has_star.star (r + s) = has_star.star r + has_star.star s

@[simp] theorem star_add {R : Type u} [semiring R] [star_ring R] (r : R) (s : R) :
...
@[simp] theorem star_ring_equiv_apply {R : Type u} [semiring R] [star_ring R] (x : R) :
    coe_fn star_ring_equiv x = opposite.op (star x) :=
  Eq.refl (coe_fn star_ring_equiv x)

@[simp] theorem star_sum {R : Type u} [semiring R] [star_ring R] {α : Type u_1} (s : finset α)
    (f : α → R) : star (finset.sum s fun (x : α) => f x) = finset.sum s fun (x : α) => star (f x) :=
...
@[simp] theorem star_sub {R : Type u} [ring R] [star_ring R] (r : R) (s : R) :
    star (r - s) = star r - star s :=
  add_equiv.map_sub star_add_equiv r s

@[simp] theorem star_bit0 {R : Type u} [ring R] [star_ring R] (r : R) :
    star (bit0 r) = bit0 (star r) :=
  sorry

@[simp] theorem star_bit1 {R : Type u} [ring R] [star_ring R] (r : R) :
...
-/
def star_ring_of_comm {R : Type u_1} [comm_semiring R] : star_ring R := star_ring.mk sorry

@[simp] theorem star_id_of_comm {R : Type u_1} [comm_semiring R] {x : R} : star x = x := rfl
