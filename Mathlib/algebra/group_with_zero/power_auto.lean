...
@[simp] theorem zero_pow_eq_zero {M : Type u_1} [monoid_with_zero M] [nontrivial M] {n : ℕ} :
    0 ^ n = 0 ↔ 0 < n :=
  sorry

@[simp] theorem inv_pow' {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (n : ℕ) :
    a⁻¹ ^ n = (a ^ n⁻¹) :=
  sorry

theorem pow_sub' {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) {m : ℕ} {n : ℕ} (ha : a ≠ 0)
    (h : n ≤ m) : a ^ (m - n) = a ^ m * (a ^ n⁻¹) :=
  sorry

theorem pow_inv_comm' {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (m : ℕ) (n : ℕ) :
    a⁻¹ ^ m * a ^ n = a ^ n * a⁻¹ ^ m :=
...
-/
def fpow {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) : ℤ → G₀ := sorry

protected instance int.has_pow {G₀ : Type u_1} [group_with_zero G₀] : has_pow G₀ ℤ :=
  has_pow.mk fpow

@[simp] theorem fpow_coe_nat {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (n : ℕ) :
    a ^ ↑n = a ^ n :=
  rfl
...
@[simp] theorem fpow_neg_succ_of_nat {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (n : ℕ) :
    a ^ Int.negSucc n = (a ^ Nat.succ n⁻¹) :=
  rfl

@[simp] theorem fpow_zero {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) : a ^ 0 = 1 := rfl

@[simp] theorem fpow_one {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) : a ^ 1 = a := mul_one a
...
@[simp] theorem fpow_neg {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (n : ℤ) :
    a ^ (-n) = (a ^ n⁻¹) :=
  sorry
...
theorem fpow_add {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} (ha : a ≠ 0) (m : ℤ) (n : ℤ) :
    a ^ (m + n) = a ^ m * a ^ n :=
  sorry

theorem fpow_add' {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {m : ℤ} {n : ℤ}
    (h : a ≠ 0 ∨ m + n ≠ 0 ∨ m = 0 ∧ n = 0) : a ^ (m + n) = a ^ m * a ^ n :=
  sorry

theorem fpow_one_add {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} (h : a ≠ 0) (i : ℤ) :
    a ^ (1 + i) = a * a ^ i :=
  eq.mpr (id (Eq._oldrec (Eq.refl (a ^ (1 + i) = a * a ^ i)) (fpow_add h 1 i)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (a ^ 1 * a ^ i = a * a ^ i)) (fpow_one a)))
      (Eq.refl (a * a ^ i)))

theorem semiconj_by.fpow_right {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {x : G₀} {y : G₀}
  ...
  int.cases_on m (fun (m : ℕ) => idRhs (semiconj_by a (x ^ m) (y ^ m)) (semiconj_by.pow_right h m))
    fun (m : ℕ) =>
      idRhs (semiconj_by a (x ^ (m + 1)⁻¹) (y ^ (m + 1)⁻¹))
        (semiconj_by.inv_right' (semiconj_by.pow_right h (m + 1)))

theorem commute.fpow_right {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {b : G₀} (h : commute a b)
    (m : ℤ) : commute a (b ^ m) :=
  semiconj_by.fpow_right h

theorem commute.fpow_left {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {b : G₀} (h : commute a b)
    (m : ℤ) : commute (a ^ m) b :=
  commute.symm (commute.fpow_right (commute.symm h) m)

theorem commute.fpow_fpow {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {b : G₀} (h : commute a b)
    (m : ℤ) (n : ℤ) : commute (a ^ m) (b ^ n) :=
...
theorem commute.fpow_fpow_self {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (m : ℤ) (n : ℤ) :
    commute (a ^ m) (a ^ n) :=
...
theorem fpow_mul {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (m : ℤ) (n : ℤ) :
    a ^ (m * n) = (a ^ m) ^ n :=
  sorry
...
theorem fpow_mul' {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (m : ℤ) (n : ℤ) :
  ...
  eq.mpr (id (Eq._oldrec (Eq.refl (a ^ (m * n) = (a ^ n) ^ m)) (mul_comm m n)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (a ^ (n * m) = (a ^ n) ^ m)) (fpow_mul a n m)))
      (Eq.refl ((a ^ n) ^ m)))

@[simp] theorem units.coe_gpow' {G₀ : Type u_1} [group_with_zero G₀] (u : units G₀) (n : ℤ) :
    ↑(u ^ n) = ↑u ^ n :=
  sorry
...
theorem fpow_sub {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} (ha : a ≠ 0) (z1 : ℤ) (z2 : ℤ) :
    a ^ (z1 - z2) = a ^ z1 / a ^ z2 :=
  sorry

theorem commute.mul_fpow {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} {b : G₀} (h : commute a b)
    (i : ℤ) : (a * b) ^ i = a ^ i * b ^ i :=
  sorry

theorem mul_fpow {G₀ : Type u_1} [comm_group_with_zero G₀] (a : G₀) (b : G₀) (m : ℤ) :
    (a * b) ^ m = a ^ m * b ^ m :=
...
theorem fpow_bit1' {G₀ : Type u_1} [group_with_zero G₀] (a : G₀) (n : ℤ) :
  ...
  eq.mpr (id (Eq._oldrec (Eq.refl (a ^ bit1 n = (a * a) ^ n * a)) (fpow_bit1 a n)))
    (eq.mpr
      (id
        (Eq._oldrec (Eq.refl (a ^ n * a ^ n * a = (a * a) ^ n * a))
          (commute.mul_fpow (commute.refl a) n)))
      (Eq.refl (a ^ n * a ^ n * a)))
...
theorem fpow_neg_mul_fpow_self {G₀ : Type u_1} [group_with_zero G₀] (n : ℤ) {x : G₀} (h : x ≠ 0) :
    x ^ (-n) * x ^ n = 1 :=
  eq.mpr (id (Eq._oldrec (Eq.refl (x ^ (-n) * x ^ n = 1)) (fpow_neg x n)))
    (inv_mul_cancel (fpow_ne_zero n h))
...
@[simp] theorem inv_fpow' {G₀ : Type u_1} [group_with_zero G₀] {a : G₀} (n : ℤ) :
    a⁻¹ ^ n = a ^ (-n) :=
  sorry

@[simp] theorem div_pow {G₀ : Type u_1} [comm_group_with_zero G₀] (a : G₀) (b : G₀) (n : ℕ) :
    (a / b) ^ n = a ^ n / b ^ n :=
  sorry

@[simp] theorem div_fpow {G₀ : Type u_1} [comm_group_with_zero G₀] (a : G₀) {b : G₀} (n : ℤ) :
    (a / b) ^ n = a ^ n / b ^ n :=
  sorry
...
theorem div_sq_cancel {G₀ : Type u_1} [comm_group_with_zero G₀] {a : G₀} (ha : a ≠ 0) (b : G₀) :
  ...
  eq.mpr (id (Eq._oldrec (Eq.refl (a ^ bit0 1 * b / a = a * b)) (pow_two a)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (a * a * b / a = a * b)) (mul_assoc a a b)))
      (eq.mpr (id (Eq._oldrec (Eq.refl (a * (a * b) / a = a * b)) (mul_div_cancel_left (a * b) ha)))
        (Eq.refl (a * b))))

/-- If a monoid homomorphism `f` between two `group_with_zero`s maps `0` to `0`, then it maps `x^n`,
`n : ℤ`, to `(f x)^n`. -/
theorem monoid_with_zero_hom.map_fpow {G₀ : Type u_1} {G₀' : Type u_2} [group_with_zero G₀]
    [group_with_zero G₀'] (f : monoid_with_zero_hom G₀ G₀') (x : G₀) (n : ℤ) :
