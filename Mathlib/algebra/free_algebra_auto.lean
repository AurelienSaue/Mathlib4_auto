...
-/
inductive pre (R : Type u_1) [comm_semiring R] (X : Type u_2) where
| of : X → pre R X
| of_scalar : R → pre R X
...
protected instance inhabited (R : Type u_1) [comm_semiring R] (X : Type u_2) :
    Inhabited (pre R X) :=
  { default := of_scalar 0 }
...
def has_mul (R : Type u_1) [comm_semiring R] (X : Type u_2) : Mul (pre R X) := { mul := mul }

/-- Zero in `pre R X` defined as the image of `0` from `R`. Note: Used for notation only. -/
def has_add (R : Type u_1) [comm_semiring R] (X : Type u_2) : Add (pre R X) := { add := add }
...
-/
def lift_fun (R : Type u_1) [comm_semiring R] (X : Type u_2) {A : Type u_3} [semiring A]
    [algebra R A] (f : X → A) : pre R X → A :=
  fun (t : pre R X) =>
    pre.rec_on t f (⇑(algebra_map R A)) (fun (_x _x : pre R X) => Add.add)
      fun (_x _x : pre R X) => Mul.mul
...
inductive rel (R : Type u_1) [comm_semiring R] (X : Type u_2) : pre R X → pre R X → Prop where
...
-/
def free_algebra (R : Type u_1) [comm_semiring R] (X : Type u_2) := Quot sorry

namespace free_algebra


protected instance semiring (R : Type u_1) [comm_semiring R] (X : Type u_2) :
...
protected instance has_scalar (R : Type u_1) [comm_semiring R] (X : Type u_2) :
    has_scalar R (free_algebra R X) :=
  has_scalar.mk
    fun (r : R) (a : free_algebra R X) =>
      quot.lift_on a (fun (x : pre R X) => Quot.mk (rel R X) (↑r * x)) sorry

protected instance algebra (R : Type u_1) [comm_semiring R] (X : Type u_2) :
...
@[simp] theorem quot_mk_eq_ι (R : Type u_1) [comm_semiring R] {X : Type u_2} (m : X) :
    Quot.mk (rel R X) ↑m = ι R m :=
  rfl
...
-/
def lift (R : Type u_1) [comm_semiring R] {X : Type u_2} {A : Type u_3} [semiring A] [algebra R A] :
    (X → A) ≃ alg_hom R (free_algebra R X) A :=
  equiv.mk (lift_aux R) (fun (F : alg_hom R (free_algebra R X) A) => ⇑F ∘ ι R) sorry sorry

@[simp] theorem lift_aux_eq (R : Type u_1) [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (f : X → A) : lift_aux R f = coe_fn (lift R) f :=
  rfl

@[simp] theorem lift_symm_apply (R : Type u_1) [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (F : alg_hom R (free_algebra R X) A) :
    coe_fn (equiv.symm (lift R)) F = ⇑F ∘ ι R :=
  rfl

@[simp] theorem ι_comp_lift {R : Type u_1} [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (f : X → A) : ⇑(coe_fn (lift R) f) ∘ ι R = f :=
  funext fun (x : X) => Eq.refl (function.comp (⇑(coe_fn (lift R) f)) (ι R) x)

@[simp] theorem lift_ι_apply {R : Type u_1} [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (f : X → A) (x : X) : coe_fn (coe_fn (lift R) f) (ι R x) = f x :=
  rfl

@[simp] theorem lift_unique {R : Type u_1} [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (f : X → A) (g : alg_hom R (free_algebra R X) A) :
...
@[simp] theorem lift_comp_ι {R : Type u_1} [comm_semiring R] {X : Type u_2} {A : Type u_3}
    [semiring A] [algebra R A] (g : alg_hom R (free_algebra R X) A) :
    coe_fn (lift R) (⇑g ∘ ι R) = g :=
  eq.mpr
    (id (Eq._oldrec (Eq.refl (coe_fn (lift R) (⇑g ∘ ι R) = g)) (Eq.symm (lift_symm_apply R g))))
    (equiv.apply_symm_apply (lift R) g)

/-- See note [partially-applied ext lemmas]. -/
theorem hom_ext {R : Type u_1} [comm_semiring R] {X : Type u_2} {A : Type u_3} [semiring A]
    [algebra R A] {f : alg_hom R (free_algebra R X) A} {g : alg_hom R (free_algebra R X) A}
    (w : ⇑f ∘ ι R = ⇑g ∘ ι R) : f = g :=
  equiv.injective (equiv.symm (lift R))
    (eq.mp
      (Eq._oldrec (Eq.refl (coe_fn (equiv.symm (lift R)) f = ⇑g ∘ ι R))
        (Eq.symm (lift_symm_apply R g)))
...
for example.
-/
def equiv_monoid_algebra_free_monoid {R : Type u_1} [comm_semiring R] {X : Type u_2} :
    alg_equiv R (free_algebra R X) (monoid_algebra R (free_monoid X)) :=
  alg_equiv.of_alg_hom
    (coe_fn (lift R) fun (x : X) => coe_fn (monoid_algebra.of R (free_monoid X)) (free_monoid.of x))
    (coe_fn (monoid_algebra.lift R (free_monoid X) (free_algebra R X))
      (coe_fn free_monoid.lift (ι R)))
    sorry sorry

protected instance nontrivial {R : Type u_1} [comm_semiring R] {X : Type u_2} [nontrivial R] :
    nontrivial (free_algebra R X) :=
  equiv.nontrivial (alg_equiv.to_equiv equiv_monoid_algebra_free_monoid)

/-- The left-inverse of `algebra_map`. -/
def algebra_map_inv {R : Type u_1} [comm_semiring R] {X : Type u_2} :
    alg_hom R (free_algebra R X) R :=
  coe_fn (lift R) 0

theorem algebra_map_left_inverse {R : Type u_1} [comm_semiring R] {X : Type u_2} :
...
theorem ι_injective {R : Type u_1} [comm_semiring R] {X : Type u_2} [nontrivial R] :
    function.injective (ι R) :=
  sorry
...
-/
theorem induction (R : Type u_1) [comm_semiring R] (X : Type u_2) {C : free_algebra R X → Prop}
    (h_grade0 : ∀ (r : R), C (coe_fn (algebra_map R (free_algebra R X)) r))
    (h_grade1 : ∀ (x : X), C (ι R x)) (h_mul : ∀ (a b : free_algebra R X), C a → C b → C (a * b))
    (h_add : ∀ (a b : free_algebra R X), C a → C b → C (a + b)) (a : free_algebra R X) : C a :=
  sorry

/-- The star ring formed by reversing the elements of products -/
protected instance star_ring (R : Type u_1) [comm_semiring R] (X : Type u_2) :
    star_ring (free_algebra R X) :=
  star_ring.mk sorry

@[simp] theorem star_ι (R : Type u_1) [comm_semiring R] (X : Type u_2) (x : X) :
    star (ι R x) = ι R x :=
  sorry

@[simp] theorem star_algebra_map (R : Type u_1) [comm_semiring R] (X : Type u_2) (r : R) :
    star (coe_fn (algebra_map R (free_algebra R X)) r) =
        coe_fn (algebra_map R (free_algebra R X)) r :=
  sorry

/-- `star` as an `alg_equiv` -/
def star_hom (R : Type u_1) [comm_semiring R] (X : Type u_2) :
    alg_equiv R (free_algebra R X) (free_algebra R Xᵒᵖ) :=
  alg_equiv.mk (ring_equiv.to_fun star_ring_equiv) (ring_equiv.inv_fun star_ring_equiv) sorry sorry
    sorry sorry sorry
