...
theorem stream_eq_none_of_fr_eq_zero {K : Type u_1} [linear_ordered_field K] [floor_ring K] {v : K}
    {n : ℕ} {ifp_n : int_fract_pair K} (stream_nth_eq : int_fract_pair.stream v n = some ifp_n)
    (nth_fr_eq_zero : fr ifp_n = 0) : int_fract_pair.stream v (n + 1) = none :=
  sorry
...
-/
theorem succ_nth_stream_eq_none_iff {K : Type u_1} [linear_ordered_field K] [floor_ring K] {v : K}
    {n : ℕ} :
    int_fract_pair.stream v (n + 1) = none ↔
        int_fract_pair.stream v n = none ∨
          ∃ (ifp : int_fract_pair K), int_fract_pair.stream v n = some ifp ∧ fr ifp = 0 :=
  sorry
...
-/
theorem succ_nth_stream_eq_some_iff {K : Type u_1} [linear_ordered_field K] [floor_ring K] {v : K}
    {n : ℕ} {ifp_succ_n : int_fract_pair K} :
    int_fract_pair.stream v (n + 1) = some ifp_succ_n ↔
        ∃ (ifp_n : int_fract_pair K),
          int_fract_pair.stream v n = some ifp_n ∧
            fr ifp_n ≠ 0 ∧ int_fract_pair.of (fr ifp_n⁻¹) = ifp_succ_n :=
  sorry

theorem exists_succ_nth_stream_of_fr_zero {K : Type u_1} [linear_ordered_field K] [floor_ring K]
    {v : K} {n : ℕ} {ifp_succ_n : int_fract_pair K}
    (stream_succ_nth_eq : int_fract_pair.stream v (n + 1) = some ifp_succ_n)
    (succ_nth_fr_eq_zero : fr ifp_succ_n = 0) :
    ∃ (ifp_n : int_fract_pair K),
        int_fract_pair.stream v n = some ifp_n ∧ fr ifp_n⁻¹ = ↑(floor (fr ifp_n⁻¹)) :=
  sorry
...
@[simp] theorem int_fract_pair.seq1_fst_eq_of {K : Type u_1} [linear_ordered_field K] [floor_ring K]
    {v : K} : prod.fst (int_fract_pair.seq1 v) = int_fract_pair.of v :=
  rfl

theorem of_h_eq_int_fract_pair_seq1_fst_b {K : Type u_1} [linear_ordered_field K] [floor_ring K]
    {v : K} :
    h (generalized_continued_fraction.of v) =
        ↑(int_fract_pair.b (prod.fst (int_fract_pair.seq1 v))) :=
  sorry

/-- The head term of the gcf of `v` is `⌊v⌋`. -/
@[simp] theorem of_h_eq_floor {K : Type u_1} [linear_ordered_field K] [floor_ring K] {v : K} :
    h (generalized_continued_fraction.of v) = ↑(floor v) :=
  sorry
...
theorem int_fract_pair.nth_seq1_eq_succ_nth_stream {K : Type u_1} [linear_ordered_field K]
    [floor_ring K] {v : K} {n : ℕ} :
    seq.nth (prod.snd (int_fract_pair.seq1 v)) n = int_fract_pair.stream v (n + 1) :=
  rfl
...
theorem of_terminated_at_iff_int_fract_pair_seq1_terminated_at {K : Type u_1}
    [linear_ordered_field K] [floor_ring K] {v : K} {n : ℕ} :
    terminated_at (generalized_continued_fraction.of v) n ↔
        seq.terminated_at (prod.snd (int_fract_pair.seq1 v)) n :=
  sorry

theorem of_terminated_at_n_iff_succ_nth_int_fract_pair_stream_eq_none {K : Type u_1}
    [linear_ordered_field K] [floor_ring K] {v : K} {n : ℕ} :
    terminated_at (generalized_continued_fraction.of v) n ↔
        int_fract_pair.stream v (n + 1) = none :=
  sorry
...
theorem int_fract_pair.exists_succ_nth_stream_of_gcf_of_nth_eq_some {K : Type u_1}
    [linear_ordered_field K] [floor_ring K] {v : K} {n : ℕ} {gp_n : pair K}
    (s_nth_eq : seq.nth (s (generalized_continued_fraction.of v)) n = some gp_n) :
    ∃ (ifp : int_fract_pair K),
        int_fract_pair.stream v (n + 1) = some ifp ∧ ↑(int_fract_pair.b ifp) = pair.b gp_n :=
  sorry
...
-/
theorem nth_of_eq_some_of_succ_nth_int_fract_pair_stream {K : Type u_1} [linear_ordered_field K]
    [floor_ring K] {v : K} {n : ℕ} {ifp_succ_n : int_fract_pair K}
    (stream_succ_nth_eq : int_fract_pair.stream v (n + 1) = some ifp_succ_n) :
    seq.nth (s (generalized_continued_fraction.of v)) n =
        some (pair.mk 1 ↑(int_fract_pair.b ifp_succ_n)) :=
  sorry
...
-/
theorem nth_of_eq_some_of_nth_int_fract_pair_stream_fr_ne_zero {K : Type u_1}
    [linear_ordered_field K] [floor_ring K] {v : K} {n : ℕ} {ifp_n : int_fract_pair K}
    (stream_nth_eq : int_fract_pair.stream v n = some ifp_n)
    (nth_fr_ne_zero : int_fract_pair.fr ifp_n ≠ 0) :
    seq.nth (s (generalized_continued_fraction.of v)) n =
        some (pair.mk 1 ↑(int_fract_pair.b (int_fract_pair.of (int_fract_pair.fr ifp_n⁻¹)))) :=
  sorry
