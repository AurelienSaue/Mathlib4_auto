...
`⟨b, fr⟩`.
-/
structure int_fract_pair (K : Type u_1) where
  b : ℤ
  fr : K
...
protected instance has_coe_to_int_fract_pair {K : Type u_1} {β : Type u_2} [has_coe K β] :
    has_coe (int_fract_pair K) (int_fract_pair β) :=
  has_coe.mk (mapFr coe)

@[simp] theorem coe_to_int_fract_pair {K : Type u_1} {β : Type u_2} [has_coe K β] {b : ℤ} {fr : K} :
    ↑(mk b fr) = mk b ↑fr :=
  rfl
...
protected def of {K : Type u_1} [linear_ordered_field K] [floor_ring K] (v : K) :
    int_fract_pair K :=
  mk (floor v) (fract v)
...
-/
protected def stream {K : Type u_1} [linear_ordered_field K] [floor_ring K] (v : K) :
...
-/
theorem stream_is_seq {K : Type u_1} [linear_ordered_field K] [floor_ring K] (v : K) :
    stream.is_seq (int_fract_pair.stream v) :=
  sorry
...
-/
protected def seq1 {K : Type u_1} [linear_ordered_field K] [floor_ring K] (v : K) :
    seq1 (int_fract_pair K) :=
