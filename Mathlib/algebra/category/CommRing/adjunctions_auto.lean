...
@[simp] theorem free_obj_coe {α : Type u} :
    ↥(category_theory.functor.obj free α) = mv_polynomial α ℤ :=
  rfl

@[simp] theorem free_map_coe {α : Type u} {β : Type u} {f : α → β} :
    ⇑(category_theory.functor.map free f) = ⇑(mv_polynomial.rename f) :=
  rfl
...
def adj : free ⊣ category_theory.forget CommRing :=
  category_theory.adjunction.mk_of_hom_equiv
    (category_theory.adjunction.core_hom_equiv.mk
      fun (X : Type (max u_1 u_2)) (R : CommRing) => mv_polynomial.hom_equiv)
