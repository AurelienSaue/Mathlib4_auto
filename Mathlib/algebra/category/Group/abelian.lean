...
def normal_mono {X : AddCommGroup} {Y : AddCommGroup} (f : X ⟶ Y) (hf : category_theory.mono f) : category_theory.normal_mono f :=
  category_theory.equivalence_reflects_normal_mono
    (category_theory.functor.inv (category_theory.forget₂ (Module ℤ) AddCommGroup))
    (Module.normal_mono
      (category_theory.functor.map (category_theory.functor.inv (category_theory.forget₂ (Module ℤ) AddCommGroup)) f)
      sorry)

/-- In the category of abelian groups, every epimorphism is normal. -/
def normal_epi {X : AddCommGroup} {Y : AddCommGroup} (f : X ⟶ Y) (hf : category_theory.epi f) : category_theory.normal_epi f :=
...
protected instance category_theory.abelian : category_theory.abelian AddCommGroup :=
  category_theory.abelian.mk (fun (X Y : AddCommGroup) => normal_mono) fun (X Y : AddCommGroup) => normal_epi
