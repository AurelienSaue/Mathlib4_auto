...
def free : Type u ⥤ AddCommGroup :=
  category_theory.functor.mk (fun (α : Type u) => of (free_abelian_group α))
    fun (X Y : Type u) => free_abelian_group.map

@[simp] theorem free_obj_coe {α : Type u} :
    ↥(category_theory.functor.obj free α) = free_abelian_group α :=
  rfl

@[simp] theorem free_map_coe {α : Type u} {β : Type u} {f : α → β} (x : free_abelian_group α) :
    coe_fn (category_theory.functor.map free f) x = f <$> x :=
  rfl
