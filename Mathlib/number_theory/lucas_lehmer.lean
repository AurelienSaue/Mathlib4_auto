...
def s_zmod (p : ℕ) : ℕ → zmod (bit0 1 ^ p - 1) :=
  sorry
...
def lucas_lehmer_residue (p : ℕ) : zmod (bit0 1 ^ p - 1) :=
  s_zmod p (p - bit0 1)

theorem residue_eq_zero_iff_s_mod_eq_zero (p : ℕ) (w : 1 < p) : lucas_lehmer_residue p = 0 ↔ s_mod p (p - bit0 1) = 0 := sorry
...
def q (p : ℕ) : ℕ+ :=
  { val := nat.min_fac (mersenne p), property := sorry }

protected instance fact_pnat_pos (q : ℕ+) : fact (0 < ↑q) :=
  subtype.property q
...
theorem ext {q : ℕ+} {x : X q} {y : X q} (h₁ : prod.fst x = prod.fst y) (h₂ : prod.snd x = prod.snd y) : x = y := sorry
...
@[simp] theorem neg_fst {q : ℕ+} (x : X q) : prod.fst (-x) = -prod.fst x :=
  rfl

@[simp] theorem neg_snd {q : ℕ+} (x : X q) : prod.snd (-x) = -prod.snd x :=
  rfl

protected instance has_mul {q : ℕ+} : Mul (X q) :=
  { mul :=
      fun (x y : X q) =>
        (prod.fst x * prod.fst y + bit1 1 * prod.snd x * prod.snd y, prod.fst x * prod.snd y + prod.snd x * prod.fst y) }

@[simp] theorem mul_fst {q : ℕ+} (x : X q) (y : X q) : prod.fst (x * y) = prod.fst x * prod.fst y + bit1 1 * prod.snd x * prod.snd y :=
  rfl

@[simp] theorem mul_snd {q : ℕ+} (x : X q) (y : X q) : prod.snd (x * y) = prod.fst x * prod.snd y + prod.snd x * prod.fst y :=
  rfl

protected instance has_one {q : ℕ+} : HasOne (X q) :=
  { one := (1, 0) }

@[simp] theorem one_fst {q : ℕ+} : prod.fst 1 = 1 :=
  rfl

@[simp] theorem one_snd {q : ℕ+} : prod.snd 1 = 0 :=
  rfl

@[simp] theorem bit0_fst {q : ℕ+} (x : X q) : prod.fst (bit0 x) = bit0 (prod.fst x) :=
  rfl

@[simp] theorem bit0_snd {q : ℕ+} (x : X q) : prod.snd (bit0 x) = bit0 (prod.snd x) :=
  rfl

@[simp] theorem bit1_fst {q : ℕ+} (x : X q) : prod.fst (bit1 x) = bit1 (prod.fst x) :=
  rfl

@[simp] theorem bit1_snd {q : ℕ+} (x : X q) : prod.snd (bit1 x) = bit0 (prod.snd x) := sorry

protected instance monoid {q : ℕ+} : monoid (X q) :=
  monoid.mk Mul.mul sorry (1, 0) sorry sorry
...
protected instance ring {q : ℕ+} : ring (X q) :=
  ring.mk add_comm_group.add sorry add_comm_group.zero sorry sorry add_comm_group.neg add_comm_group.sub sorry sorry
    monoid.mul sorry monoid.one sorry sorry left_distrib right_distrib
...
protected instance nontrivial {q : ℕ+} [fact (1 < ↑q)] : nontrivial (X q) :=
  nontrivial.mk
    (Exists.intro 0
      (Exists.intro 1 fun (h : 0 = 1) => prod.mk.inj_arrow h fun (h1 : 0 = 1) (ᾰ : 0 = 0) => zero_ne_one h1))

@[simp] theorem nat_coe_fst {q : ℕ+} (n : ℕ) : prod.fst ↑n = ↑n :=
  Nat.rec (Eq.refl (prod.fst ↑0))
    (fun (n_n : ℕ) (n_ih : prod.fst ↑n_n = ↑n_n) => id (eq.mpr (id (propext (add_left_inj 1))) n_ih)) n
...
theorem ω_pow_formula (p' : ℕ) (h : lucas_lehmer_residue (p' + bit0 1) = 0) : ∃ (k : ℤ), X.ω ^ bit0 1 ^ (p' + 1) = ↑k * ↑(mersenne (p' + bit0 1)) * X.ω ^ bit0 1 ^ p' - 1 := sorry

/-- `q` is the minimum factor of `mersenne p`, so `M p = 0` in `X q`. -/
theorem mersenne_coe_X (p : ℕ) : ↑(mersenne p) = 0 := sorry

theorem ω_pow_eq_neg_one (p' : ℕ) (h : lucas_lehmer_residue (p' + bit0 1) = 0) : X.ω ^ bit0 1 ^ (p' + 1) = -1 := sorry

theorem ω_pow_eq_one (p' : ℕ) (h : lucas_lehmer_residue (p' + bit0 1) = 0) : X.ω ^ bit0 1 ^ (p' + bit0 1) = 1 := sorry

/-- `ω` as an element of the group of units. -/
def ω_unit (p : ℕ) : units (X (q p)) :=
  units.mk X.ω X.ωb sorry sorry

@[simp] theorem ω_unit_coe (p : ℕ) : ↑(ω_unit p) = X.ω :=
  rfl

/-- The order of `ω` in the unit group is exactly `2^p`. -/
theorem order_ω (p' : ℕ) (h : lucas_lehmer_residue (p' + bit0 1) = 0) : order_of (ω_unit (p' + bit0 1)) = bit0 1 ^ (p' + bit0 1) := sorry

theorem order_ineq (p' : ℕ) (h : lucas_lehmer_residue (p' + bit0 1) = 0) : bit0 1 ^ (p' + bit0 1) < ↑(q (p' + bit0 1)) ^ bit0 1 :=
...
theorem s_mod_succ {p : ℕ} {a : ℤ} {i : ℕ} {b : ℤ} {c : ℤ} (h1 : bit0 1 ^ p - 1 = a) (h2 : s_mod p i = b) (h3 : (b * b - bit0 1) % a = c) : s_mod p (i + 1) = c := sorry
...
theorem modeq_mersenne (n : ℕ) (k : ℕ) : nat.modeq (bit0 1 ^ n - 1) k (k / bit0 1 ^ n + k % bit0 1 ^ n) := sorry
