...
class is_subring {R : Type u} [ring R] (S : set R) extends is_add_subgroup S, is_submonoid S where
...
/-- The ring structure on a subring coerced to a type. -/
def subtype.ring {R : Type u} [ring R] {S : set R} [is_subring S] : ring (Subtype S) := subset.ring

namespace ring_hom


protected instance is_subring_preimage {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S)
    (s : set S) [is_subring s] : is_subring (⇑f ⁻¹' s) :=
  is_subring.mk

protected instance is_subring_image {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S)
    (s : set R) [is_subring s] : is_subring (⇑f '' s) :=
  is_subring.mk

protected instance is_subring_set_range {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) :
...
def ring_hom.cod_restrict {R : Type u} {S : Type v} [ring R] [ring S] (f : R →+* S) (s : set S)
    [is_subring s] (h : ∀ (x : R), coe_fn f x ∈ s) : R →+* ↥s :=
...
@[simp] theorem is_subring.coe_subtype {R : Type u} [ring R] {S : set R} [is_subring S] :
...
protected instance is_subring.inter {R : Type u} [ring R] (S₁ : set R) (S₂ : set R) [is_subring S₁]
    [is_subring S₂] : is_subring (S₁ ∩ S₂) :=
  is_subring.mk

protected instance is_subring.Inter {R : Type u} [ring R] {ι : Sort u_1} (S : ι → set R)
    [h : ∀ (y : ι), is_subring (S y)] : is_subring (set.Inter S) :=
  is_subring.mk

theorem is_subring_Union_of_directed {R : Type u} [ring R] {ι : Type u_1} [hι : Nonempty ι]
    (s : ι → set R) [∀ (i : ι), is_subring (s i)]
    (directed : ∀ (i j : ι), ∃ (k : ι), s i ⊆ s k ∧ s j ⊆ s k) :
    is_subring (set.Union fun (i : ι) => s i) :=
  is_subring.mk

namespace ring


def closure {R : Type u} [ring R] (s : set R) : set R := add_group.closure (monoid.closure s)

theorem exists_list_of_mem_closure {R : Type u} [ring R] {s : set R} {a : R} (h : a ∈ closure s) :
    ∃ (L : List (List R)),
        (∀ (l : List R), l ∈ L → ∀ (x : R), x ∈ l → x ∈ s ∨ x = -1) ∧
          list.sum (list.map list.prod L) = a :=
  sorry
...
theorem closure_subset {R : Type u} [ring R] {s : set R} {t : set R} [is_subring t] :
    s ⊆ t → closure s ⊆ t :=
  add_group.closure_subset ∘ monoid.closure_subset

theorem closure_subset_iff {R : Type u} [ring R] (s : set R) (t : set R) [is_subring t] :
    closure s ⊆ t ↔ s ⊆ t :=
...
theorem image_closure {R : Type u} [ring R] {S : Type u_1} [ring S] (f : R →+* S) (s : set R) :
