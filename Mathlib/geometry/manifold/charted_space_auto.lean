...
structure structure_groupoid (H : Type u) [topological_space H] where
  members : set (local_homeomorph H H)
  trans' :
    ∀ (e e' : local_homeomorph H H),
      e ∈ members → e' ∈ members → local_homeomorph.trans e e' ∈ members
...
protected instance structure_groupoid.has_mem {H : Type u} [topological_space H] :
    has_mem (local_homeomorph H H) (structure_groupoid H) :=
  has_mem.mk
    fun (e : local_homeomorph H H) (G : structure_groupoid H) => e ∈ structure_groupoid.members G

theorem structure_groupoid.trans {H : Type u} [topological_space H] (G : structure_groupoid H)
    {e : local_homeomorph H H} {e' : local_homeomorph H H} (he : e ∈ G) (he' : e' ∈ G) :
    local_homeomorph.trans e e' ∈ G :=
  structure_groupoid.trans' G e e' he he'

theorem structure_groupoid.symm {H : Type u} [topological_space H] (G : structure_groupoid H)
    {e : local_homeomorph H H} (he : e ∈ G) : local_homeomorph.symm e ∈ G :=
  structure_groupoid.symm' G e he

theorem structure_groupoid.id_mem {H : Type u} [topological_space H] (G : structure_groupoid H) :
    local_homeomorph.refl H ∈ G :=
  structure_groupoid.id_mem' G

theorem structure_groupoid.locality {H : Type u} [topological_space H] (G : structure_groupoid H)
    {e : local_homeomorph H H}
    (h :
      ∀ (x : H),
        x ∈ local_equiv.source (local_homeomorph.to_local_equiv e) →
          ∃ (s : set H), is_open s ∧ x ∈ s ∧ local_homeomorph.restr e s ∈ G) :
    e ∈ G :=
  structure_groupoid.locality' G e h

theorem structure_groupoid.eq_on_source {H : Type u} [topological_space H]
    (G : structure_groupoid H) {e : local_homeomorph H H} {e' : local_homeomorph H H} (he : e ∈ G)
    (h : e' ≈ e) : e' ∈ G :=
  structure_groupoid.eq_on_source' G e e' he h

/-- Partial order on the set of groupoids, given by inclusion of the members of the groupoid -/
protected instance structure_groupoid.partial_order {H : Type u} [topological_space H] :
    partial_order (structure_groupoid H) :=
  partial_order.lift structure_groupoid.members sorry

theorem structure_groupoid.le_iff {H : Type u} [topological_space H] {G₁ : structure_groupoid H}
    {G₂ : structure_groupoid H} : G₁ ≤ G₂ ↔ ∀ (e : local_homeomorph H H), e ∈ G₁ → e ∈ G₂ :=
  iff.rfl
    ...
    (singleton (local_homeomorph.refl H) ∪
      set_of
        fun (e : local_homeomorph H H) =>
          local_equiv.source (local_homeomorph.to_local_equiv e) = ∅)
    sorry sorry sorry sorry sorry

/-- Every structure groupoid contains the identity groupoid -/
protected instance structure_groupoid.order_bot {H : Type u} [topological_space H] :
    order_bot (structure_groupoid H) :=
  order_bot.mk (id_groupoid H) partial_order.le partial_order.lt sorry sorry sorry sorry

protected instance structure_groupoid.inhabited (H : Type u) [topological_space H] :
    Inhabited (structure_groupoid H) :=
...
structure pregroupoid (H : Type u_5) [topological_space H] where
  property : (H → H) → set H → Prop
  comp :
    ∀ {f g : H → H} {u v : set H},
      property f u →
        property g v →
          is_open u → is_open v → is_open (u ∩ f ⁻¹' v) → property (g ∘ f) (u ∩ f ⁻¹' v)
  id_mem : property id set.univ
  locality :
    ∀ {f : H → H} {u : set H},
      is_open u →
        (∀ (x : H), x ∈ u → ∃ (v : set H), is_open v ∧ x ∈ v ∧ property f (u ∩ v)) → property f u
  congr :
    ∀ {f g : H → H} {u : set H},
      is_open u → (∀ (x : H), x ∈ u → g x = f x) → property f u → property g u

/-- Construct a groupoid of local homeos for which the map and its inverse have some property,
from a pregroupoid asserting that this property is stable under composition. -/
def pregroupoid.groupoid {H : Type u} [topological_space H] (PG : pregroupoid H) :
    structure_groupoid H :=
  structure_groupoid.mk
    (set_of
      fun (e : local_homeomorph H H) =>
        pregroupoid.property PG (⇑e) (local_equiv.source (local_homeomorph.to_local_equiv e)) ∧
          pregroupoid.property PG (⇑(local_homeomorph.symm e))
            (local_equiv.target (local_homeomorph.to_local_equiv e)))
    sorry sorry sorry sorry sorry

theorem mem_groupoid_of_pregroupoid {H : Type u} [topological_space H] {PG : pregroupoid H}
    {e : local_homeomorph H H} :
...
theorem groupoid_of_pregroupoid_le {H : Type u} [topological_space H] (PG₁ : pregroupoid H)
    (PG₂ : pregroupoid H)
    (h : ∀ (f : H → H) (s : set H), pregroupoid.property PG₁ f s → pregroupoid.property PG₂ f s) :
    pregroupoid.groupoid PG₁ ≤ pregroupoid.groupoid PG₂ :=
  sorry

theorem mem_pregroupoid_of_eq_on_source {H : Type u} [topological_space H] (PG : pregroupoid H)
    {e : local_homeomorph H H} {e' : local_homeomorph H H} (he' : e ≈ e')
    (he : pregroupoid.property PG (⇑e) (local_equiv.source (local_homeomorph.to_local_equiv e))) :
    pregroupoid.property PG (⇑e') (local_equiv.source (local_homeomorph.to_local_equiv e')) :=
  sorry
...
protected instance pregroupoid.inhabited (H : Type u_1) [topological_space H] :
    Inhabited (pregroupoid H) :=
...
protected instance structure_groupoid.order_top {H : Type u} [topological_space H] :
    order_top (structure_groupoid H) :=
...
class closed_under_restriction {H : Type u} [topological_space H] (G : structure_groupoid H) where
  closed_under_restriction :
    ∀ {e : local_homeomorph H H}, e ∈ G → ∀ (s : set H), is_open s → local_homeomorph.restr e s ∈ G

theorem closed_under_restriction' {H : Type u} [topological_space H] {G : structure_groupoid H}
    [closed_under_restriction G] {e : local_homeomorph H H} (he : e ∈ G) {s : set H}
    (hs : is_open s) : local_homeomorph.restr e s ∈ G :=
...
def id_restr_groupoid {H : Type u} [topological_space H] : structure_groupoid H :=
  structure_groupoid.mk
    (set_of
      fun (e : local_homeomorph H H) =>
        Exists fun {s : set H} => ∃ (h : is_open s), e ≈ local_homeomorph.of_set s h)
    sorry sorry sorry sorry sorry

theorem id_restr_groupoid_mem {H : Type u} [topological_space H] {s : set H} (hs : is_open s) :
    local_homeomorph.of_set s hs ∈ id_restr_groupoid :=
...
protected instance closed_under_restriction_id_restr_groupoid {H : Type u} [topological_space H] :
    closed_under_restriction id_restr_groupoid :=
  sorry

/-- A groupoid is closed under restriction if and only if it contains the trivial restriction-closed
groupoid. -/
theorem closed_under_restriction_iff_id_le {H : Type u} [topological_space H]
    (G : structure_groupoid H) : closed_under_restriction G ↔ id_restr_groupoid ≤ G :=
  sorry

/-- The groupoid of all local homeomorphisms on a topological space `H` is closed under restriction.
-/
protected instance continuous_groupoid.closed_under_restriction {H : Type u} [topological_space H] :
    closed_under_restriction (continuous_groupoid H) :=
  iff.mpr (closed_under_restriction_iff_id_le (continuous_groupoid H))
    (eq.mpr
      ((fun (ᾰ ᾰ_1 : structure_groupoid H) (e_2 : ᾰ = ᾰ_1) (ᾰ_2 ᾰ_3 : structure_groupoid H)
...
-/
class charted_space (H : Type u_5) [topological_space H] (M : Type u_6) [topological_space M] where
  atlas : set (local_homeomorph M H)
  chart_at : M → local_homeomorph M H
  mem_chart_source :
    ∀ (x : M), x ∈ local_equiv.source (local_homeomorph.to_local_equiv (chart_at x))
...
protected instance charted_space_self (H : Type u_1) [topological_space H] : charted_space H H :=
  charted_space.mk (singleton (local_homeomorph.refl H)) (fun (x : H) => local_homeomorph.refl H)
    sorry sorry
...
@[simp] theorem charted_space_self_atlas {H : Type u_1} [topological_space H]
    {e : local_homeomorph H H} : e ∈ charted_space.atlas H H ↔ e = local_homeomorph.refl H :=
  sorry

/-- In the model space, chart_at is always the identity -/
@[simp] theorem chart_at_self_eq {H : Type u_1} [topological_space H] {x : H} :
...
def model_prod (H : Type u_1) (H' : Type u_2) := H × H'

protected instance model_prod_inhabited {α : Type u_1} {β : Type u_2} [Inhabited α] [Inhabited β] :
    Inhabited (model_prod α β) :=
  { default := (Inhabited.default, Inhabited.default) }

protected instance model_prod.topological_space (H : Type u_1) [topological_space H] (H' : Type u_2)
    [topological_space H'] : topological_space (model_prod H H') :=
  prod.topological_space

/- Next lemma shows up often when dealing with derivatives, register it as simp. -/

@[simp] theorem model_prod_range_prod_id {H : Type u_1} {H' : Type u_2} {α : Type u_3} (f : H → α) :
    (set.range fun (p : model_prod H H') => (f (prod.fst p), prod.snd p)) =
        set.prod (set.range f) set.univ :=
  sorry

/-- The product of two charted spaces is naturally a charted space, with the canonical
construction of the atlas of product maps. -/
protected instance prod_charted_space (H : Type u_1) [topological_space H] (M : Type u_2)
    [topological_space M] [charted_space H M] (H' : Type u_3) [topological_space H'] (M' : Type u_4)
    [topological_space M'] [charted_space H' M'] : charted_space (model_prod H H') (M × M') :=
  charted_space.mk
    (set_of
      fun (f : local_homeomorph (M × M') (model_prod H H')) =>
        ∃ (g : local_homeomorph M H),
          ∃ (H_1 : g ∈ charted_space.atlas H M),
            ∃ (h : local_homeomorph M' H'),
              ∃ (H_2 : h ∈ charted_space.atlas H' M'), f = local_homeomorph.prod g h)
    (fun (x : M × M') =>
      local_homeomorph.prod (charted_space.chart_at H (prod.fst x))
        (charted_space.chart_at H' (prod.snd x)))
    sorry sorry

@[simp] theorem prod_charted_space_chart_at {H : Type u} {H' : Type u_1} {M : Type u_2}
    {M' : Type u_3} [topological_space H] [topological_space M] [charted_space H M]
    [topological_space H'] [topological_space M'] [charted_space H' M'] {x : M × M'} :
    charted_space.chart_at (model_prod H H') x =
        local_homeomorph.prod (charted_space.chart_at H (prod.fst x))
          (charted_space.chart_at H' (prod.snd x)) :=
  rfl
...
structure charted_space_core (H : Type u_5) [topological_space H] (M : Type u_6) where
  ...
  open_source :
    ∀ (e e' : local_equiv M H),
      e ∈ atlas →
        e' ∈ atlas → is_open (local_equiv.source (local_equiv.trans (local_equiv.symm e) e'))
  continuous_to_fun :
    ∀ (e e' : local_equiv M H),
      e ∈ atlas →
        e' ∈ atlas →
...
protected def to_topological_space {H : Type u} {M : Type u_2} [topological_space H]
    (c : charted_space_core H M) : topological_space M :=
      ...
      fun (e : local_equiv M H) =>
        set.Union
          fun (he : e ∈ atlas c) =>
            set.Union
              fun (s : set H) =>
                set.Union fun (s_open : is_open s) => singleton (⇑e ⁻¹' s ∩ local_equiv.source e))

theorem open_source' {H : Type u} {M : Type u_2} [topological_space H] (c : charted_space_core H M)
    {e : local_equiv M H} (he : e ∈ atlas c) : is_open (local_equiv.source e) :=
  sorry

theorem open_target {H : Type u} {M : Type u_2} [topological_space H] (c : charted_space_core H M)
    {e : local_equiv M H} (he : e ∈ atlas c) : is_open (local_equiv.target e) :=
  sorry
...
definition. -/
def local_homeomorph {H : Type u} {M : Type u_2} [topological_space H] (c : charted_space_core H M)
    (e : local_equiv M H) (he : e ∈ atlas c) : local_homeomorph M H :=
  local_homeomorph.mk
    (local_equiv.mk (local_equiv.to_fun e) (local_equiv.inv_fun e) (local_equiv.source e)
      (local_equiv.target e) (local_equiv.map_source' e) (local_equiv.map_target' e)
      (local_equiv.left_inv' e) (local_equiv.right_inv' e))
    sorry sorry sorry sorry

/-- Given a charted space without topology, endow it with a genuine charted space structure with
respect to the topology constructed from the atlas. -/
def to_charted_space {H : Type u} {M : Type u_2} [topological_space H]
    (c : charted_space_core H M) : charted_space H M :=
  charted_space.mk
    (set.Union
      fun (e : local_equiv M H) =>
        set.Union fun (he : e ∈ atlas c) => singleton (local_homeomorph c e he))
...
groupoid -/
class has_groupoid {H : Type u_5} [topological_space H] (M : Type u_6) [topological_space M]
    [charted_space H M] (G : structure_groupoid H)
    where
  compatible :
    ∀ {e e' : local_homeomorph M H},
      e ∈ charted_space.atlas H M →
        e' ∈ charted_space.atlas H M → local_homeomorph.trans (local_homeomorph.symm e) e' ∈ G

/-- Reformulate in the `structure_groupoid` namespace the compatibility condition of charts in a
charted space admitting a structure groupoid, to make it more easily accessible with dot
notation. -/
theorem structure_groupoid.compatible {H : Type u_1} [topological_space H]
    (G : structure_groupoid H) {M : Type u_2} [topological_space M] [charted_space H M]
    [has_groupoid M G] {e : local_homeomorph M H} {e' : local_homeomorph M H}
    (he : e ∈ charted_space.atlas H M) (he' : e' ∈ charted_space.atlas H M) :
    local_homeomorph.trans (local_homeomorph.symm e) e' ∈ G :=
  has_groupoid.compatible G he he'

theorem has_groupoid_of_le {H : Type u} {M : Type u_2} [topological_space H] [topological_space M]
    [charted_space H M] {G₁ : structure_groupoid H} {G₂ : structure_groupoid H}
    (h : has_groupoid M G₁) (hle : G₁ ≤ G₂) : has_groupoid M G₂ :=
  has_groupoid.mk
    fun (e e' : local_homeomorph M H) (he : e ∈ charted_space.atlas H M)
      (he' : e' ∈ charted_space.atlas H M) => hle (has_groupoid.compatible G₁ he he')

theorem has_groupoid_of_pregroupoid {H : Type u} {M : Type u_2} [topological_space H]
    [topological_space M] [charted_space H M] (PG : pregroupoid H)
    (h :
      ∀ {e e' : local_homeomorph M H},
        e ∈ charted_space.atlas H M →
          e' ∈ charted_space.atlas H M →
    ...
    has_groupoid M (pregroupoid.groupoid PG) :=
  has_groupoid.mk
    fun (e e' : local_homeomorph M H) (he : e ∈ charted_space.atlas H M)
      (he' : e' ∈ charted_space.atlas H M) =>
      iff.mpr mem_groupoid_of_pregroupoid { left := h he he', right := h he' he }

/-- The trivial charted space structure on the model space is compatible with any groupoid -/
protected instance has_groupoid_model_space (H : Type u_1) [topological_space H]
...
protected instance has_groupoid_continuous_groupoid {H : Type u} {M : Type u_2}
    [topological_space H] [topological_space M] [charted_space H M] :
    has_groupoid M (continuous_groupoid H) :=
  has_groupoid.mk
    fun (e e' : local_homeomorph M H) (he : e ∈ charted_space.atlas H M)
      (he' : e' ∈ charted_space.atlas H M) =>
      eq.mpr
        (id
          (Eq._oldrec
            (Eq.refl (local_homeomorph.trans (local_homeomorph.symm e) e' ∈ continuous_groupoid H))
...
def structure_groupoid.maximal_atlas {H : Type u} (M : Type u_2) [topological_space H]
...
theorem structure_groupoid.mem_maximal_atlas_of_mem_atlas {H : Type u} {M : Type u_2}
    [topological_space H] [topological_space M] [charted_space H M] (G : structure_groupoid H)
    [has_groupoid M G] {e : local_homeomorph M H} (he : e ∈ charted_space.atlas H M) :
    e ∈ structure_groupoid.maximal_atlas M G :=
  fun (e' : local_homeomorph M H) (he' : e' ∈ charted_space.atlas H M) =>
    { left := structure_groupoid.compatible G he he',
      right := structure_groupoid.compatible G he' he }

theorem structure_groupoid.chart_mem_maximal_atlas {H : Type u} {M : Type u_2} [topological_space H]
    [topological_space M] [charted_space H M] (G : structure_groupoid H) [has_groupoid M G]
    (x : M) : charted_space.chart_at H x ∈ structure_groupoid.maximal_atlas M G :=
  structure_groupoid.mem_maximal_atlas_of_mem_atlas G (charted_space.chart_mem_atlas H x)

theorem mem_maximal_atlas_iff {H : Type u} {M : Type u_2} [topological_space H]
    [topological_space M] [charted_space H M] {G : structure_groupoid H}
    {e : local_homeomorph M H} :
    e ∈ structure_groupoid.maximal_atlas M G ↔
        ∀ (e' : local_homeomorph M H),
...
theorem structure_groupoid.compatible_of_mem_maximal_atlas {H : Type u} {M : Type u_2}
    [topological_space H] [topological_space M] [charted_space H M] {G : structure_groupoid H}
...
theorem structure_groupoid.id_mem_maximal_atlas {H : Type u} [topological_space H]
    (G : structure_groupoid H) : local_homeomorph.refl H ∈ structure_groupoid.maximal_atlas H G :=
  structure_groupoid.mem_maximal_atlas_of_mem_atlas G
    (eq.mpr
      (id
        (Eq.trans (propext charted_space_self_atlas)
          (propext (eq_self_iff_true (local_homeomorph.refl H)))))
      trivial)
...
def singleton_charted_space {H : Type u} [topological_space H] {α : Type u_5} [topological_space α]
    (e : local_homeomorph α H)
    (h : local_equiv.source (local_homeomorph.to_local_equiv e) = set.univ) : charted_space H α :=
  charted_space.mk (singleton e) (fun (_x : α) => e) sorry sorry

theorem singleton_charted_space_one_chart {H : Type u} [topological_space H] {α : Type u_5}
    [topological_space α] (e : local_homeomorph α H)
    (h : local_equiv.source (local_homeomorph.to_local_equiv e) = set.univ)
    (e' : local_homeomorph α H) (h' : e' ∈ charted_space.atlas H α) : e' = e :=
  h'
...
theorem singleton_has_groupoid {H : Type u} [topological_space H] {α : Type u_5}
    [topological_space α] (e : local_homeomorph α H)
    (h : local_equiv.source (local_homeomorph.to_local_equiv e) = set.univ)
    (G : structure_groupoid H) [closed_under_restriction G] : has_groupoid α G :=
  sorry
...
protected instance charted_space {H : Type u} {M : Type u_2} [topological_space H]
    [topological_space M] [charted_space H M] (s : opens M) : charted_space H ↥s :=
  charted_space.mk
    (set.Union
      fun (x : ↥s) =>
        singleton (local_homeomorph.subtype_restr (charted_space.chart_at H (subtype.val x)) s))
    (fun (x : ↥s) => local_homeomorph.subtype_restr (charted_space.chart_at H (subtype.val x)) s)
    sorry sorry
...
protected instance has_groupoid {H : Type u} {M : Type u_2} [topological_space H]
    [topological_space M] [charted_space H M] (G : structure_groupoid H) [has_groupoid M G]
    (s : opens M) [closed_under_restriction G] : has_groupoid (↥s) G :=
  sorry
...
structure structomorph {H : Type u} [topological_space H] (G : structure_groupoid H) (M : Type u_5)
    (M' : Type u_6) [topological_space M] [topological_space M'] [charted_space H M]
    [charted_space H M']
    extends M ≃ₜ M' where
  mem_groupoid :
    ∀ (c : local_homeomorph M H) (c' : local_homeomorph M' H),
...
def structomorph.refl {H : Type u} [topological_space H] {G : structure_groupoid H} (M : Type u_1)
    [topological_space M] [charted_space H M] [has_groupoid M G] : structomorph G M M :=
  structomorph.mk (homeomorph.mk (homeomorph.to_equiv (homeomorph.refl M))) sorry

/-- The inverse of a structomorphism is a structomorphism -/
def structomorph.symm {H : Type u} {M : Type u_2} {M' : Type u_3} [topological_space H]
    [topological_space M] [charted_space H M] [topological_space M'] {G : structure_groupoid H}
    [charted_space H M'] (e : structomorph G M M') : structomorph G M' M :=
  structomorph.mk
    (homeomorph.mk (homeomorph.to_equiv (homeomorph.symm (structomorph.to_homeomorph e)))) sorry

/-- The composition of structomorphisms is a structomorphism -/
def structomorph.trans {H : Type u} {M : Type u_2} {M' : Type u_3} {M'' : Type u_4}
    [topological_space H] [topological_space M] [charted_space H M] [topological_space M']
    [topological_space M''] {G : structure_groupoid H} [charted_space H M'] [charted_space H M'']
    (e : structomorph G M M') (e' : structomorph G M' M'') : structomorph G M M'' :=
  structomorph.mk
    (homeomorph.mk
      (homeomorph.to_equiv
        (homeomorph.trans (structomorph.to_homeomorph e) (structomorph.to_homeomorph e'))))
    sorry
