...
theorem hahn_decomposition {α : Type u_1} [measurable_space α] {μ : measure α} {ν : measure α} (hμ : coe_fn μ set.univ < ⊤) (hν : coe_fn ν set.univ < ⊤) : ∃ (s : set α),
  is_measurable s ∧
    (∀ (t : set α), is_measurable t → t ⊆ s → coe_fn ν t ≤ coe_fn μ t) ∧
      ∀ (t : set α), is_measurable t → t ⊆ (sᶜ) → coe_fn μ t ≤ coe_fn ν t := sorry
