...
def shift_up : ℤ × ℤ ↪ ℤ × ℤ :=
  function.embedding.prod_map (function.embedding.refl ℤ)
    (function.embedding.mk (fun (n : ℤ) => n + 1) sorry)

/-- The embedding `(x, y) ↦ (x+1, y)`. -/
def shift_right : ℤ × ℤ ↪ ℤ × ℤ :=
  function.embedding.prod_map (function.embedding.mk (fun (n : ℤ) => n + 1) sorry)
    (function.embedding.refl ℤ)
...
def left (b : board) : finset (ℤ × ℤ) := b ∩ finset.map shift_up b

def right (b : board) : finset (ℤ × ℤ) := b ∩ finset.map shift_right b
...
theorem card_of_mem_right {b : board} {m : ℤ × ℤ} (h : m ∈ right b) : bit0 1 ≤ finset.card b :=
  sorry

theorem move_left_card {b : board} {m : ℤ × ℤ} (h : m ∈ left b) :
    finset.card (move_left b m) + bit0 1 = finset.card b :=
  sorry

theorem move_right_card {b : board} {m : ℤ × ℤ} (h : m ∈ right b) :
    finset.card (move_right b m) + bit0 1 = finset.card b :=
  sorry

theorem move_left_smaller {b : board} {m : ℤ × ℤ} (h : m ∈ left b) :
    finset.card (move_left b m) / bit0 1 < finset.card b / bit0 1 :=
  sorry

theorem move_right_smaller {b : board} {m : ℤ × ℤ} (h : m ∈ right b) :
    finset.card (move_right b m) / bit0 1 < finset.card b / bit0 1 :=
  sorry

/-- The instance describing allowed moves on a Domineering board. -/
protected instance state : state board :=
  state.mk (fun (s : board) => finset.card s / bit0 1)
    (fun (s : board) => finset.image (move_left s) (left s))
...
def domineering (b : domineering.board) : pgame := of b
...
def domineering.one : pgame := domineering (list.to_finset [(0, 0), (0, 1)])

/-- The `L` shaped Domineering board, in which Left is exactly half a move ahead. -/
def domineering.L : pgame := domineering (list.to_finset [(0, bit0 1), (0, 1), (0, 0), (1, 0)])
