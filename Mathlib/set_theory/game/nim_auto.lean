...
theorem nim_def (O : ordinal) :
    nim O =
        mk (Well_order.α (ordinal.out O)) (Well_order.α (ordinal.out O))
          (fun (O₂ : Well_order.α (ordinal.out O)) =>
            nim (ordinal.typein (Well_order.r (ordinal.out O)) O₂))
          fun (O₂ : Well_order.α (ordinal.out O)) =>
            nim (ordinal.typein (Well_order.r (ordinal.out O)) O₂) :=
  sorry

theorem nim_wf_lemma {O₁ : ordinal} (O₂ : Well_order.α (ordinal.out O₁)) :
    ordinal.typein (Well_order.r (ordinal.out O₁)) O₂ < O₁ :=
  sorry

protected instance nim_impartial (O : ordinal) : impartial (nim O) := sorry

theorem exists_ordinal_move_left_eq (O : ordinal) (i : left_moves (nim O)) :
    ∃ (O' : ordinal), ∃ (H : O' < O), move_left (nim O) i = nim O' :=
  sorry

theorem exists_move_left_eq (O : ordinal) (O' : ordinal) (H : O' < O) :
    ∃ (i : left_moves (nim O)), move_left (nim O) i = nim O' :=
  sorry
...
theorem sum_first_loses_iff_eq (O₁ : ordinal) (O₂ : ordinal) :
    first_loses (nim O₁ + nim O₂) ↔ O₁ = O₂ :=
  sorry

theorem sum_first_wins_iff_neq (O₁ : ordinal) (O₂ : ordinal) :
    first_wins (nim O₁ + nim O₂) ↔ O₁ ≠ O₂ :=
  sorry
...
def grundy_value (G : pgame) [impartial G] : ordinal := sorry

theorem grundy_value_def (G : pgame) [impartial G] :
    grundy_value G =
        ordinal.omin (nonmoves fun (i : left_moves G) => grundy_value (move_left G i))
          (nonmoves_nonempty fun (i : left_moves G) => grundy_value (move_left G i)) :=
  sorry
...
theorem equiv_nim_iff_grundy_value_eq (G : pgame) [impartial G] (O : ordinal) :
    equiv G (nim O) ↔ grundy_value G = O :=
  sorry

theorem nim.grundy_value (O : ordinal) : grundy_value (nim O) = O :=
  eq.mpr
    (id
      (Eq._oldrec (Eq.refl (grundy_value (nim O) = O))
        (Eq.symm (propext (equiv_nim_iff_grundy_value_eq (nim O) O)))))
    (equiv_refl (nim O))

theorem equiv_iff_grundy_value_eq (G : pgame) (H : pgame) [impartial G] [impartial H] :
    equiv G H ↔ grundy_value G = grundy_value H :=
  iff.trans (iff.mp equiv_congr_left (equiv_nim_grundy_value H) G)
    (equiv_nim_iff_grundy_value_eq G (grundy_value H))
...
theorem equiv_zero_iff_grundy_value (G : pgame) [impartial G] : equiv G 0 ↔ grundy_value G = 0 :=
  eq.mpr
    (id
      (Eq._oldrec (Eq.refl (equiv G 0 ↔ grundy_value G = 0))
        (propext (equiv_iff_grundy_value_eq G 0))))
    (eq.mpr
      (id
        (Eq._oldrec (Eq.refl (grundy_value G = grundy_value 0 ↔ grundy_value G = 0))
          grundy_value_zero))
      (iff.refl (grundy_value G = 0)))

theorem grundy_value_nim_add_nim (n : ℕ) (m : ℕ) :
    grundy_value (nim ↑n + nim ↑m) = ↑(nat.lxor n m) :=
  sorry

theorem nim_add_nim_equiv {n : ℕ} {m : ℕ} : equiv (nim ↑n + nim ↑m) (nim ↑(nat.lxor n m)) := sorry

theorem grundy_value_add (G : pgame) (H : pgame) [impartial G] [impartial H] {n : ℕ} {m : ℕ}
    (hG : grundy_value G = ↑n) (hH : grundy_value H = ↑m) :
    grundy_value (G + H) = ↑(nat.lxor n m) :=
  sorry
