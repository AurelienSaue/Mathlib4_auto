...
def inv_val {l : Type u_1} {r : Type u_1} (L : l → pgame) (R : r → pgame) (IHl : l → pgame)
    (IHr : r → pgame) {b : Bool} : inv_ty l r b → pgame :=
  sorry
...
def inv' : pgame → pgame := sorry

/-- The inverse of a surreal number in terms of the inverse on positive surreals. -/
def inv (x : pgame) : pgame := ite (x = 0) 0 (ite (0 < x) (inv' x) (inv' (-x)))

protected instance has_inv : has_inv pgame := has_inv.mk inv

protected instance has_div : Div pgame := { div := fun (x y : pgame) => x * (y⁻¹) }

/-- A pre-game is numeric if everything in the L set is less than everything in the R set,
and all the elements of L and R are also numeric. -/
def numeric : pgame → Prop := sorry

theorem numeric.move_left {x : pgame} (o : numeric x) (i : left_moves x) :
    numeric (move_left x i) :=
  sorry

theorem numeric.move_right {x : pgame} (o : numeric x) (j : right_moves x) :
    numeric (move_right x j) :=
  sorry

theorem numeric_rec {C : pgame → Prop}
    (H :
      ∀ (l r : Type u_1) (L : l → pgame) (R : r → pgame),
        (∀ (i : l) (j : r), L i < R j) →
          (∀ (i : l), numeric (L i)) →
            (∀ (i : r), numeric (R i)) →
              (∀ (i : l), C (L i)) → (∀ (i : r), C (R i)) → C (mk l r L R))
    (x : pgame) : numeric x → C x :=
  sorry
...
theorem lt_iff_le_not_le {x : pgame} {y : pgame} (ox : numeric x) (oy : numeric y) :
    x < y ↔ x ≤ y ∧ ¬y ≤ x :=
...
theorem numeric.lt_move_right {x : pgame} (o : numeric x) (j : right_moves x) :
    x < move_right x j :=
  eq.mpr (id (Eq._oldrec (Eq.refl (x < move_right x j)) (propext lt_def_le)))
    (Or.inr (Exists.intro j (id (le_refl (move_right x j)))))

theorem numeric.le_move_right {x : pgame} (o : numeric x) (j : right_moves x) :
    x ≤ move_right x j :=
  le_of_lt o (numeric.move_right o j) (numeric.lt_move_right o j)

theorem add_lt_add {w : pgame} {x : pgame} {y : pgame} {z : pgame} (ow : numeric w) (ox : numeric x)
    (oy : numeric y) (oz : numeric z) (hwx : w < x) (hyz : y < z) : w + y < x + z :=
  sorry

theorem numeric_add {x : pgame} {y : pgame} (ox : numeric x) (oy : numeric y) : numeric (x + y) :=
  sorry
...
def surreal.equiv (x : Subtype fun (x : pgame) => pgame.numeric x)
    (y : Subtype fun (x : pgame) => pgame.numeric x) :=
  pgame.equiv (subtype.val x) (subtype.val y)

protected instance surreal.setoid : setoid (Subtype fun (x : pgame) => pgame.numeric x) :=
  setoid.mk
    (fun (x y : Subtype fun (x : pgame) => pgame.numeric x) =>
      pgame.equiv (subtype.val x) (subtype.val y))
    sorry
...
def surreal := quotient sorry

namespace surreal


/-- Construct a surreal number from a numeric pre-game. -/
def mk (x : pgame) (h : pgame.numeric x) : surreal := quotient.mk { val := x, property := h }
...
protected instance inhabited : Inhabited surreal := { default := 0 }

/-- Lift an equivalence-respecting function on pre-games to surreals. -/
def lift {α : Sort u_1} (f : (x : pgame) → pgame.numeric x → α)
    (H :
      ∀ {x y : pgame} (hx : pgame.numeric x) (hy : pgame.numeric y),
        pgame.equiv x y → f x hx = f y hy) :
    surreal → α :=
  quotient.lift (fun (x : Subtype fun (x : pgame) => pgame.numeric x) => f (subtype.val x) sorry)
    sorry

/-- Lift a binary equivalence-respecting function on pre-games to surreals. -/
def lift₂ {α : Sort u_1} (f : (x : pgame) → (y : pgame) → pgame.numeric x → pgame.numeric y → α)
    (H :
      ∀ {x₁ : pgame} {y₁ : pgame} {x₂ : pgame} {y₂ : pgame} (ox₁ : pgame.numeric x₁)
        (oy₁ : pgame.numeric y₁) (ox₂ : pgame.numeric x₂) (oy₂ : pgame.numeric y₂),
        pgame.equiv x₁ x₂ → pgame.equiv y₁ y₂ → f x₁ y₁ ox₁ oy₁ = f x₂ y₂ ox₂ oy₂) :
    surreal → surreal → α :=
  lift
    (fun (x : pgame) (ox : pgame.numeric x) =>
      lift (fun (y : pgame) (oy : pgame.numeric y) => f x y ox oy) sorry)
    sorry
...
protected instance preorder : preorder surreal := preorder.mk le lt sorry sorry
...
protected instance has_add : Add surreal := { add := add }

theorem add_assoc (x : surreal) (y : surreal) (z : surreal) : x + y + z = x + (y + z) := sorry

protected instance add_semigroup : add_semigroup surreal := add_semigroup.mk Add.add add_assoc
