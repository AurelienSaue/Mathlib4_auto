...
protected instance subtype.decidable_eq {α : Type u} {p : α → Prop} [DecidableEq α] :
    DecidableEq (Subtype fun (x : α) => p x) :=
  id
        ...
        fun (val : α) (property : p val) (w : Subtype fun (x : α) => p x) =>
          subtype.cases_on w
            fun (w_val : α) (w_property : p w_val) =>
              decidable.by_cases
                (fun (ᾰ : val = w_val) =>
                  Eq._oldrec (fun (w_property : p val) => is_true sorry) ᾰ w_property)
