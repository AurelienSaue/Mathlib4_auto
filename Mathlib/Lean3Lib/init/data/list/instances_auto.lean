...
protected instance list.monad : Monad List :=
  { toApplicative :=
      { toFunctor :=
          { map := list.map, mapConst := fun (α β : Type u_1) => list.map ∘ function.const β },
        toPure := { pure := list.ret },
        toSeq :=
          { seq :=
              fun (α β : Type u_1) (f : List (α → β)) (x : List α) =>
                list.bind f fun (_x : α → β) => list.map _x x },
        toSeqLeft :=
          { seqLeft :=
              fun (α β : Type u_1) (a : List α) (b : List β) =>
                (fun (α β : Type u_1) (f : List (α → β)) (x : List α) =>
                    list.bind f fun (_x : α → β) => list.map _x x)
                  β α (list.map (function.const β) a) b },
        toSeqRight :=
          { seqRight :=
              fun (α β : Type u_1) (a : List α) (b : List β) =>
                (fun (α β : Type u_1) (f : List (α → β)) (x : List α) =>
                    list.bind f fun (_x : α → β) => list.map _x x)
                  β β (list.map (function.const α id) a) b } },
    toBind := { bind := list.bind } }

protected instance list.is_lawful_monad : is_lawful_monad List := sorry

protected instance list.alternative : alternative List := alternative.mk List.nil
...
protected instance decidable_bex {α : Type u} (p : α → Prop) [decidable_pred p] (l : List α) :
    Decidable (∃ (x : α), ∃ (H : x ∈ l), p x) :=
  sorry

protected instance decidable_ball {α : Type u} (p : α → Prop) [decidable_pred p] (l : List α) :
