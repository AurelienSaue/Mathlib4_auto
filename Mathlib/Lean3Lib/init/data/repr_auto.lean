...
def repr {α : Type u} [has_repr α] : α → string := has_repr.repr
...
protected def list.repr_aux {α : Type u} [has_repr α] : Bool → List α → string := sorry

protected def list.repr {α : Type u} [has_repr α] : List α → string := sorry
...
protected instance sum.has_repr {α : Type u} {β : Type v} [has_repr α] [has_repr β] :
    has_repr (α ⊕ β) :=
  has_repr.mk fun (s : α ⊕ β) => sorry

protected instance prod.has_repr {α : Type u} {β : Type v} [has_repr α] [has_repr β] :
    has_repr (α × β) :=
  has_repr.mk fun (_x : α × β) => sorry

protected instance sigma.has_repr {α : Type u} {β : α → Type v} [has_repr α]
    [s : (x : α) → has_repr (β x)] : has_repr (sigma β) :=
  has_repr.mk fun (_x : sigma β) => sorry

protected instance subtype.has_repr {α : Type u} {p : α → Prop} [has_repr α] :
    has_repr (Subtype p) :=
...
def digit_succ (base : ℕ) : List ℕ → List ℕ := sorry

def to_digits (base : ℕ) : ℕ → List ℕ := sorry
...
protected instance nat.has_repr : has_repr ℕ := has_repr.mk nat.repr

def hex_digit_repr (n : ℕ) : string := string.singleton (nat.digit_char n)
...
def string.quote_aux : List char → string := sorry
...
def string.quote (s : string) : string :=
  ite (string.is_empty s = tt)
    ...
    (string.str string.empty (char.of_nat (bit0 (bit1 (bit0 (bit0 (bit0 1)))))) ++
        string.quote_aux (string.to_list s) ++
      string.str string.empty (char.of_nat (bit0 (bit1 (bit0 (bit0 (bit0 1)))))))

protected instance string.has_repr : has_repr string := has_repr.mk string.quote
...
def char.repr (c : char) : string := repr c
