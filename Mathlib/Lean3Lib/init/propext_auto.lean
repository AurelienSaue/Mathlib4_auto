...
theorem forall_congr_eq {a : Sort u} {p : a → Prop} {q : a → Prop} (h : ∀ (x : a), p x = q x) :
    (∀ (x : a), p x) = ∀ (x : a), q x :=
...
theorem imp_congr_ctx_eq {a : Prop} {b : Prop} {c : Prop} {d : Prop} (h₁ : a = c) (h₂ : c → b = d) :
    (a → b) = (c → d) :=
  propext (imp_congr_ctx (eq.to_iff h₁) fun (hc : c) => eq.to_iff (h₂ hc))

theorem eq_true_intro {a : Prop} (h : a) : a = True := propext (iff_true_intro h)

theorem eq_false_intro {a : Prop} (h : ¬a) : a = False := propext (iff_false_intro h)

theorem iff.to_eq {a : Prop} {b : Prop} (h : a ↔ b) : a = b := propext h
