...
def has_bind.and_then {α : Type u} {β : Type u} {m : Type u → Type v} [Bind m] (x : m α) (y : m β) :
    m β :=
  do 
    x 
    y
...
def return {m : Type u → Type v} [Monad m] {α : Type u} : α → m α := pure

/- Identical to has_bind.and_then, but it is not inlined. -/

def has_bind.seq {α : Type u} {β : Type u} {m : Type u → Type v} [Bind m] (x : m α) (y : m β) :
    m β :=
  do 
    x 
    y
