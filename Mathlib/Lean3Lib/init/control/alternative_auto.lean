...
class has_orelse (f : Type u → Type v) where
  orelse : {α : Type u} → f α → f α → f α

infixr:2 " <|> " => Mathlib.has_orelse.orelse

class alternative (f : Type u → Type v) extends has_orelse f, Applicative f where
  failure : {α : Type u} → f α

def failure {f : Type u → Type v} [alternative f] {α : Type u} : f α := alternative.failure
...
def guardb {f : Type → Type v} [alternative f] : Bool → f Unit := sorry
