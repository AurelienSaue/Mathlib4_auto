...
class preorder (α : Type u) extends HasLess α, HasLessEq α where
  le : α → α → Prop
  lt : α → α → Prop
  le_refl : ∀ (a : α), a ≤ a
  le_trans : ∀ (a b c : α), a ≤ b → b ≤ c → a ≤ c
  lt_iff_le_not_le :
    autoParam (∀ (a b : α), a < b ↔ a ≤ b ∧ ¬b ≤ a)
...
theorem le_refl {α : Type u} [preorder α] (a : α) : a ≤ a := preorder.le_refl
...
theorem lt_of_le_not_le {α : Type u} [preorder α] {a : α} {b : α} : a ≤ b → ¬b ≤ a → a < b :=
  fun (ᾰ : a ≤ b) (ᾰ_1 : ¬b ≤ a) =>
    idRhs (a < b) (iff.mpr lt_iff_le_not_le { left := ᾰ, right := ᾰ_1 })
...
theorem gt_irrefl {α : Type u} [preorder α] (a : α) : ¬a > a := lt_irrefl
...
theorem lt_of_lt_of_le {α : Type u} [preorder α] {a : α} {b : α} {c : α} : a < b → b ≤ c → a < c :=
  sorry

theorem lt_of_le_of_lt {α : Type u} [preorder α] {a : α} {b : α} {c : α} : a ≤ b → b < c → a < c :=
  sorry
...
protected instance decidable_lt_of_decidable_le {α : Type u} [preorder α] [DecidableRel LessEq] :
    DecidableRel Less :=
  sorry
...
class partial_order (α : Type u) extends preorder α where
  le_antisymm : ∀ (a b : α), a ≤ b → b ≤ a → a = b

theorem le_antisymm {α : Type u} [partial_order α] {a : α} {b : α} : a ≤ b → b ≤ a → a = b :=
  partial_order.le_antisymm

theorem le_antisymm_iff {α : Type u} [partial_order α] {a : α} {b : α} : a = b ↔ a ≤ b ∧ b ≤ a :=
  sorry

theorem lt_or_eq_of_le {α : Type u} [partial_order α] {a : α} {b : α} : a ≤ b → a < b ∨ a = b :=
  sorry
...
protected instance decidable_eq_of_decidable_le {α : Type u} [partial_order α]
    [DecidableRel LessEq] : DecidableEq α :=
  sorry
...
class linear_order (α : Type u) extends partial_order α where
...
theorem lt_or_gt_of_ne {α : Type u} [linear_order α] {a : α} {b : α} (h : a ≠ b) : a < b ∨ a > b :=
  sorry
...
protected instance has_lt.lt.decidable {α : Type u} [linear_order α] (a : α) (b : α) :
    Decidable (a < b) :=
  linear_order.decidable_lt a b

protected instance has_le.le.decidable {α : Type u} [linear_order α] (a : α) (b : α) :
    Decidable (a ≤ b) :=
...
theorem eq_or_lt_of_not_lt {α : Type u} [linear_order α] {a : α} {b : α} (h : ¬a < b) :
    a = b ∨ b < a :=
  dite (a = b) (fun (h₁ : a = b) => Or.inl h₁)
    fun (h₁ : ¬a = b) => Or.inr (lt_of_not_ge fun (hge : b ≥ a) => h (lt_of_le_of_ne hge h₁))

protected instance has_le.le.is_total_preorder {α : Type u} [linear_order α] :
    is_total_preorder α LessEq :=
  is_total_preorder.mk

/- TODO(Leo): decide whether we should keep this instance or not -/

protected instance is_strict_weak_order_of_linear_order {α : Type u} [linear_order α] :
    is_strict_weak_order α Less :=
  is_strict_weak_order_of_is_total_preorder lt_iff_not_ge

/- TODO(Leo): decide whether we should keep this instance or not -/

protected instance is_strict_total_order_of_linear_order {α : Type u} [linear_order α] :
...
theorem lt_or_eq_of_le {α : Type u} [partial_order α] [DecidableRel LessEq] {a : α} {b : α}
    (hab : a ≤ b) : a < b ∨ a = b :=
  dite (b ≤ a) (fun (hba : b ≤ a) => Or.inr (le_antisymm hab hba))
    fun (hba : ¬b ≤ a) => Or.inl (lt_of_le_not_le hab hba)

theorem eq_or_lt_of_le {α : Type u} [partial_order α] [DecidableRel LessEq] {a : α} {b : α}
    (hab : a ≤ b) : a = b ∨ a < b :=
  or.swap (lt_or_eq_of_le hab)

theorem le_iff_lt_or_eq {α : Type u} [partial_order α] [DecidableRel LessEq] {a : α} {b : α} :
    a ≤ b ↔ a < b ∨ a = b :=
...
def lt_by_cases {α : Type u} [linear_order α] (x : α) (y : α) {P : Sort u_1} (h₁ : x < y → P)
    (h₂ : x = y → P) (h₃ : y < x → P) : P :=
