...
class random_gen (g : Type u) where
  range : g → ℕ × ℕ
  next : g → ℕ × g
  split : g → g × g
...
protected instance std_gen.has_repr : has_repr std_gen := has_repr.mk fun (_x : std_gen) => sorry

def std_next : std_gen → ℕ × std_gen := sorry

def std_split : std_gen → std_gen × std_gen := sorry
...
def rand_bool {gen : Type u} [random_gen gen] (g : gen) : Bool × gen := sorry
