...
inductive io.mode where
| read : io.mode
| write : io.mode
| read_write : io.mode
| append : io.mode

inductive io.process.stdio where
...
structure io.process.spawn_args where
  cmd : string
  args : List string
...
class monad_io (m : Type → Type → Type) where
...
class monad_io_terminal (m : Type → Type → Type) where
  put_str : string → m io.error Unit
  get_line : m io.error string
  cmdline_args : List string

class monad_io_net_system (m : Type → Type → Type) [monad_io m] where
  socket : Type
...
class monad_io_file_system (m : Type → Type → Type) [monad_io m] where
...
class monad_io_environment (m : Type → Type → Type) where
...
class monad_io_process (m : Type → Type → Type) [monad_io m] where
  child : Type
...
class monad_io_random (m : Type → Type → Type) where
  set_rand_gen : std_gen → m io.error Unit
  rand : ℕ → ℕ → m io.error ℕ

protected instance monad_io_is_monad (m : Type → Type → Type) (e : Type) [monad_io m] :
    Monad (m e) :=
  monad_io.monad e

protected instance monad_io_is_monad_fail (m : Type → Type → Type) [monad_io m] :
    monad_fail (m io.error) :=
  monad_fail.mk fun (α : Type) (s : string) => monad_io.fail io.error α (io.error.other s)

protected instance monad_io_is_alternative (m : Type → Type → Type) [monad_io m] :
    alternative (m io.error) :=
  alternative.mk
    fun (α : Type) =>
