...
inductive parse_result (α : Type) where
...
def parser (α : Type) := char_buffer → ℕ → parse_result α
...
protected instance is_lawful_monad : is_lawful_monad parser :=
  is_lawful_monad.mk (fun (_x _x_1 : Type) (_x_2 : _x) (_x_3 : _x → parser _x_1) => rfl)
    parser.bind_assoc

protected instance monad_fail : monad_fail parser := monad_fail.mk parser.fail
...
protected instance alternative : alternative parser := alternative.mk parser.failure

protected instance inhabited {α : Type} : Inhabited (parser α) := { default := parser.failure }
...
def eps : parser Unit := return Unit.unit
...
def one_of' (cs : List char) : parser Unit := one_of cs >> eps
...
def many {α : Type} (p : parser α) : parser (List α) := foldr List.cons p []

def many_char (p : parser char) : parser string := list.as_string <$> many p

/-- Matches zero or more occurrences of `p`. -/
def many' {α : Type} (p : parser α) : parser Unit := many p >> eps

/-- Matches one or more occurrences of `p`. -/
def many1 {α : Type} (p : parser α) : parser (List α) := List.cons <$> p <*> many p

/-- Matches one or more occurences of the char parser `p` and implodes them into a string. -/
def many_char1 (p : parser char) : parser string := list.as_string <$> many1 p
...
def fix_core {α : Type} (F : parser α → parser α) (max_depth : ℕ) : parser α := sorry
  ...
  decorate_error
        ...
        (string.str
          (string.str
            (string.str
              (string.str
                (string.str
                  (string.str string.empty (char.of_nat (bit0 (bit0 (bit1 (bit1 (bit1 1)))))))
    ...
    (do 
      let c ←
        sat
          fun (c : char) =>
            char.of_nat (bit0 (bit0 (bit0 (bit0 (bit1 1))))) ≤ c ∧
              c ≤ char.of_nat (bit1 (bit0 (bit0 (bit1 (bit1 1)))))
...
def run {α : Type} (p : parser α) (input : char_buffer) : string ⊕ α := sorry
