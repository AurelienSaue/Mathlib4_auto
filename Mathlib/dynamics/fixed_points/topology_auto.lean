...
theorem is_fixed_pt_of_tendsto_iterate {α : Type u_1} [topological_space α] [t2_space α] {f : α → α} {x : α} {y : α} (hy : filter.tendsto (fun (n : ℕ) => nat.iterate f n x) filter.at_top (nhds y)) (hf : continuous_at f y) : function.is_fixed_pt f y := sorry
