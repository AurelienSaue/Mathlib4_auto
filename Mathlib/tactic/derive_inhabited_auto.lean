...
protected instance ordering.inhabited : Inhabited ordering := { default := ordering.lt }

protected instance bin_tree.inhabited {α : Type u_1} : Inhabited (bin_tree α) :=
  { default := bin_tree.empty }

protected instance unsigned.inhabited : Inhabited unsigned := { default := 0 }
...
protected instance rbtree.inhabited {α : Type u_1}
    {lt :
      autoParam (α → α → Prop)
...
protected instance rbmap.inhabited {α : Type u_1} {β : Type u_2}
    {lt :
      autoParam (α → α → Prop)
        (Lean.Syntax.ident Lean.SourceInfo.none (String.toSubstring "Mathlib.rbtree.default_lt")
          (Lean.Name.mkStr
