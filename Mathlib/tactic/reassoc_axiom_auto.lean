...
def calculated_Prop {α : Sort u_1} (β : Prop) (hh : α) := β
...
```
-/
theorem category_theory.reassoc_of {α : Sort u_1} (hh : α) {β : Prop}
    (x :
      autoParam (tactic.calculated_Prop β hh)
        (Lean.Syntax.ident Lean.SourceInfo.none
          (String.toSubstring "Mathlib.tactic.derive_reassoc_proof")
          (Lean.Name.mkStr
