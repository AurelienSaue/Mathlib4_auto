...
theorem tactic.group.gpow_trick {G : Type u_1} [group G] (a : G) (b : G) (n : ℤ) (m : ℤ) :
    a * b ^ n * b ^ m = a * b ^ (n + m) :=
  eq.mpr
    (id (Eq._oldrec (Eq.refl (a * b ^ n * b ^ m = a * b ^ (n + m))) (mul_assoc a (b ^ n) (b ^ m))))
    (eq.mpr
      (id (Eq._oldrec (Eq.refl (a * (b ^ n * b ^ m) = a * b ^ (n + m))) (Eq.symm (gpow_add b n m))))
      (Eq.refl (a * b ^ (n + m))))

theorem tactic.group.gpow_trick_one {G : Type u_1} [group G] (a : G) (b : G) (m : ℤ) :
    a * b * b ^ m = a * b ^ (m + 1) :=
...
theorem tactic.group.gpow_trick_one' {G : Type u_1} [group G] (a : G) (b : G) (n : ℤ) :
    a * b ^ n * b = a * b ^ (n + 1) :=
...
theorem tactic.group.gpow_trick_sub {G : Type u_1} [group G] (a : G) (b : G) (n : ℤ) (m : ℤ) :
    a * b ^ n * b ^ (-m) = a * b ^ (n - m) :=
  eq.mpr
    (id
      (Eq._oldrec (Eq.refl (a * b ^ n * b ^ (-m) = a * b ^ (n - m)))
        (mul_assoc a (b ^ n) (b ^ (-m)))))
    (eq.mpr
      (id
        (Eq._oldrec (Eq.refl (a * (b ^ n * b ^ (-m)) = a * b ^ (n - m)))
          (Eq.symm (gpow_add b n (-m)))))
      (Eq.refl (a * b ^ (n + -m))))
