...
def mk_fintype {α : Type u_1} (enum : α → ℕ) (s : finset_above α enum 0)
    (H : ∀ (x : α), x ∈ subtype.val s) : fintype α :=
  fintype.mk (subtype.val s) H

/-- This is the case for a simple variant (no arguments) in an inductive type. -/
def finset_above.cons {α : Type u_1} {enum : α → ℕ} (n : ℕ) (a : α) (h : enum a = n)
    (s : finset_above α enum (n + 1)) : finset_above α enum n :=
  { val := finset.cons a (subtype.val s) sorry, property := sorry }

theorem finset_above.mem_cons_self {α : Type u_1} {enum : α → ℕ} {n : ℕ} {a : α} {h : enum a = n}
    {s : finset_above α enum (n + 1)} : a ∈ subtype.val (finset_above.cons n a h s) :=
  multiset.mem_cons_self a (finset.val (subtype.val s))

theorem finset_above.mem_cons_of_mem {α : Type u_1} {enum : α → ℕ} {n : ℕ} {a : α} {h : enum a = n}
    {s : finset_above α enum (n + 1)} {b : α} :
    b ∈ subtype.val s → b ∈ subtype.val (finset_above.cons n a h s) :=
...
protected instance finset_above.inhabited (α : Type u_1) (enum : α → ℕ) (n : ℕ) :
    Inhabited (finset_above α enum n) :=
...
variant. -/
def finset_in {α : Type u_1} (P : α → Prop) := Subtype fun (s : finset α) => ∀ (x : α), x ∈ s → P x
...
`enum (C_n a b c ...) = n`. -/
def finset_in.mk {α : Type u_1} {P : α → Prop} (Γ : Type u_2) [fintype Γ] (f : Γ → α)
    (inj : function.injective f) (mem : ∀ (x : Γ), P (f x)) : finset_in P :=
  { val := finset.map (function.embedding.mk f inj) finset.univ, property := sorry }

theorem finset_in.mem_mk {α : Type u_1} {P : α → Prop} {Γ : Type u_2} {s : fintype Γ} {f : Γ → α}
    {inj : function.injective f} {mem : ∀ (x : Γ), P (f x)} {a : α} (b : Γ) (H : f b = a) :
    a ∈ subtype.val (finset_in.mk Γ f inj mem) :=
...
def finset_above.union {α : Type u_1} {enum : α → ℕ} (n : ℕ)
    (s : finset_in fun (a : α) => enum a = n) (t : finset_above α enum (n + 1)) :
    finset_above α enum n :=
  { val := finset.disj_union (subtype.val s) (subtype.val t) sorry, property := sorry }

theorem finset_above.mem_union_left {α : Type u_1} {enum : α → ℕ} {n : ℕ}
    {s : finset_in fun (a : α) => enum a = n} {t : finset_above α enum (n + 1)} {a : α}
    (H : a ∈ subtype.val s) : a ∈ subtype.val (finset_above.union n s t) :=
  iff.mpr multiset.mem_add (Or.inl H)

theorem finset_above.mem_union_right {α : Type u_1} {enum : α → ℕ} {n : ℕ}
    {s : finset_in fun (a : α) => enum a = n} {t : finset_above α enum (n + 1)} {a : α}
    (H : a ∈ subtype.val t) : a ∈ subtype.val (finset_above.union n s t) :=
