...
/-- Preterm evaluation -/
def val (v : ℕ → ℕ) : preterm → ℕ := sorry

@[simp] theorem val_const {v : ℕ → ℕ} {m : ℕ} : val v (cst m) = m := rfl

@[simp] theorem val_var {v : ℕ → ℕ} {m : ℕ} {n : ℕ} : val v (var m n) = m * v n := sorry

@[simp] theorem val_add {v : ℕ → ℕ} {t : preterm} {s : preterm} :
    val v (add t s) = val v t + val v s :=
  rfl

@[simp] theorem val_sub {v : ℕ → ℕ} {t : preterm} {s : preterm} :
    val v (sub t s) = val v t - val v s :=
  rfl

/-- Fresh de Brujin index not used by any variable in argument -/
def fresh_index : preterm → ℕ := sorry

/-- If variable assignments `v` and `w` agree on all variables that occur
in term `t`, the value of `t` under `v` and `w` are identical. -/
theorem val_constant (v : ℕ → ℕ) (w : ℕ → ℕ) (t : preterm) :
    (∀ (x : ℕ), x < fresh_index t → v x = w x) → val v t = val w t :=
  sorry

def repr : preterm → string := sorry

@[simp] def add_one (t : preterm) : preterm := add t (cst 1)

/-- Preterm is free of subtractions -/
def sub_free : preterm → Prop := sorry
...
@[simp] def canonize : preterm → term := sorry

@[simp] theorem val_canonize {v : ℕ → ℕ} {t : preterm} :
    preterm.sub_free t → term.val (fun (x : ℕ) => ↑(v x)) (canonize t) = ↑(preterm.val v t) :=
  sorry
