...
def sub_subst (t : preterm) (s : preterm) (k : ℕ) : preterm → preterm :=
  sorry

theorem val_sub_subst {k : ℕ} {x : preterm} {y : preterm} {v : ℕ → ℕ} {t : preterm} : fresh_index t ≤ k → val (update k (val v x - val v y) v) (sub_subst x y k t) = val v t := sorry
...
@[simp] def sub_subst (x : preterm) (y : preterm) (k : ℕ) : preform → preform :=
  sorry
...
theorem holds_is_diff {t : preterm} {s : preterm} {k : ℕ} {v : ℕ → ℕ} : v k = preterm.val v t - preterm.val v s → preform.holds v (is_diff t s k) := sorry
...
theorem sub_subst_equiv {k : ℕ} {x : preterm} {y : preterm} {v : ℕ → ℕ} (p : preform) : preform.fresh_index p ≤ k →
  (preform.holds (update k (preterm.val v x - preterm.val v y) v) (preform.sub_subst x y k p) ↔ preform.holds v p) := sorry

theorem sat_sub_elim {t : preterm} {s : preterm} {p : preform} : preform.sat p → preform.sat (sub_elim t s p) := sorry

theorem unsat_of_unsat_sub_elim (t : preterm) (s : preterm) (p : preform) : preform.unsat (sub_elim t s p) → preform.unsat p :=
  mt sat_sub_elim
