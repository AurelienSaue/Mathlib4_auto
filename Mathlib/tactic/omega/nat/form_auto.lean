...
@[simp] def holds (v : ℕ → ℕ) : preform → Prop := sorry

end preform


/-- `univ_close p n` := `p` closed by prepending `n` universal quantifiers -/
@[simp] def univ_close (p : preform) : (ℕ → ℕ) → ℕ → Prop := sorry
...
def sub_free : preform → Prop := sorry

/-- Fresh de Brujin index not used by any variable in argument -/
def fresh_index : preform → ℕ := sorry

theorem holds_constant {v : ℕ → ℕ} {w : ℕ → ℕ} (p : preform) :
    (∀ (x : ℕ), x < fresh_index p → v x = w x) → (holds v p ↔ holds w p) :=
  sorry

/-- All valuations satisfy argument -/
def valid (p : preform) := ∀ (v : ℕ → ℕ), holds v p

/-- There exists some valuation that satisfies argument -/
def sat (p : preform) := ∃ (v : ℕ → ℕ), holds v p

/-- `implies p q` := under any valuation, `q` holds if `p` holds -/
def implies (p : preform) (q : preform) := ∀ (v : ℕ → ℕ), holds v p → holds v q

/-- `equiv p q` := under any valuation, `p` holds iff `q` holds -/
def equiv (p : preform) (q : preform) := ∀ (v : ℕ → ℕ), holds v p ↔ holds v q
...
theorem univ_close_of_valid {p : preform} {m : ℕ} {v : ℕ → ℕ} :
