...
def get {α : Type} (p : dir_pair α) : expr_lens.dir → α := sorry

/-- Set one side of the pair, picking the side according to the direction. -/
def set {α : Type} (p : dir_pair α) : expr_lens.dir → α → dir_pair α := sorry

/-- Convert the pair to a list of its elements. -/
def to_list {α : Type} (p : dir_pair α) : List α := [l p, r p]

/-- Convert the pair to a readable string format. -/
def to_string {α : Type} [has_to_string α] (p : dir_pair α) : string :=
  to_string (l p) ++ string.str string.empty (char.of_nat (bit1 (bit0 (bit1 (bit1 (bit0 1)))))) ++
    to_string (r p)
...
def app_addr := _nest_1_1.tactic.rewrite_search.using_conv.app_addr
