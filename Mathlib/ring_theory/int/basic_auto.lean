...
protected instance gcd_monoid : gcd_monoid ℤ :=
  gcd_monoid.mk norm_unit sorry sorry sorry (fun (a b : ℤ) => ↑(gcd a b)) (fun (a b : ℤ) => ↑(lcm a b)) sorry sorry sorry
    sorry sorry sorry sorry

theorem coe_gcd (i : ℤ) (j : ℤ) : ↑(gcd i j) = gcd i j :=
  rfl

theorem coe_lcm (i : ℤ) (j : ℤ) : ↑(lcm i j) = lcm i j :=
  rfl

theorem nat_abs_gcd (i : ℤ) (j : ℤ) : nat_abs (gcd i j) = gcd i j :=
  rfl

theorem nat_abs_lcm (i : ℤ) (j : ℤ) : nat_abs (lcm i j) = lcm i j :=
  rfl
...
theorem sqr_of_gcd_eq_one {a : ℤ} {b : ℤ} {c : ℤ} (h : gcd a b = 1) (heq : a * b = c ^ bit0 1) : ∃ (a0 : ℤ), a = a0 ^ bit0 1 ∨ a = -a0 ^ bit0 1 := sorry

theorem sqr_of_coprime {a : ℤ} {b : ℤ} {c : ℤ} (h : is_coprime a b) (heq : a * b = c ^ bit0 1) : ∃ (a0 : ℤ), a = a0 ^ bit0 1 ∨ a = -a0 ^ bit0 1 :=
...
def associates_int_equiv_nat : associates ℤ ≃ ℕ :=
  equiv.mk (fun (z : associates ℤ) => int.nat_abs (associates.out z)) (fun (n : ℕ) => associates.mk ↑n) sorry sorry

theorem int.prime.dvd_mul {m : ℤ} {n : ℤ} {p : ℕ} (hp : nat.prime p) (h : ↑p ∣ m * n) : p ∣ int.nat_abs m ∨ p ∣ int.nat_abs n :=
  iff.mp (nat.prime.dvd_mul hp)
    (eq.mpr (id (Eq._oldrec (Eq.refl (p ∣ int.nat_abs m * int.nat_abs n)) (Eq.symm (int.nat_abs_mul m n))))
...
theorem int.prime.dvd_pow {n : ℤ} {k : ℕ} {p : ℕ} (hp : nat.prime p) (h : ↑p ∣ n ^ k) : p ∣ int.nat_abs n :=
...
theorem int.prime.dvd_pow' {n : ℤ} {k : ℕ} {p : ℕ} (hp : nat.prime p) (h : ↑p ∣ n ^ k) : ↑p ∣ n :=
  eq.mpr (id (Eq._oldrec (Eq.refl (↑p ∣ n)) (propext int.coe_nat_dvd_left))) (int.prime.dvd_pow hp h)

theorem prime_two_or_dvd_of_dvd_two_mul_pow_self_two {m : ℤ} {p : ℕ} (hp : nat.prime p) (h : ↑p ∣ bit0 1 * m ^ bit0 1) : p = bit0 1 ∨ p ∣ int.nat_abs m := sorry
...
theorem nat.factors_multiset_prod_of_irreducible {s : multiset ℕ} (h : ∀ (x : ℕ), x ∈ s → irreducible x) : unique_factorization_monoid.factors (multiset.prod s) = s := sorry

namespace multiplicity


theorem finite_int_iff_nat_abs_finite {a : ℤ} {b : ℤ} : finite a b ↔ finite (int.nat_abs a) (int.nat_abs b) := sorry
