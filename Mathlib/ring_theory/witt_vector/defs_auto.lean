...
def witt_vector (p : ℕ) (R : Type u_1) := ℕ → R
...
def mk (p : ℕ) {R : Type u_1} (x : ℕ → R) : witt_vector p R := x
...
-/
def coeff {p : ℕ} {R : Type u_1} (x : witt_vector p R) (n : ℕ) : R := x n

theorem ext {p : ℕ} {R : Type u_1} {x : witt_vector p R} {y : witt_vector p R}
    (h : ∀ (n : ℕ), coeff x n = coeff y n) : x = y :=
  funext fun (n : ℕ) => h n

theorem ext_iff {p : ℕ} {R : Type u_1} {x : witt_vector p R} {y : witt_vector p R} :
    x = y ↔ ∀ (n : ℕ), coeff x n = coeff y n :=
  { mp :=
      fun (h : x = y) (n : ℕ) =>
        eq.mpr (id (Eq._oldrec (Eq.refl (coeff x n = coeff y n)) h)) (Eq.refl (coeff y n)),
    mpr := ext }

@[simp] theorem coeff_mk (p : ℕ) {R : Type u_1} (x : ℕ → R) : coeff (mk p x) = x := rfl
...
def peval {R : Type u_1} [comm_ring R] {k : ℕ} (φ : mv_polynomial (fin k × ℕ) ℤ)
    (x : fin k → ℕ → R) : R :=
...
-/
def eval {p : ℕ} {R : Type u_1} [comm_ring R] {k : ℕ} (φ : ℕ → mv_polynomial (fin k × ℕ) ℤ)
    (x : fin k → witt_vector p R) : witt_vector p R :=
  mk p fun (n : ℕ) => peval (φ n) fun (i : fin k) => coeff (x i)

protected instance has_zero {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    HasZero (witt_vector p R) :=
  { zero := eval (witt_zero p) matrix.vec_empty }

protected instance has_one {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    HasOne (witt_vector p R) :=
  { one := eval (witt_one p) matrix.vec_empty }

protected instance has_add {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    Add (witt_vector p R) :=
  { add :=
      fun (x y : witt_vector p R) =>
        eval (witt_add p) (matrix.vec_cons x (matrix.vec_cons y matrix.vec_empty)) }

protected instance has_sub {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    Sub (witt_vector p R) :=
  { sub :=
      fun (x y : witt_vector p R) =>
        eval (witt_sub p) (matrix.vec_cons x (matrix.vec_cons y matrix.vec_empty)) }

protected instance has_mul {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    Mul (witt_vector p R) :=
  { mul :=
      fun (x y : witt_vector p R) =>
        eval (witt_mul p) (matrix.vec_cons x (matrix.vec_cons y matrix.vec_empty)) }

protected instance has_neg {p : ℕ} (R : Type u_1) [comm_ring R] [fact (nat.prime p)] :
    Neg (witt_vector p R) :=
  { neg := fun (x : witt_vector p R) => eval (witt_neg p) (matrix.vec_cons x matrix.vec_empty) }

@[simp] theorem witt_zero_eq_zero (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) : witt_zero p n = 0 :=
  sorry

@[simp] theorem witt_one_zero_eq_one (p : ℕ) [hp : fact (nat.prime p)] : witt_one p 0 = 1 := sorry

@[simp] theorem witt_one_pos_eq_zero (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) (hn : 0 < n) :
    witt_one p n = 0 :=
  sorry

@[simp] theorem witt_add_zero (p : ℕ) [hp : fact (nat.prime p)] :
    witt_add p 0 = mv_polynomial.X (0, 0) + mv_polynomial.X (1, 0) :=
  sorry

@[simp] theorem witt_sub_zero (p : ℕ) [hp : fact (nat.prime p)] :
    witt_sub p 0 = mv_polynomial.X (0, 0) - mv_polynomial.X (1, 0) :=
  sorry

@[simp] theorem witt_mul_zero (p : ℕ) [hp : fact (nat.prime p)] :
    witt_mul p 0 = mv_polynomial.X (0, 0) * mv_polynomial.X (1, 0) :=
  sorry

@[simp] theorem witt_neg_zero (p : ℕ) [hp : fact (nat.prime p)] :
    witt_neg p 0 = -mv_polynomial.X (0, 0) :=
  sorry

@[simp] theorem constant_coeff_witt_add (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    coe_fn mv_polynomial.constant_coeff (witt_add p n) = 0 :=
  sorry

@[simp] theorem constant_coeff_witt_sub (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    coe_fn mv_polynomial.constant_coeff (witt_sub p n) = 0 :=
  sorry

@[simp] theorem constant_coeff_witt_mul (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    coe_fn mv_polynomial.constant_coeff (witt_mul p n) = 0 :=
  sorry

@[simp] theorem constant_coeff_witt_neg (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    coe_fn mv_polynomial.constant_coeff (witt_neg p n) = 0 :=
  sorry

@[simp] theorem zero_coeff (p : ℕ) (R : Type u_1) [hp : fact (nat.prime p)] [comm_ring R] (n : ℕ) :
    coeff 0 n = 0 :=
  sorry

@[simp] theorem one_coeff_zero (p : ℕ) (R : Type u_1) [hp : fact (nat.prime p)] [comm_ring R] :
    coeff 1 0 = 1 :=
  sorry

@[simp] theorem one_coeff_eq_of_pos (p : ℕ) (R : Type u_1) [hp : fact (nat.prime p)] [comm_ring R]
    (n : ℕ) (hn : 0 < n) : coeff 1 n = 0 :=
  sorry

theorem add_coeff {p : ℕ} {R : Type u_1} [hp : fact (nat.prime p)] [comm_ring R]
...
theorem sub_coeff {p : ℕ} {R : Type u_1} [hp : fact (nat.prime p)] [comm_ring R]
    (x : witt_vector p R) (y : witt_vector p R) (n : ℕ) :
    coeff (x - y) n =
        peval (witt_sub p n)
          (matrix.vec_cons (coeff x) (matrix.vec_cons (coeff y) matrix.vec_empty)) :=
...
theorem mul_coeff {p : ℕ} {R : Type u_1} [hp : fact (nat.prime p)] [comm_ring R]
    (x : witt_vector p R) (y : witt_vector p R) (n : ℕ) :
    coeff (x * y) n =
        peval (witt_mul p n)
          (matrix.vec_cons (coeff x) (matrix.vec_cons (coeff y) matrix.vec_empty)) :=
...
theorem neg_coeff {p : ℕ} {R : Type u_1} [hp : fact (nat.prime p)] [comm_ring R]
    (x : witt_vector p R) (n : ℕ) :
    coeff (-x) n = peval (witt_neg p n) (matrix.vec_cons (coeff x) matrix.vec_empty) :=
  rfl

theorem witt_add_vars (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    mv_polynomial.vars (witt_add p n) ⊆ finset.product finset.univ (finset.range (n + 1)) :=
  witt_structure_int_vars p (mv_polynomial.X 0 + mv_polynomial.X 1) n

theorem witt_sub_vars (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    mv_polynomial.vars (witt_sub p n) ⊆ finset.product finset.univ (finset.range (n + 1)) :=
  witt_structure_int_vars p (mv_polynomial.X 0 - mv_polynomial.X 1) n

theorem witt_mul_vars (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    mv_polynomial.vars (witt_mul p n) ⊆ finset.product finset.univ (finset.range (n + 1)) :=
  witt_structure_int_vars p (mv_polynomial.X 0 * mv_polynomial.X 1) n

theorem witt_neg_vars (p : ℕ) [hp : fact (nat.prime p)] (n : ℕ) :
    mv_polynomial.vars (witt_neg p n) ⊆ finset.product finset.univ (finset.range (n + 1)) :=
