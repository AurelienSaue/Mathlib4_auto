...
@[simp] theorem aleph_idx.initial_seg_coe : ⇑aleph_idx.initial_seg = aleph_idx :=
  rfl
...
@[simp] theorem aleph_idx_le {a : cardinal} {b : cardinal} : aleph_idx a ≤ aleph_idx b ↔ a ≤ b := sorry

theorem aleph_idx.init {a : cardinal} {b : ordinal} : b < aleph_idx a → ∃ (c : cardinal), aleph_idx c = b :=
...
/-- The `aleph'` index function, which gives the ordinal index of a cardinal.
  ...
  For the basic function version, see `aleph_idx`. -/
def aleph_idx.rel_iso : Less ≃r Less :=
  rel_iso.of_surjective ↑aleph_idx.initial_seg sorry

@[simp] theorem aleph_idx.rel_iso_coe : ⇑aleph_idx.rel_iso = aleph_idx :=
  rfl
...
def aleph'.rel_iso : Less ≃r Less :=
  rel_iso.symm aleph_idx.rel_iso
...
@[simp] theorem aleph'.rel_iso_coe : ⇑aleph'.rel_iso = aleph' :=
  rfl
...
theorem aleph'_le_of_limit {o : ordinal} (l : ordinal.is_limit o) {c : cardinal} : aleph' o ≤ c ↔ ∀ (o' : ordinal), o' < o → aleph' o' ≤ c := sorry
...
theorem omega_le_aleph' {o : ordinal} : omega ≤ aleph' o ↔ ordinal.omega ≤ o :=
  eq.mpr (id (Eq._oldrec (Eq.refl (omega ≤ aleph' o ↔ ordinal.omega ≤ o)) (Eq.symm aleph'_omega)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (aleph' ordinal.omega ≤ aleph' o ↔ ordinal.omega ≤ o)) (propext aleph'_le)))
...
theorem omega_le_aleph (o : ordinal) : omega ≤ aleph o :=
  eq.mpr (id (Eq._oldrec (Eq.refl (omega ≤ aleph o)) (aleph.equations._eqn_1 o)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (omega ≤ aleph' (ordinal.omega + o))) (propext omega_le_aleph')))
...
theorem mul_eq_max {a : cardinal} {b : cardinal} (ha : omega ≤ a) (hb : omega ≤ b) : a * b = max a b := sorry

theorem mul_lt_of_lt {a : cardinal} {b : cardinal} {c : cardinal} (hc : omega ≤ c) (h1 : a < c) (h2 : b < c) : a * b < c := sorry

theorem mul_le_max_of_omega_le_left {a : cardinal} {b : cardinal} (h : omega ≤ a) : a * b ≤ max a b := sorry

theorem mul_eq_max_of_omega_le_left {a : cardinal} {b : cardinal} (h : omega ≤ a) (h' : b ≠ 0) : a * b = max a b := sorry

theorem mul_eq_left {a : cardinal} {b : cardinal} (ha : omega ≤ a) (hb : b ≤ a) (hb' : b ≠ 0) : a * b = a :=
...
theorem mul_eq_right {a : cardinal} {b : cardinal} (hb : omega ≤ b) (ha : a ≤ b) (ha' : a ≠ 0) : a * b = b :=
...
theorem mul_eq_left_iff {a : cardinal} {b : cardinal} : a * b = a ↔ max omega b ≤ a ∧ b ≠ 0 ∨ b = 1 ∨ a = 0 := sorry
...
theorem add_eq_max {a : cardinal} {b : cardinal} (ha : omega ≤ a) : a + b = max a b :=
  le_antisymm (add_eq_self (le_trans ha (le_max_left a b)) ▸ add_le_add (le_max_left a b) (le_max_right a b))
    (max_le (self_le_add_right a b) (self_le_add_left b a))

theorem add_lt_of_lt {a : cardinal} {b : cardinal} {c : cardinal} (hc : omega ≤ c) (h1 : a < c) (h2 : b < c) : a + b < c := sorry

theorem eq_of_add_eq_of_omega_le {a : cardinal} {b : cardinal} {c : cardinal} (h : a + b = c) (ha : a < c) (hc : omega ≤ c) : b = c := sorry
...
theorem add_eq_right_iff {a : cardinal} {b : cardinal} : a + b = b ↔ max omega a ≤ b ∨ a = 0 :=
  eq.mpr (id (Eq._oldrec (Eq.refl (a + b = b ↔ max omega a ≤ b ∨ a = 0)) (add_comm a b)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (b + a = b ↔ max omega a ≤ b ∨ a = 0)) (propext add_eq_left_iff)))
...
protected theorem eq_of_add_eq_add_left {a : cardinal} {b : cardinal} {c : cardinal} (h : a + b = a + c) (ha : a < omega) : b = c := sorry

protected theorem eq_of_add_eq_add_right {a : cardinal} {b : cardinal} {c : cardinal} (h : a + b = c + b) (hb : b < omega) : a = c := sorry
...
theorem mk_bounded_set_le_of_omega_le (α : Type u) (c : cardinal) (hα : omega ≤ mk α) : mk (Subtype fun (t : set α) => mk ↥t ≤ c) ≤ mk α ^ c := sorry

theorem mk_bounded_set_le (α : Type u) (c : cardinal) : mk (Subtype fun (t : set α) => mk ↥t ≤ c) ≤ max (mk α) omega ^ c := sorry

theorem mk_bounded_subset_le {α : Type u} (s : set α) (c : cardinal) : mk (Subtype fun (t : set α) => t ⊆ s ∧ mk ↥t ≤ c) ≤ max (mk ↥s) omega ^ c := sorry

/-! ### Properties of `compl` -/

theorem mk_compl_of_omega_le {α : Type u_1} (s : set α) (h : omega ≤ mk α) (h2 : mk ↥s < mk α) : mk ↥(sᶜ) = mk α :=
  eq_of_add_eq_of_omega_le (mk_sum_compl s) h2 h

theorem mk_compl_finset_of_omega_le {α : Type u_1} (s : finset α) (h : omega ≤ mk α) : mk ↥(↑sᶜ) = mk α :=
  mk_compl_of_omega_le (↑s) h (lt_of_lt_of_le (finset_card_lt_omega s) h)

theorem mk_compl_eq_mk_compl_infinite {α : Type u_1} {s : set α} {t : set α} (h : omega ≤ mk α) (hs : mk ↥s < mk α) (ht : mk ↥t < mk α) : mk ↥(sᶜ) = mk ↥(tᶜ) :=
  eq.mpr (id (Eq._oldrec (Eq.refl (mk ↥(sᶜ) = mk ↥(tᶜ))) (mk_compl_of_omega_le s h hs)))
    (eq.mpr (id (Eq._oldrec (Eq.refl (mk α = mk ↥(tᶜ))) (mk_compl_of_omega_le t h ht))) (Eq.refl (mk α)))

theorem mk_compl_eq_mk_compl_finite_lift {α : Type u} {β : Type v} {s : set α} {t : set β} (hα : mk α < omega) (h1 : lift (mk α) = lift (mk β)) (h2 : lift (mk ↥s) = lift (mk ↥t)) : lift (mk ↥(sᶜ)) = lift (mk ↥(tᶜ)) := sorry

theorem mk_compl_eq_mk_compl_finite {α : Type u} {β : Type u} {s : set α} {t : set β} (hα : mk α < omega) (h1 : mk α = mk β) (h : mk ↥s = mk ↥t) : mk ↥(sᶜ) = mk ↥(tᶜ) := sorry

theorem mk_compl_eq_mk_compl_finite_same {α : Type u_1} {s : set α} {t : set α} (hα : mk α < omega) (h : mk ↥s = mk ↥t) : mk ↥(sᶜ) = mk ↥(tᶜ) :=
  mk_compl_eq_mk_compl_finite hα rfl h

/-! ### Extending an injection to an equiv -/

theorem extend_function {α : Type u_1} {β : Type u_2} {s : set α} (f : ↥s ↪ β) (h : Nonempty (↥(sᶜ) ≃ ↥(set.range ⇑fᶜ))) : ∃ (g : α ≃ β), ∀ (x : ↥s), coe_fn g ↑x = coe_fn f x := sorry

theorem extend_function_finite {α : Type u_1} {β : Type u_2} {s : set α} (f : ↥s ↪ β) (hs : mk α < omega) (h : Nonempty (α ≃ β)) : ∃ (g : α ≃ β), ∀ (x : ↥s), coe_fn g ↑x = coe_fn f x := sorry

theorem extend_function_of_lt {α : Type u_1} {β : Type u_2} {s : set α} (f : ↥s ↪ β) (hs : mk ↥s < mk α) (h : Nonempty (α ≃ β)) : ∃ (g : α ≃ β), ∀ (x : ↥s), coe_fn g ↑x = coe_fn f x := sorry
...
@[simp] theorem one_le_bit1 (a : cardinal) : 1 ≤ bit1 a :=
  self_le_add_left 1 (bit0 a)
...
@[simp] theorem bit1_le_bit1 {a : cardinal} {b : cardinal} : bit1 a ≤ bit1 b ↔ a ≤ b :=
  { mp := fun (h : bit1 a ≤ bit1 b) => iff.mp bit0_le_bit1 (le_trans (self_le_add_right (bit0 a) 1) h),
    mpr :=
      fun (h : a ≤ b) => le_trans (add_le_add_right (add_le_add_left h a) 1) (add_le_add_right (add_le_add_right h b) 1) }

@[simp] theorem bit1_le_bit0 {a : cardinal} {b : cardinal} : bit1 a ≤ bit0 b ↔ a < b ∨ a ≤ b ∧ omega ≤ a := sorry
...
@[simp] theorem bit0_lt_bit1 {a : cardinal} {b : cardinal} : bit0 a < bit1 b ↔ a < b ∨ a ≤ b ∧ a < omega := sorry
